...
- 2026-09-16 H480 send from a script (partial): `nerva send` now takes its body Hermes's way — the MESSAGE argument, else `-f PATH` (`-` reads stdin), else whatever is piped on stdin, and a terminal is never read; `-s LINE` is a one-line subject that ntfy carries as its native title and every other channel as the first line (subject, blank line, message — Hermes's shape); `-q` prints nothing on success; `--list CHANNEL` filters the target list — a name that can never be a direct-send channel is a usage error decided offline, and a channel an older hub does not list is exit 1 with what it does list named; a blank MESSAGE argument counts as no message and falls through to `--file`/stdin. The exit ladder is stated in `--help`: 0 sent · 1 not delivered · 2 usage · 3 no hub · 4 not authorised. A binary `--file` is refused with the attachment guidance and its bytes are never echoed; the 4,000-character bound counts the subject and is enforced before any request. `POST /api/channels/send` gains an optional one-line `subject` (≤200) and the outbound seam applies the bound to what is actually delivered. 11 CLI + 5 outbound/route tests, each red-proofed by neutering its behaviour; the real `ChannelManager` was probed to admit the ntfy `title` kwarg. Remaining, named in the row: `MEDIA:<path>` attachments (file egress needs a kernel action and a confined source directory before a route may read local paths), no-gateway operation (Nerva's send needs the hub for the IntentLog and rate limit — a documented divergence), bodies over 4,000 characters are refused rather than chunked, Slack/Discord stay reply-only (H018), and the `JobRunner._send` duplication is left to the jobs lane.
