...
| AUD-10 | 🟢 **partial in #324** — **Supply-chain / CI**. **Done:** all 36 `uses:` across the 8 workflows SHA-pinned (`@<40-hex>  # vX`, Dependabot-tracked, F32); pytest-socket loopback-only guard in `pytest.ini` (`--allow-hosts=127.0.0.1,::1,localhost`) so a stray *real* network call fails fast instead of hanging to the `--timeout` backstop (F37); `.pre-commit-config.yaml` (gitleaks/ruff/hygiene) + advisory gitleaks CI lane (`security.yml`, F36); advisory SAST (semgrep) + dependency-audit (pip-audit) CI lanes (`security.yml`, continue-on-error, F34/F35). **Remaining (follow-up, needs a CI-connected index):** Python lockfile + `--require-hashes` (F33 — *blocked from the sandbox:* the reachable mirror tops out at `numpy 2.4.6` vs the pinned `numpy>=2.5.0`, so `pip-compile` can't resolve the graph; must be generated where the real index lives); flip the lint/CVE/secret lanes advisory → **blocking** with baselines — ruff + bandit blocking, `pip-audit` ignore-list, gitleaks→blocking (F34/F35, baseline-then-block). *Extends Dependency-upkeep + SEC-4 + CQ sections.* | 5 | P1 | every `uses:` is a SHA ✅; stray network in tests fails fast ✅; CI fails on lint/CVE/secret (remaining) |
