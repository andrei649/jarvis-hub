...
| BUG-2 🚧 | Frontend test infrastructure missing — 0% coverage on React HUD (~6 000 LOC: `app.js`, `admin.js`, `systems.js`, `workflows.js`, etc). No test runner (jest/vitest), no React Testing Library, no E2E (Playwright/Cypress). **In progress 2026-06-02:** Vitest + JSDOM harness added (`tests/frontend/`, `package.json`, `vitest.config.js`) that loads the real shipped global scripts (vendored React 18 UMD + static files) — no bundler/build step needed. 42 tests green: `components.js` helpers + `Bracket`/`StatusDot`, `i18n.js`, `admin.js` form rows (Toggle/Input/Select/Slider/TagInput/Button/Info/Group), `systems.js` (`SystemsTabBar` + `FusedRecallBox`). Wired into CI (`frontend` job). **Caught a real shipped bug on first run:** `systems.js` `ResilienceTab` was missing its closing brace → the entire Systems panel failed to parse/load in the browser (present on `main`); fixed in the same PR. Remaining: rest of panels (`workflows`/`network`/`observability`/`cognition`, remaining `systems`/`admin` tabs), `data.js`, `app.js`, and E2E. See `tests/frontend/README.md`. | MEDIUM | Identified in test coverage audit 2026-06-02; backend gap closed (121 tests added on branch `claude/hud-human-interface-testing-r8IQS`) |
