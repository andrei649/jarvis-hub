...
| **STAB-4 Governed email replies** | **P0** | **#869 merged 2026-08-12 — two of the four original conditions remain open.** Delivered: the external effect routes through the governed `channel.reply` executor (`send_channel_reply` is a separate transport seam; generic `ChannelManager.send()` excludes email, so inbound mail cannot auto-trigger SMTP), and default-recipient leakage is blocked because the `channel_reply` contract's `reply_target_present` predicate requires `to` for email (the `default_recipient` fallback in `EmailChannel.send` is unreachable on that path). **Still open:** no paired/allowlisted-sender check and no reply rate-limit anywhere in `channel_reply.py` / `channels/manager.py`, so any inbound sender that opens an inbox thread can cause outbound mail once approved, uncapped. | End-to-end IMAP → orchestrator → kernel/channel → SMTP tests prove the actual sender is used, default-recipient leakage cannot occur, unknown senders cannot trigger outbound mail, and audit/approval policy is preserved. |
...
<!-- AUDIT-2026-08-10:AI-DEV-EFFICIENCY:START -->
## AI development efficiency backlog — 2026-08-10 audit

> **Goal:** shorten idea-to-trusted-merge time without weakening authority, security, or evidence.
> The current rules and automation are individually well-intentioned, but they have accumulated
> contradictory control loops and expensive feedback paths. This PR is a representative case: a
> BACKLOG-only change launched the broad Ubuntu/Windows, security, CodeQL, frontend, HUD, OpenAPI,
> sandbox and signal lanes, then the full Python lane reported a generated `project-status.json`
> drift that could have been identified in seconds.
>
> **Observed rule debt:** `AGENTS.md` requires branch + draft-PR delivery while
> `PARALLEL_WORKFLOW.md` still tells Claude to push directly to `main`; the latter also admits its
> local lock file does not synchronize across machines. OpenCode's methodology still rebases from
> `master`, forces two-to-five-minute tasks and pushes every unit, while `.opencode/summary.md` and
> `docs/SPRINT.md` retain obsolete branch/session state. `BACKLOG.md` plus `docs/SPRINT.md` exceed
> 620 KB, so loading both for every task is itself a material context and latency tax.
>
> **Ordering:** AI-EFF-0 through AI-EFF-5 are the control-plane prerequisites and may proceed with
> the matching STAB rows. Later optimizations must not create a new merge bypass or treat a faster
> untrusted result as accepted evidence.

| ID | Priority | Improvement | Acceptance evidence |
|---|---:|---|---|
| **AI-EFF-0 One executable workflow contract** | **P0** | Consolidate `AGENTS.md`, `PARALLEL_WORKFLOW.md`, `.opencode/plans/dev-methodology.md` and conductor guidance into one canonical policy. Archive or generate the others; remove `master`, direct-to-`main`, fixed-vendor order and obsolete session instructions. | A repository check finds one authority source, one default branch and no contradictory push/merge rule; every supported agent receives the same generated preflight capsule. |
| **AI-EFF-1 Independent integration + merge queue** | **P0** | Implement STAB-1 with separate builder, reviewer and integrator identities and GitHub's protected merge queue instead of the hourly shell loop. Builders can branch and update draft PRs but cannot approve, attest or merge their own head. | Hostile tests prove self-merge, stale approval, builder-token bypass and direct/API merge fail; exact-head acceptance is required and queue latency is measured. |
| **AI-EFF-2 Protected exception control plane** | **P0** | Replace candidate-editable PR-body controls such as `unpark:` and the Nerva movement marker with protected labels, environments or integrator-owned attestations. Treat body text as description only. | Editing a title/body cannot change merge eligibility, parked status or security scope; every exception records actor, policy, head SHA, reason and expiry. |
| **AI-EFF-3 Central change-risk classifier** | **P0** | Add one reviewed job that classifies the exact diff into docs-only, generated truth, Python runtime, frontend/mobile, contracts/security, workflow and dependency scopes. All workflows consume the same immutable classification instead of maintaining divergent path filters. | Classification fixtures cover renames, generated files, workflow edits and dependency locks; a stable required-check aggregator reports skipped lanes correctly; docs-only PRs run only the bounded docs/secrets/policy set. |
| **AI-EFF-4 Two-stage CI feedback** | **P0** | Run a fast gate first: scope, formatting, focused lint/types, manifest/generated drift and impacted unit tests. Start expensive OS matrices, security suites and browser tests only after fast green and when the risk class, ready-for-review state or protected label requires them. | Median time-to-first-actionable-result is under two minutes and p95 under five; full required coverage remains green on ready PRs and `main`; cancelled superseded heads stop consuming runners. |
| **AI-EFF-5 Single generated-status architecture** | **P0** | Make one machine ledger authoritative and derive human views without running full Python/JS suites or embedding a self-referential commit stamp. Move status drift out of `test_release_gate.py` into the fast gate and reconcile the script's current “non-blocking” documentation with actual policy. | `status_sync --check` is deterministic and sub-minute; one source change cannot require manual edits to five surfaces; generated output is byte-clean and the release gate consumes the same ledger. |
| **AI-EFF-6 Local/CI parity preflight** | **P1** | Provide a single `agent preflight --changed` entry point that runs the exact fast-gate commands for the current diff and emits both readable and JSON results. Agents run it before the first push and attach its receipt to the PR. | Command hashes/tool versions match CI; failures include exact reproduction and safe fix commands; a clean receipt is bound to the tested head and diff. |
| **AI-EFF-7 Test-impact selection with a full-suite backstop** | **P1** | Maintain a reviewed module/route/contract-to-test map and select focused tests from the diff. Keep full Ubuntu/Windows and integration suites for high-risk paths, ready-for-review, `main` and nightly runs rather than every prose change. | Mutation/fixture tests prove important dependants are selected; missed-impact rate and escape rate stay within an owner-set budget; nightly full runs detect map gaps and open repair issues. |
| **AI-EFF-8 Cross-machine path-intent leases** | **P1** | Replace local-only `lock.py` and blanket “all draft-PR files are read-only” rules with GitHub-backed path-intent leases. Use exact base/head, owner, paths, TTL and heartbeat; warn on semantic overlap and allow recorded transfer. | Two machines observe the same lease; active work cannot expire silently; stale leases are recoverable; an overlap bot blocks only conflicting paths instead of unrelated work. |
| **AI-EFF-9 Capability-based agent routing** | **P1** | Route tasks by measured role and risk — planning/synthesis, implementation, cheap local iteration, security critic and independent integration — rather than permanent OpenCode/Claude file ownership. Use parallel specialists only for separable outputs and one accountable conductor for synthesis. | Task cards state role, scope, evidence and stop conditions; benchmarked routing beats the baseline on cycle time/rework; no agent reviews or integrates its own security-sensitive result. |
| **AI-EFF-10 Bounded context capsules** | **P1** | Generate a small task-context artifact from the canonical ledger: objective, current head, active dependencies, relevant decisions, file ownership, commands and open evidence. Load targeted source sections on demand instead of injecting the 560 KB backlog, stale sprint log and anchored summaries into every session. | Capsule is freshness-hashed, under an owner-set token budget and rejects stale base/head; context-load latency and contradictory-instruction incidents fall measurably. |
| **AI-EFF-11 Plan/document lifecycle metadata** | **P1** | Give every plan/spec `status`, owner, issue/PR, dependency, accepted head and `supersedes/superseded_by` metadata. Archive delivered templates and distinguish proposal, active, parked, accepted and historical evidence. | A checker rejects two active canonical plans for one scope, expired ownerless work and unchecked historical templates presented as open work; search defaults to active records. |
| **AI-EFF-12 Coherent iteration and CI debounce** | **P1** | Replace forced two-to-five-minute task units plus push-every-unit with risk-sized rollback units. Permit local checkpoints, then push one coherent tested change; debounce/cancel CI for rapidly superseded heads. | PRs remain small and reviewable while pushes-per-accepted-change, duplicate runner minutes and rebase churn decrease; emergency bisect/revert remains practical. |
| **AI-EFF-13 Delta-based code-health gate** | **P1** | Run `scripts/code_health.py` once, render all views from that result and block only new complexity, dead-code and `@ts-nocheck` debt above an accepted baseline. Keep legacy debt visible with owners rather than rerunning the analyzer or making all findings advisory. | One analyzer invocation produces text/JSON/SARIF; zero unowned baseline growth; each accepted exception is scoped and expiring. |
| **AI-EFF-14 Deterministic, cached bootstrap** | **P1** | Replace session-start `npm install` with lockfile-keyed cache validation and cold-path `npm ci`; expose one pinned bootstrap command for Python, frontend, mobile and Playwright tools. Never mutate dependencies merely because an AI session starts or compacts. | Warm start performs no install and meets a latency budget; cold start is reproducible from locks; toolchain provenance is recorded and dependency drift fails closed. |
| **AI-EFF-15 Layered browser/E2E loop** | **P1** | Add a cheap backend readiness and HUD-mount smoke before Playwright, cache browser binaries by version and shard the bounded PR suite. Reserve browser matrix, repetitions, soak and failure injection for nightly/RC unless the change class requires them. | Boot failures return logs before browser install; PR E2E p95 and flake rate meet budgets; full nightly coverage and artifacts remain intact. |
| **AI-EFF-16 Structured failure router** | **P1** | Classify failures as source, generated truth, policy, dependency, infrastructure or known flake and post the shortest exact reproduction path on the PR. Automatically rerun at most one signature-matched infrastructure flake; never rerun semantic/security failures into green. | Every red check has owner, category and next command; mean time-to-diagnosis falls; rerun rate and false-green rate are tracked. |
| **AI-EFF-17 Risk-budgeted independent critique** | **P1** | Require structured planner/implementer/critic disagreement for authority, security, privacy, supply-chain and destructive-effect changes. Use one bounded reviewer for docs and low-risk mechanical work; default to at most two fix/re-review rounds before park or owner escalation. | Policy maps risk class to reviewer count, loop cap and evidence; high-risk changes retain exact-head independent acceptance while low-risk review latency and reviewer ping-pong drop. |
| **AI-EFF-18 Serialized truth integrator** | **P2** | Let a dedicated bot/integrator regenerate status, manifests and documentation views at an exact accepted base after semantic PRs are green, rather than making every agent edit shared generated surfaces. Serialize only this narrow lane. | Semantic PRs avoid generated-file conflicts; regeneration is deterministic, separately reviewed when policy changes and automatically rebases on the accepted head. |
| **AI-EFF-19 Development-system telemetry** | **P2** | Measure time to first signal, push-to-green, review wait, rebase/conflict rate, stale approvals, reruns/flakes, wasted docs-only runner minutes, AI proposal acceptance/rework/revert rate and owner-gate age. Set budgets before optimizing further. | A weekly digest uses workflow/PR facts, not self-reported agent claims; regressions create bounded backlog items with owner and expiry. |
| **AI-EFF-20 Consumer-before-contract gate** | **P2** | Stop counting additional schemas, ledgers or design documents as delivery unless an identified runtime/evaluation consumer and evidence owner exist. Require reuse/extension review before creating another validator or truth surface. | New contracts name consumer, decision changed, test/evidence path and retirement condition; orphan or duplicate contracts are parked/archived and module count trends down. |
| **AI-EFF-21 Directly collected Nerva tests** | **P1** | Complete STAB-6 by converting the ten private `_nerva_*_checks.py` mega-suites into parametrized pytest nodes instead of invoking thousands of assertions through unrelated legacy tests. Give each contract/scenario a stable case ID and duration. | Every scenario is directly discoverable and shardable; no wrapper node exceeds 60 seconds; a failure names the exact contract and xdist shard imbalance stays within budget. |
| **AI-EFF-22 Shared setup, complete cache keys and cancellation** | **P1** | Reuse one pinned setup action or wheelhouse across workflows, key caches on OS, runtime and every applicable lockfile, and pin container images by digest. Add concurrency cancellation to Smoke, Nerva Roadmap, Park Guard and Code Health as well as the primary lanes. | Unchanged-lock cache hit exceeds 90%; setup stays below 20% of job time; obsolete heads cancel within 15 seconds and consume less than one additional runner-minute. |
| **AI-EFF-23 Honest security-check semantics** | **P1** | Stop treating a `continue-on-error` CodeQL analysis as green acceptance evidence. Until enforceable CodeQL is available, label it advisory and make a locally evaluated, fail-closed Semgrep/Bandit/secrets result the required SAST signal. | A vulnerable fixture makes the required security check red; scanner failure is distinguishable from a clean scan and can never be presented as security acceptance. |
| **AI-EFF-24 Hermetic DNS and warning budget** | **P1** | Patch DNS/socket access at the resolver boundary in hermetic tests instead of allowing public ING, Tavily or example.com resolution attempts before connection blocking. Baseline current warnings and reject only new warning classes/count growth. | CI performs zero non-loopback DNS/connect attempts; no external hostname leaks from tests; warning debt cannot grow and known deprecations have bounded owners. |
| **AI-EFF-25 Machine-checkable PR evidence receipt** | **P1** | Replace the seven-line generic PR template with a schema containing change class, issue/dependencies, exact head, affected surfaces, authority impact, rollback, generated state, commands/results, owner-live limits and non-claims. Let changed paths raise but never lower the declared risk class. | CI validates the receipt and binds it to the current head; ready-for-review is blocked on missing/stale evidence; low-risk docs use a bounded receipt while authority/release changes require full integration evidence. |
| **AI-EFF-26 Orthogonal development state machine** | **P1** | Stop overloading draft status to mean WIP, lock, parked work and merge exclusion. Track delivery state, CI state, governance decision and path lease separately with explicit planned, claimed, implementing, review, ready, blocked, parked, superseded and merged transitions. | Every transition records owner, exact head, time, blocker and next action; event-driven automation can resume safely; no state word silently grants merge authority. |
| **AI-EFF-27 Reusable SHA-bound verification receipts** | **P2** | Cache trustworthy verification by command, head SHA, dirty-tree digest, toolchain/environment fingerprint and artifact. Reuse it while those inputs are identical instead of rerunning unchanged suites merely because an agent handed off or a conversation compacted. | Relevant diffs invalidate the receipt automatically; local and hosted consumers verify its provenance; repeated unchanged handoffs do not rerun expensive checks and never reuse evidence from another head. |

<!-- AUDIT-2026-08-10:AI-DEV-EFFICIENCY:END -->
...
| H23.4 | Promote **eval/regression harness to a pre-release gate** | 🟡 **partial (2026-07-04)** — companion `--ci-gate` deterministic drift/min-score lane and scheduled workflow exist; the eval store is now cache-backed in the nightly workflow (`actions/cache/restore` + `save`, explicit `JARVIS_EVAL_STORE` / `--store-root`), so baseline compare bites after the first successful scheduled/manual run for a dataset hash. Live-model eval still needs a persistent/live runner. Tracked as M2.4 / O26-P3.3. | 0.13 |
...
| 6 | V1 live contracts · V4 blocking eval gate | M–L | O24 Track V | 🟡 partial — V3 components/skills coverage ✅; M2.4 deterministic drift gate ✅; persistent eval baseline ✅ (#506); remaining Track V work is live contracts + live eval runner |
