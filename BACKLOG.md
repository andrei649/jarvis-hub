...
| **0.16.0** | HUD depth + observability UI | TASK-2 ~37 surfaces incl. **north-star panel** + **network monitor** (watch LOCAL_ONLY make zero calls — *egress data layer + `GET /api/admin/network/calls` done, H23.16; HUD panel pending*); LIVE/SEED indicators; OpenAPI types; plugin-gated modes |
...
| H23.16 | **Network monitor** HUD panel (prove LOCAL_ONLY agents make zero outbound calls) | PARTIAL — **data layer + API done**: thread-safe `observability/egress_monitor.py` (in-memory ring buffer + monotonic per-plugin counters) records *every* outbound attempt — allowed **and** blocked — at the `http_client.py` choke point (all 6 verbs via one `_guard`); `GET /api/admin/network/calls?plugin=&limit=` (admin-guarded) serves per-plugin tallies + recent events + `local_only_violations` (the proof: a NONE/LAN plugin with an allowed external call surfaces as a violation → `clean=False`). `tests/test_network_monitor.py` (+9, MockTransport — no real socket). **Remaining:** wire the HUD panel to this endpoint (owner-runtime-gated per CDX-9 / OWNER_TASKS — offline-wired panels hide breakage). | 0.16 |
