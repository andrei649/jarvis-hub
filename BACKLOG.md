...
- 2026-09-18 H002 one scripted turn from a pipe (partial): `nerva chat -z "prompt"` runs one turn non-interactively and puts **only** the final assistant text on stdout — whole escape sequences and C0/C1 control bytes stripped, newlines and tabs kept — with every other word on stderr. The prompt resolves exactly the way `nerva send`'s body does (argument, else `-f PATH` where `-` is stdin, else piped stdin, and a terminal is never read), and a prompt over the hub's own 4,096-character `ChatRequest` bound is a usage error **before** the request rather than an HTTP 422 a script would read as a failed turn. The ladder is stated in `--help`: 0 answered · 1 the turn did not complete · 2 usage · 3 no hub · 4 not authorised · 130 interrupted — Ctrl-C now returns 130 with one stderr line instead of an uncaught traceback that could spill a partial answer into a pipe. **The row's governance clause is implemented against Hermes, not after it:** Hermes sets YOLO and accept-hooks inside `-z` because "no user is watching"; here there is no such flag, a turn that queues an action stays queued, and the verb exits 1 naming the pending id and `nerva approvals`. The one-shot path issues no decision call at all. **An independent adversarial review then broke that clause in one line, and the fix is the substance of this row:** the verdict was an exact-string match on the reply, but the hub does not hand `/chat` the agent's reply verbatim — `Orchestrator._synthesize` wraps a single specialist's answer as `[athena]: …` whenever `jarvis` is not among the responders, which is the ordinary routed path — so `-z` exited **0** with a queued approval printed as the answer (reproduced with the hub's own synthesiser). Matching is now by containment over the sanitised text, and the record states what containment cannot do: it survives the `[agent]: …` wrapper, nothing survives an LLM re-write, and the real fix is the hub reporting `pending_approvals` — the slice stacked directly on this one. The table also gained the five tool-loop stop reasons it was missing (a loop that gave up part-way used to exit 0 with its stop reason stored as the answer) plus a prefix rule for the one the hub builds with an f-string, and the drift test now imports all ten constants and asserts each is matched both verbatim and wrapped. Sanitising moved before the verdict, so a reply of pure control bytes is an incomplete turn rather than exit 0 and a blank line. The rest of the round, each red-proofed: the receipt is written on the interrupt path too; `_write_usage` unlinks its temp file on a failed write and catches the `UnicodeEncodeError` that used to crash the verb after the answer was already out; `--usage-file ""` says so instead of vanishing; a non-`-z` receipt no longer claims `completed: true` beside a queued approval; a non-list `pending_approvals` is reported as unreadable instead of exploded into one fabricated id per character; `-f /dev/tty` is refused rather than hanging; the OSC branch of the escape regex is bounded and an unterminated sequence no longer leaks its payload as visible text; the answer survives a stdout that cannot encode it; and `_read_body` bounds in characters, not bytes. One test was replaced rather than kept — the old atomicity pin passed under a plain truncating write, so it pinned "overwritten", not atomic. 51 tests, 23 mutations each turning a test red. A stdlib-only copy of the hub's seven refusal replies maps each to a machine reason, and a drift test imports the real constants so a renamed reply is a red test rather than a silent exit 0 on a refused turn. `--usage-file PATH` writes `nerva.chat.usage.v1` atomically on every path — answered, refused, queued, usage error, no hub, not authorised — and an unwritable path warns without changing the verdict. 29 tests; nine mutations of the shipped behaviour each turn at least one of them red. **The spend half of the row is a schema of nulls, and that is the honest half:** the hub cannot attribute one turn's cost today — `AgentConfig.model` defaults to `google/gemma-4-31b-a4b`, none of the 18 shipped agents declares a model, so `cost_tracker` meters every turn at the default price row and reports $0.00, and an id it does not know is priced from the `default` row with no `priced` flag. `estimated_cost_usd`, the token counts, `api_calls`, `model` and `provider` are reported as `null` with `cost_basis: "unavailable"` rather than as measured zeros; H002 stays **partial** until a turn's real spend comes back from the hub. Also open: `POST /chat` returns no trace/invocation/session id (so `session_id` is echoed only when the caller supplied one), `-z` does not silence hub-side logging, and a piped body is read whole up to the bound rather than streamed. Counters 12,090→12,141 (12,119 after the first round, then the review fixes).
- 2026-09-16 H480 send from a script (partial): `nerva send` now takes its body Hermes's way — the MESSAGE argument, else `-f PATH` (`-` reads stdin), else whatever is piped on stdin, and a terminal is never read (with or without `-f -`; a closed stdin is a usage error, not a traceback; only the bound plus one character is read; a UTF-8 BOM is dropped); `-s LINE` is one printable subject line that ntfy carries as its native title (printable ASCII ≤120, refused with the reason rather than silently mangled by the adapter) and every other channel as the first line (subject, blank line, message — Hermes's shape); `-q` prints nothing on success; `--list CHANNEL` filters the target list — a name that can never be a direct-send channel is a usage error decided offline, and a channel an older hub does not list is exit 1 with what it does list named; a blank MESSAGE argument counts as no message and falls through to `--file`/stdin. The exit ladder is stated in `--help`: 0 sent · 1 not delivered · 2 usage · 3 no hub · 4 not authorised. A binary `--file` is refused with the attachment guidance and its bytes are never echoed; the 4,000-character bound is the seam's own rule (the subject counts where it becomes the first line, not for ntfy) and the CLI enforces the same rule before any request; hub and shell strings are cleaned of control characters before the terminal. `POST /api/channels/send` gains an optional one-line `subject` (≤200) and the outbound seam applies the bound to what is actually delivered. The real `ChannelManager` was probed to admit the ntfy `title` kwarg. An independent adversarial review found fifteen items, two of them majors (two pre-existing `send` pins broken), all fixed in the same PR with tests; `frontend/src/api/schema.gen.ts` regenerated for the new field. Remaining, named in the row: `MEDIA:<path>` attachments (file egress needs a kernel action and a confined source directory before a route may read local paths), no-gateway operation (Nerva's send needs the hub for the IntentLog and rate limit — a documented divergence), bodies over 4,000 characters are refused rather than chunked, Slack/Discord stay reply-only (H018), and the `JobRunner._send` duplication is left to the jobs lane.
- 2026-09-15 H022 dependency audit (partial): `nerva security audit` enumerates this interpreter's distributions plus the exact Python pins of any `--extension` descriptor, asks OSV.dev through `PluginHTTPClient` (pinned DNS, owner TLS anchors, breaker; only name+version pairs leave, disclosed on stderr — from the CLI process there is no kernel mediation and the egress ledger does not persist), merges the GHSA/PYSEC mirrors of one CVE into one finding carrying the best *stated* severity, and exits 0 clean · 1 at/above `--fail-on` · 5 database not consulted. Two rules are load-bearing and red-proofed: an unrated advisory fails every threshold (no CVSS scoring is attempted), and an unreachable database is exit 5 with nothing claimed — never "clean". MCP — owner-configured stdio commands on Nerva — is listed as *not audited*, with that reason. Live from a transparent-proxy sandbox with its CA in `SSL_CERT_FILE`: 48 distributions, 0 findings; a scratch pin of `requests 2.19.0` surfaced 5 findings (1 high, 4 moderate) once 5 PYSEC duplicates merged — 10 before merging, which would have failed `--fail-on high` on duplicates of moderate advisories. An independent adversarial review found nine defects, all fixed before ready-for-review: the MCP reason was false (Nerva's MCP is stdio-first); one distribution with undecodable metadata crashed the audit with exit 1 — it is now skipped and named, and any audit that cannot complete is exit 5; advisory text reached the terminal raw (ESC/newline could forge a finding row) — now control-stripped and cut, JSON untouched; `querybatch` pagination was ignored — pages are followed, and a batch still open after 20 is `unavailable`; the fix named could be a downgrade — now the fix whose range contains the installed version; alias spellings deduplicated; a false docstring claim about `rg` corrected. Remaining, named in the row: MCP `npx`/`uvx` commands are not resolved to packages; the acquired extension catalog needs the running hub and is not read offline (only named descriptors); no bare `nerva security` default and no `--skip-*`; vector-only advisories stay unrated; default `--fail-on low` is deliberately stricter than Hermes's `critical`; no operator proxy knob (H281); egress ledger/kernel mediation do not apply from the CLI process; the exit ladder is Nerva's 0/1/2/5, not Hermes's 0/1/2.
...
- 2026-09-15 URL monitors (H453 partial): each exact public GET and redirect crosses enforced signed approval; complete 256 KiB UTF-8 identity responses are scrubbed before persistence and share monitor baseline/diff behavior. 35 new cases; 232 focused and independent-review checks pass. Fresh per-hop approvals remain mandatory; private/credentialed/compressed URLs and unattended authority remain outside this supported scope.
- 2026-09-15 native build compatibility: declare the already locked Expo core at the app level so inherited AudioPlayer events resolve; all 1,315 package entries retain their versions/integrities. Two absent FlatList components use the current optional-prop type. Complete native TypeScript and 137 Jest tests pass independently; no audio API or device playback claim.
- 2026-09-15 appearance fonts (H135 partial): five fixed local UI font choices persist through the existing CAS/offline journal and root/prefix HUD. Code monospace is independent; stale font IDs coerce to theme. Six new backend and two frontend cases, 19/20 focused checks, and two real-browser font/persistence checks pass. Native consumer and full curated catalog remain open.
- 2026-09-15 appearance sync (H135 missing → partial): browser/ambient/responsive HUD choices persist in the Settings DB; immutable offline edits and revision checks protect multi-tab saves. System motion follows OS accessibility. 13 new backend and 18 frontend cases; 24 real-browser checks and full 1,247 frontend suite pass. Native mobile consumer and font choices remain open. [Contract](frontend/docs/h135-appearance-sync.md).
- 2026-09-15 Gemini stream usage (H673 partial): terminal-local provider counts publish only after clean EOF/close and supported single-candidate or blocked-prompt completion. Error trailers, ambiguous candidate IDs and interrupted attempts keep estimates. 43 new cases and 181 independently repeated focused checks pass. Undocumented terminal layouts, fixed image estimates and heterogeneous route limits remain open.
- 2026-09-15 stream context usage (H673 partial): Ollama, LM Studio and Anthropic retain one confirmed terminal usage measurement; interrupted/malformed streams keep estimates. Cumulative counters replace rather than add, retries reset and thought filtering remains intact. 46 new cases and 270 focused checks pass; independently reviewed. Gemini stream accounting and other documented context gaps remain open; no live-provider proof.
- 2026-09-15 text context usage (H673 partial): complete provider text responses reach closed request-local observers, and non-stream agent gathers retain per-agent usage. Tools and inherited OpenRouter streaming report once; retries and late tasks remain isolated. 30 new cases and 274 focused checks pass. Actual streaming terminal accounting and non-conversational metering remain open.
- 2026-09-15 cloud context usage (H673 partial): Gemini and explicitly compatible structured turns retain validated provider totals; cached Anthropic prefixes now fully contribute to context anchors, including cache-only requests. Billing remains unchanged. 56 new cases and 181 focused checks pass; non-structured usage, heterogeneous route limits and cache pricing remain open.
- 2026-09-15 local context usage (H673 partial): LM Studio/Ollama structured replies now carry validated provider input/output counts to existing usage sinks. Tool-loop meters accumulate while context anchors use the latest request; malformed counts preserve answers. 38 new cases and 215 focused checks pass. Other provider/non-structured usage and image estimates remain open.
...
- 2026-09-15 gallery regression synchronization: wait for selection cleanup after a rejected continuation rather than treating the retry-button render as completion. CI exposed the asynchronous assertion race; runtime export already intersects selection with currently available items. No product behavior or Hermes status change.
...
> Seventeen file-partitioned builder slices landed as one commit (`214bc5eb`), each with its own
> hermetic tests and its own red-proof. **Statuses below are exactly what each slice reported** —
> ✅ = delivered · 🟡 = partial with the remainder named.
>
> **Marker change, owner decision 2026-09-07.** This file used to carry a third status, 🔨, for
> *delivered but not yet proven on owner hardware, credentials or network*. The owner's call is that
> **verification happens in production**, so a row is no longer held open waiting for a bench test it
> was never going to get here: the 64 🔨 rows are now ✅. What that ✅ means has changed with them —
> it is **delivered**, not **proven in CI**, and the two are not the same claim.
>
> Nothing about *what has actually been exercised* was deleted in the flip. Every row that had a
...
> [`docs/OWNER_TASKS.md`](docs/OWNER_TASKS.md) · module index →
> [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §3 · theme-by-theme plan →
> [`docs/NERVA_2_ROADMAP.md`](docs/NERVA_2_ROADMAP.md) §4.

**Operator hands**

- [x] ✅ **op-permission-ledger — one consent ledger.** `agents/core/permission_ledger.py`:
  per-app / per-site / OS-input / file-root / terminal-target grants {once, session, always, never}
  with a built-in default-deny list (banks, brokerages, crypto wallets, password managers, SSO IdPs,
  adult; secret file roots), kernel kind `permission.grant` (EXTERNAL tier, ASK → decision inbox;
  applied only from the human-decided task; `never` rows immutable; session grants die with the boot;
  `once` consumed exactly once; os_input restore tokens in the SecretStore). `GET /api/permissions` +
  `POST /api/permissions/{id}/revoke` (user-guarded, narrowing only) + Trust `PermissionsPanel`.
  Flag `JARVIS_PERMISSION_LEDGER` (default off). 44 pytest + 6 vitest. **Open:** surface callers
  (browser / desktop / file tools / terminal targets) do not consult the ledger yet — see the
  integrator notes; the ledger is a substrate until they are wired.
- [x] ✅ **OP-TERM-1 — governed local terminal transport.** `LocalHostTransport` (argv only,
  cwd-jailed, capped, kill-on-timeout) behind `JARVIS_TERMINAL_LOCAL_HOST`; wired in
  `GovernedTargetRunner` after hardline → target policy → durable approval → `terminal.exec` contract
  → Action Kernel GRANT. 73 hermetic tests + one real `python -c` child. **Not yet proven on the
  owner's host** — needs kernel registration + coordinator binding, then the OWNER_TASKS packet.
- [x] ✅ **OP-TERM-2 — static HARDLINE denylist.** `rm -rf <root>`, `mkfs`, `dd of=/dev`, raw device
  writes, fork bomb, `curl|sh`, shutdown/reboot, `reg delete HKLM`, `rd /s /q C:\`, `chmod -R 777 /`,
  `find / -delete`, `kill -1`, `crontab -r`, `/etc/passwd` overwrite, firewall/SELinux/Defender
  disablement — evaluated **before** authorize on every backend including docker; a hardline hit
  leaves no audit entry and never spawns.
- [x] ✅ **OP-TERM-3 — kernel kind `terminal.exec` + `TERMINAL_EXEC_CONTRACT`.** Fingerprinted argv,
  cwd inside roots, timeout ≤ 600s, durable approved task, `requires_approval=True`, rollback `none`
  (shell effects are not automatically reversible — honest). Contract and kind constant shipped in
  `agents/core/environments/terminal_contract.py`; the registry / manifest / `action_auth.json`
  registration is the integrator's step (owned by I1).
- [x] ✅ **OP-TERM-4 — `local-host` target.** `default_targets()` gains `local-host` (jarvis/ultron;
  `terminal.read` / `terminal.exec` / `file.read`; `terminal.exec` approval-required; enabled only by
  `JARVIS_TERMINAL_LOCAL_HOST`). `bonobo-windows` / `pi-house` / `isolated-sandbox` unchanged.
- [x] ✅ **OP-VISUAL — visual grounding fallback (screen_locator + presets).**
  `agents/core/screen_locator.py` `LocalVLMLocator` binds the desktop drivers' locator seam (a11y
  first, then proven-local VLM; `provenance=local_vlm` + `screenshot_sha256` on every result; kernel
  kind stays `desktop.step`). `JARVIS_VLM_PRESET` selects a pinned Apache-2.0 grounder
  (qwen3-vl-4b/8b, ui-tars-1.5-7b, holo-3.1-35b-a3b, qwen3.8-27b) whose coordinate convention
  `screen_grounding.normalize_coords` converts to absolute pixels; `parse_grounding` reads box forms
  and `fuse_with_a11y` uses driver rect/center. `tests/test_screen_locator.py` (+23),
  `tests/test_screen_grounding_h15_2.py` (+11), `tests/test_vlm_h13_1.py` (+7).
  **Delivered-not-proven:** each preset's convention is pinned from published inference notes; an
  owner run with a real grounder on a real screenshot (OWNER_TASKS) is the proof.

**Company primitives**

- [x] ✅ **co-subagent-steer — steerable sub-agents.** `POST /api/subagents/{id}/steer|stop`,
  `SteerChannel` inbox (origin `user|agent`, never an approval — `decide()` refuses agent origin
  before the hook), `output_schema` typed hand-off (`validate_output`: type/required/enum), per-spawn
  cost (runner usage or tracker delta), spawn records → `data/subagents/spawns.jsonl` behind
  `JARVIS_SUBAGENT_SPAWN_LOG` (observability only; sub-agents are **not** the company unit).
...
- [x] ✅ **B3 / #731 criteria 5 + E3 #761 — E2.1 epistemic status, E3.2 recall admissions.**
  ...
  **Production-verification (was 🔨):** delivered, not program-accepted — the E2/E3 gates need an attestation from a reviewer
  distinct from the builder.

- [x] ✅ **B3 / #731 — Identity Manifest (#1008), E4.1.** `agents/core/identity_manifest.py` +
  ...
  **Production-verification (was 🔨):** delivered, not *program-accepted* — the E4 gate needs an attestation from a reviewer
  distinct from the builder, which is `proof-track-reviews`, not this slice.

- [x] ✅ **REL-CHANNELS — package managers, the free distribution.**
  ...
  **Production-verification (was 🔨):** nothing has been published. The Homebrew tap repo, the winget PR and making the
...
- [x] ✅ **install-one-step-native — one-step native install per OS.** `install.sh` (checkout or the
  hosted `curl|bash` one-liner) / `INSTALL.bat` → `scripts/bootstrap.py` (stdlib-only; Python ≥3.12
  floor with a named refusal, idempotent `.venv`, `pip --require-hashes` from
  `requirements-beta.lock`, loopback-only Ollama/LM Studio detection, install smoke with
  `JARVIS_HOST` pinned to `127.0.0.1` and cloud keys scrubbed) ending in the Command Center at `/v2`;
  `start.sh` / `START.bat` open `/v2` on `/readyz`; `install.ps1 -Launch`. 55 hermetic tests.
  **Delivered-not-proven on real Windows/macOS hardware** (`bash -n` + fakes only here).
- [x] ✅ **install-doctor — `scripts/doctor.py`.** `./start.sh --doctor`, `START.bat doctor`:
  required rows `python` / `venv` / `locks_in_sync` / `bind_is_loopback` / `data_root_writable`,
  advisory rows `runtimes` / `readyz` / `smoke`, one named reason each, `--json`, exit 0/1; the locks
  rule is byte-identical to `scripts/lock_deps.sh --check`; `LOOPBACK_HOSTS` pinned to `boot_guards`
  by test.
- [x] ✅ **docker-quickstart-loopback — `docker-compose.quickstart.yml`.** One container, host
  networking + `JARVIS_HOST=127.0.0.1` (same posture as native; host runtimes reachable),
  hash-verified lock install, `nerva_data` volume, optional `.env`. **Not built or run here** (no
  Docker in the sandbox); Docker Desktop needs "Enable host networking" (4.34+) and
  `env_file: {path, required: false}` needs Compose ≥ 2.24.
- [x] ✅ **MS-1 — hardware-tiered local-model recommendation.** cpu-only → `qwen2.5:3b`, 8GB →
  `qwen2.5:7b`, 12–16GB → `qwen2.5:14b`, 24GB+ → `gemma-4-31b-a4b` (spec-based, never benchmarked) +
  Apple Silicon (`sysctl hw.memsize`, argv) and AMD (`rocm-smi` CSV) GPU probes in `hardware.py`.
  Apple/AMD probes proven only against fakes (no mac/ROCm box in CI); the 75% unified-memory budget
  is a labelled assumption. Owner packet flips this to ✅.
- [x] ✅ **MS-2 — governed Ollama pull.** `action:model.pull` (reversible, rollback =
  `ollama_delete`), `JARVIS_MODEL_PULL` default-off, loopback-only URL, size cap
  `llm.model_pull_max_gb` enforced from the pull stream's layer totals, one job at a time;
  `GET /api/onboarding/model-plan` + `POST /api/onboarding/model-pull` (user-guarded, kernel
  DENY=403 / QUEUE=202) + Console "MODEL SETUP" card. +53 backend (`test_model_setup.py`) +5
  (`test_hardware_profile.py`) +7 vitest. Delivered against an httpx `MockTransport` and a fake
  authorizer — **not yet proven against a real Ollama.**
- [x] ✅ **Proof-of-Action receipts + shareable day report** *(Competitive-Gap "outside the box" #1,
  first slice — this row is new; the plan's `outside_the_box` list has no in-file table)*.
  `agents/core/day_report.py`, `agents/core/routers/report.py`, `TodayReceiptPanel`:
  `GET /api/report/today` is an allow-listed, payload-free, scanner-redacted summary of one local day
  (counts from the north-star meter + TaskQueue, titles only, %-local, model name, fingerprinted;
  empty days say so), `?format=html` a self-contained card, `POST /api/report/today/export` writes it
  under `<data_root>/reports/` through `EXPORT_CONTRACT` + the Action Kernel (kind `report.export`),
  and `GET /api/report/receipt/{audit_id}` renders one intent-log entry as a chain-verified, redacted
  receipt. 30 backend + 7 vitest tests. **Open:** mobile card (H18.x), receipt links from the
  Decision Inbox / TodayPanel rows, PNG render, per-goal receipts once WorkRun exists.

**House**

- [x] ✅ **LVP-hestia-wiring — `agents/hestia` wired onto `agents/core/house/**`.**
  `hestia_bridge.HestiaBridge(house_brain)`: `observe()` (strict-local snapshot → shared `HouseGraph`
  projection + presence writer, bounded non-sensitive picture with aggregate occupancy only) and
  `propose()` (explicit local rules → house tasks through `HouseActuator.request_*` → kernel intake
  gate → `govern_enqueue`, agent=hestia, ask floor, per-cycle/cooldown/daily caps). Default-off
  `JARVIS_HESTIA_BRIDGE`; `from_orchestrator` binds onto the router's cached runtime (shared stores).
  SOUL v0.2.0 documents the code. +25 tests (`test_hestia_bridge.py`, `test_house_brain.py`).
  **Open:** binding into `plugin_manager` / `orchestrator_bindings` is an integrator edit (I1), and
  nothing calls `observe()` / `propose()` on a schedule yet.

**MCP**

- [x] ✅ **MCP-STDIO-ENV — Hermes-parity stdio env stripping (opt-in, default-off).**
  `JARVIS_MCP_STDIO_ENV_BASELINE=1` makes a spawned stdio MCP server inherit only
  `STDIO_ENV_ALLOWLIST` (PATH/HOME/locale/temp/platform/toolchain/TLS roots) plus its per-server `env`
  overrides; API keys, tokens, proxies and every `JARVIS_*` setting are stripped
  (`client.stdio_env_baseline`). Default off so a server that relied on inheriting a hub credential
  keeps working until the owner opts in. Tested in `tests/test_mcp_transports.py`.

**Nerva 2.0 program (contracts that had no code) — delivered, not program-accepted**

- [x] ✅ **B4 / #778 M1 item 5 — `nerva.ledger.v1` cognitive-ledger records.**
  `agents/core/cognitive_ledger.py` (`LedgerRef`, `GoalSpec`, `EvidenceRecord`, `ActionIntent`,
  `AuthorizationRecord`, `ExecutionRecord`, `VerificationRecord`, `OutcomeRecord`, `LedgerChain`),
  8 hermetic tests over a real `DecisionRecord` + `issue_receipt()` receipt + `compare_outcome()`
  observation; **record_only** authority, no runtime path. Program acceptance pending — E1 stays
  `BUILDING`.
- [x] ✅ **E1.3 — cognitive-ledger contracts delivered.** [`docs/nerva2/CORTEX_E1_3.md`](docs/nerva2/CORTEX_E1_3.md);
  rollback = delete module + test + doc. Not program-accepted.
- [x] ✅ **B3 / #731 — Continuity Core evaluation suite.** Packaged `evaluation_only` on the accepted
  E9.0 `nerva.benchmark.v1` harness as suite `continuity-core-v1`
  (`agents/core/observability/continuity_suite.py`, 20 synthetic-public scenarios over 7 #731
  criteria, pinned content fingerprint, reference subject 1.0 vs naive baseline 0.35, per-criterion
  `nerva.continuity.report.v1` with acceptance owners and `can_accept_epic=false`; 6 tests;
  [`docs/nerva2/CONTINUITY_SUITE_E9_2.md`](docs/nerva2/CONTINUITY_SUITE_E9_2.md)). Metric acceptance
  stays with E3/E6/E12 (recall, contradiction, abstention), E4 #1008 (identity), E11 (migration
  parity); **#731 remains open on its own bar.** Nightly lane entry (`eval-nightly.yml`) pending the
  integrator.

<!-- WAVE:2026-09-06-OPUS-INTEGRATION:END -->
...
The catalogue's own §5 recommendation **has now had its first pass — ✅ done 2026-09-06
(`nerva2-program-control`), pass recorded, nothing adopted.** Playwright was taken through the #805
minimum-contract fields and all six §4 gates in
[`docs/nerva2/INTEGRATION_CATALOGUE_ADOPTION_PASS_PLAYWRIGHT.md`](docs/nerva2/INTEGRATION_CATALOGUE_ADOPTION_PASS_PLAYWRIGHT.md):
LICENSE read (Apache-2.0, digest recorded), Node pin `1.62.1` exact, the Python package deliberately
optional/unpinned, gates graded 1 PARTIAL / 2 N/A / 3 tool-not-provider / 4 PASS (hermetic) / 5 NOT
MEASURED / 6 PASS. The row moves `PARKED` → `PASS_RECORDED — promotion candidate`; **no dependency,
pin, provider, route or authority change.** Next Tier A pass: the MCP server surface, which needs a
network primary read. The original recommendation, for the record: take **one** Tier A
...
**B2 — live issue-ledger reconciliation → ✅ delivered, not program-accepted (2026-09-06,
`nerva2-program-control`).** The compact advisory checker `scripts/check_nerva_program_manifest.py`
is restored in its post-#981 posture (**never a PR gate**) with
`.github/workflows/nerva-manifest-check.yml`, and the manifest is reconciled to truth:
`enforcement_state` `safety_disabled` with a rollback record bound to #981 / `824ff187`, four deleted
control paths retired, E11 `owner_live` re-scoped to the owed A1 §0 run record (A8 cleared by owner
2026-08-28), #918 retained-not-accepted noted on the E5/E8 B7 gates, #1008 under E4, E5
`building`/`in_progress` with all gates open, and the CONTRACT_REGISTRY mirror byte-checked. 27 tests
in `tests/test_nerva_program_manifest_checker.py`. **Live issue state is `not_verified`** until the
first real `gh` run is recorded in `docs/nerva2/ISSUE_LEDGER_RECONCILIATION.md` — the file never
claims otherwise. The strict real-repo test is a visible skip until the five sibling E5.0 files
exist in the checkout (they did not land in this wave).
...
- [x] ✅ **DRA-06 — Item 12's 0.65 half is only half-covered — the plan builds the ScreenReflex route but
  ...
  **✅ Residual (b) closed 2026-09-06 (`op-visual-grounding`).** `_is_loopback_base`'s docstring is
  reconciled with `routers/multimodal.py`: the function computes the `is_local` **label**; the
  **gate** is enforced by the callers that hold screen bytes — the `screen_reflex` route (503) and
  `screen_locator.LocalVLMLocator`, which refuses `local_vlm_not_proven_local` before any bytes
  leave. Pinned by `tests/test_vlm_h13_1.py::test_loopback_label_docstring_names_the_real_gates`.
...
- [x] ✅ **DRA-22 — H28.2 ActionHierarchyRouter is dead code — a ✅-marked capability with zero production
  ...
  **✅ Visual-route residual closed 2026-09-06 (`op-visual-grounding`).** `OperatorRoute.VISUAL` is
  now registered as `desktop_visual` against the **existing** `action:desktop.step` (no invented
  capability id); it is considered only with `allow_visual_fallback=True` and is available only when
  the desktop host flag pair (+ the driver-factory verdict) **and** `resolve_vlm_config().is_local`
  hold — opt-in, never a default. `browser_run` (`tool:browser_run`, priority 5) sits beside
  `desktop_run` in the structured-UI rank. `tests/test_h28_action_hierarchy_router.py` (+6).
...
- [x] ✅ **DRA-25 — MCP SSE transport is unimplemented but advertised end-to-end (admin UI select, API
  ...
  **✅ Residual closed 2026-09-06 (`mcp-transports`).** A real Streamable HTTP client transport ships
  behind `JARVIS_MCP_HTTP_CLIENT` (default off): `agents/core/mcp/http_transport.py`
  `StreamableHttpTransport` — POST JSON-RPC with `Accept: application/json, text/event-stream`,
  `Mcp-Session-Id` capture/echo/DELETE release and drop-on-404, negotiated `MCP-Protocol-Version`
  echo, JSON or SSE replies bounded to 256 events / 4 MiB, every byte through the SSRF-pinned
  `PluginHTTPClient` (loopback/RFC1918 literals lan-mode, DNS names public, metadata hosts refused,
  cross-origin credential stripping inherited). `MCPServer` normalises `transport` aliases, refuses
  with named `last_error` reasons, gains transport-aware `is_connected()` and in-memory-only
  `headers`; the tool-call contract stays byte-identical stdio-only until the flag is set
  (`active_tool_call_contract()`), and the widened contract re-checks the flag per call, so unsetting
  it revokes a persisted HTTP server at the next call. The deprecated HTTP+SSE pair stays refused by
  name (`unsupported_transport:sse`) — the advertising, which was the finding, is gone.
  `tests/test_mcp_transports.py` (+24, shared with H10.5). **Admin API/UI acceptance of
  `streamable-http` is an integrator wiring in `routers/mcp.py`** (I1); until then HTTP servers load
  via `MCPManager.load_from_config`.
...
- [x] ✅ **DRA-27 — Memory write/hygiene controls are still built-but-unreachable (consolidate, decay
  ...
  **✅ Sixth leg wired 2026-09-06 (`memory-hygiene-leg6`) — the row closes.** The design decision the
  consolidate leg was blocked on is answered in code: `GET /api/memory/consolidate/preview` derives
  `existing` from fused recall in the planner's `{id, key, text}` shape (scanned, taint-scoped), so
  the planner is no longer degenerate by construction, and `POST /api/memory/consolidate/apply`
  `{plan, existing, dry_run?}` refuses `existing: []` and malformed plans with 422 + a stable reason,
  applies through a `ListStore` snapshot and persists vector-backed rows with per-op
  persisted/skipped reporting. The `CONSOLIDATE` Console panel
  (`frontend/src/panels/memory-consolidate.tsx`) drives preview → plan → dry-run/apply and withholds
  plan/apply when nothing was recalled. `tests/test_memory_hygiene_leg6.py` (+21), panel vitest (+8).
  *(Persistence goes through `orch.memory.vectors.remove` under the manager's private lock — proven
  against the in-memory store contract via fakes; the Qdrant backend path is delivered-not-proven.)*
...
- [x] ✅ **SEC-B4 — SSRF IP-pinning coverage.** **✅ hermetic; live-browser is a production-verification item (2026-09-06).** **🟡 Recounted 2026-08-29 (`DRA-09`) — the row below was
  ...
  outcome.
  **✅ Browser leg delivered 2026-09-06 (`op-browser-transport`), hermetically proven.**
  `agents/core/browser_transport.py` `PinnedResolver` pins one `resolve_and_validate`-validated IP per
  host and launches Chromium with `--host-resolver-rules=MAP host ip, …, MAP * ~NOTFOUND` (unpinned
  hosts fail at name resolution; firefox/webkit refuse `pinned_transport_requires_chromium`);
  `PlaywrightBrowserDriver.set_transport()` is now **required** before `navigate()` (no transport →
  the unchanged `browser transport unavailable`, no startup), every routed request (redirects,
  iframes, fetches) is re-validated for scheme / private range / policy / pinned host, the final URL
  is re-checked after `goto`, each run gets a throw-away `nerva-browser-*` profile (never the
  owner's), and `observe_snapshot()` gives an accessibility-first bounded element list
  (`aria_snapshot(boxes=True)` with a fallback). `JARVIS_BROWSER_ALLOW_PRIVATE_URLS` (default off) is
  the house-LAN opt-in. 30 hermetic tests — `tests/test_browser_transport.py`,
  `tests/test_h28_playwright_driver.py`, verified green together with
  `tests/test_http_client_ssrf_pinning.py` at integration (62 passed).
  **Delivered; not proven against a real Chromium** (resolver-rule ordering semantics, `boxes=True`
  availability) — owner validation with `JARVIS_PLAYWRIGHT_HOST=1` is the OWNER_TASKS packet, and
  `GovernedBrowser.run_step` still blocks `navigate` until the integrator edit lands (I1). The row
  therefore stays open on its live half.
...
- [ ] 🟡 **Re-measured 2026-09-05, and the residual above is described wrongly — the route is not
  ...
  - **Re-measured 2026-09-06 (`memory-hygiene-leg6`) — the HTTP-route leg is now closed, and it is
    NOT the fail-open shape this bullet withdrew.** The withdrawn draft reset the mark *around the
    recall call*, which would have discarded it before the handler could act — fail-open, correctly
    rejected. What shipped instead binds on entry and resets **the whole handler**, with the reset
    placed **after the response is built**, so the mark cannot be dropped while a decision is still
    being made: `memory_kg._recall_scope` over `/api/memory/search`, `/api/memory/recall`,
    `/api/memory/search-tool` and `/api/memory/consolidate/preview`. Responses now report `tainted` +
    `action_origin`; the search-tool handler re-derives the verdict from the hits, because
    `to_thread` copies the context into the worker and the tool's own mark never reached the handler.
    Regression: direct-await tests in `tests/test_sec_b5_recall_taint.py` (+2) and
    `tests/test_memory_hygiene_leg6.py` go red without the reset.
    **The row stays 🟡 on exactly one residual:** `Orchestrator.process()` (reflector, autonomy
    executor, `/api/context/compress`) still binds no turn origin — the owner decision named in the
    bullet above, untouched.
...
- [x] ✅ **The web HUD is not reachable from a phone today, by design.** `serve.py:66` defaults
  ...
  **✅ Written down 2026-09-06 (`install-bootstrap-doctor`):**
  [`docs/PHONE_ACCESS.md`](docs/PHONE_ACCESS.md) is the supported phone / second-device LAN path —
  what `assert_safe_bind` refuses, what `_user_guard` 403s/401s, why `JARVIS_HOST` +
  `JARVIS_USER_TOKEN` must be in the **process environment** (`.env` loads after the bind check),
  HUD/mobile token entry, what stays closed, and `token_store` rotation. `scripts/doctor.py`'s
  `bind_is_loopback` row reports the posture before boot. *(The topology decision in the sibling row
  below is unchanged — this ticks the "documented nowhere" half only.)*
...
- [x] ✅ **Coverage is now 16 of 16 rail modes.** The number hotkeys reach ten; `projects`,
      ...
      manually … and came back clean", and that was true and worth nothing as a gate: a hand-walk
      cannot regress-guard anything, and **`admin` is the largest surface in the HUD (482 nodes in
      demo at 1440×900) and had never been scanned by axe at all.** `a11y-modes.spec.ts` now walks
...
- [x] ✅ **The cost, stated rather than hand-waved — and now recovered (2026-09-07).** The lane no
      longer registers the worker. That cost **no assertion** at the time: there were zero PWA
      assertions in `frontend/e2e/`, and the worker's `fetch` handler served no request in any spec,
      because every spec navigates once and both its branches need a second navigation.
      `tests/test_pwa_v2.py` covered the worker by regex over its source, not behaviourally. The
      handoff this item left — "a real PWA spec should opt back in with
      `test.use({ serviceWorkers: 'allow' })`" — has been taken: **`frontend/e2e/pwa.spec.ts` (+4)**
      does exactly that, file-scoped, so the global block stays in place for every other spec. It
      loads `/v2`, waits for the worker to reach `activated`, reloads so the client is genuinely
      *controlled*, and only then asserts: root scope + a single surviving cache name; `/v2/assets/*`
      cache-first; the shell cached under `/` and **not** under the navigated URL; the cached shell
      answering a navigation with the network off. The one with teeth is **no `/api/` path in Cache
      Storage** — stated negatively *and* positively (the cache contains only `/` and
      `/v2/assets/*`, so it cannot pass vacuously), because `sw-v2.js` keeps personal data out by
      **omission**, and an omission is invisible in review. Red-proved by adding one `respondWith`
      branch too high in the served worker: the suite came back with `/api/payments`,
      `/api/kg/entities` and the rest in the diff, exactly the leak `forget` could never reach.
...
| 0.26 Capture Inbox | 🟡 partial → **inbox view ✅ · export ✅** | `passive_capture.py`+`routers/capture.py` + **HUD `CapturePanel`** (HUD-v3: the captured stream, each item's redacted preview shown + individually deletable + clear-all — the privacy promise made visible) + **`PassiveCapture.export()`/`write_export()`** (the data half of "phone export"): a portable, JSON-safe snapshot `{version, exported_at, surface, count, surfaces, records}` of the capture inbox, optionally filtered by surface. Records carry **only already-redacted previews + metadata** (secrets are scrubbed at `ingest` and raw content is never stored) so the export can't leak a secret — it's the same data the inbox exposes via `list`, packaged for off-device transfer; `write_export(dest)` dumps it to a file. `tests/test_h12_7_capture.py` (+4: packages redacted records, surface filter, empty, write-to-file + secret-never-present). **2026-09-07 — the phone half shipped.** New `GET /api/capture/export` (user-guarded, read-only) serves the same envelope `export()` builds, so the data half finally has a door; an **unknown surface is a 422, not an empty export**, because `?surface=clipboad` and `?surface=clipboard` produce identical files on an empty inbox and a typo must never be able to read as an all-clear. Native **Capture tab** (`mobile/src/screens/CaptureScreen.tsx` + the pure `capturePolicy.ts`): inspect, forget one, clear all, export to a file. **Arming stays off the phone by decision, not by permission** — `/api/capture/surfaces` and `/api/capture/ingest` are user-guarded and reachable and are deliberately absent, because enabling a surface arms an ambient recorder on a machine you are not sitting at, while deleting is safe in the direction that matters. The master switch and the per-surface opt-ins stay **unflattened** (a surface on with capture off records nothing, and gets its own sentence), a failed fetch reads as *unknown* rather than "off", a record with no id is dropped rather than shown un-deletable, and a `forgotten: false` is reported as a no-op instead of optimistically removing the row. Export filenames are **UTC-stamped and scope-named** so a filtered export cannot later be mistaken for a complete one, and an unknown surface name can never reach the filename. `tests/test_h12_7_capture.py` (+4 route), mobile +19 (10 API, 9 policy) + 6 screen-contract. / remaining: transcript sync | — |
...
| 0.52 Product Demo Factory | 🟡 partial → **capture tooling ✅ (2026-09-07)** | `docs/marketing/TEASER_PACK.md` storyboard + shot-list complete. **NEW: the capture half.** `frontend/e2e/footage.spec.ts` (+5) records one 1920×1080 `.webm` per §6 shot off the real running HUD, driven by `scripts/hud_footage.mjs` and documented in `docs/marketing/FOOTAGE_RUNBOOK.md`. The §6 reuse rule — *"never stage fake data for a shot. Demo mode is clearly badged; use it, or use real data"* — is **enforced, not just written down**: exactly two sources exist (`live`, `demo`), a third fails at collection naming both, and each is asserted in its own direction — a demo clip must have the amber `DEMO DATA` banner **visible in frame**, a live clip must not have it at all (red-proved by forcing `?demo=1` into the live lane). The specs only navigate and wait; there is no seam for feeding the HUD a prettier corpus, so an empty surface films as an empty state. Navigation goes through the command palette (hardcoded English names ⇒ survives a language toggle) and proves itself by watching the active rail button change, so re-ordering `MODES` cannot mis-shoot. Opt-in twice over: the `footage` project exists only under `FOOTAGE=1`, and every other project carries `testIgnore: /footage\.spec\.ts$/`, so `npm run e2e`, the PR lane and the nightly soak never record video even if the flag leaks in. **Not covered:** §6.6 (WorldView globe) — a separate surface with its own server, so a clip from this lane would be named `worldview.webm` and contain something that is not WorldView; and the Telegram half of §6.3, which is a device recording. Both are stated in the runbook. **Remaining (owner-gated):** the assembly/edit pass and the demo video itself (M4 / A6). | H23.22 |
...
| 0.63 Restore & Soak | 🟡 partial → **failure-injection half ✅ (2026-09-06); 72h soak stays owner** | backup/restore+drill ✅ (#302) + `resilience.py`. **Sandbox hardening follow-up to #631** (found by an independent adversarial verification of the merged safety batch): the output cap was applied *after* `proc.communicate()` drained the child to EOF into host memory, so a runaway/hostile sandboxed child (agent-generated code) could balloon host RSS for the whole timeout window — `max_output_bytes` bounded only what was returned. `environments/output_limits.py` gains `read_capped_stream()` (streams head+tail within budget, discarding the middle so peak retained memory is ~budget regardless of stream size) + `render_capped()` (honest omission notice using the *true* total); `sandbox.py._read_output_capped()` replaces every `communicate()` read site (docker/subprocess/shell/wasm) with a mock-safe fallback. `tests/test_environment_output_limits.py` (+5) + `tests/test_sandbox_output_cap.py` (+2: a real 500 KB child bounded to <2 KB carrying the true-total notice). / remaining: 72h soak (owner lane)<br>**T-0.63 failure-injection harness ✅ (test lane), 2026-09-06.** `agents/core/observability/fault_injection.py` — `FaultPlan(kind ∈ llm_down\|db_corrupt\|disk_full\|clock_skew, duration_s, target, skew_s)` (frozen, validated, SHA-256 fingerprint) + `inject(plan)` context manager returning an auditable `FaultHandle` (per-interception events, self-expiry on the monotonic clock, restore-in-finally). Default-off behind `JARVIS_FAULT_INJECT`; `JARVIS_HARDENED` refuses unconditionally and `boot_problem()` feeds the boot guard; `db_corrupt`/`disk_full` are fenced to `data_root()`; no new dependency, no subprocess, no socket. Proven offline in `tests/chaos/test_fault_injection.py` (+31): the LM Studio backend serves the clean degraded reply (never the raw exception) under `llm_down` and recovers; `verify_chain()` stays `(True, None)` while a neighbouring store is corrupt and the store restores byte-for-byte; a disk-full write is refused with `ENOSPC` / `database or disk is full` and never crashes, reads pass; `clock_skew` moves a real `AttentionLedger` day window. Manual chapter 13 §13.16b CHA-090…097; CHA-013 ❌→⚠️ (simulated, not a volume fill). **Remaining (owner):** the 72h soak lane on the self-hosted runner — not ticked, not simulated. | H23.8/12 |
...
| 0.66 SaaS Connector Breadth | ✅ **builders + executor rail delivered behind `JARVIS_WRITEBACK_LIVE`; real provider writes unproven (production-verification)** | ~20 integrations skewed messaging/IoT; the white-collar suite was missing. **NEW `agents/core/writeback_connectors.py`** adds pure, offline request builders for **Linear · Asana · Trello · Todoist · ClickUp · Google Sheets · Microsoft 365 (Outlook draft)**, same discipline as H10.30 write-back: validated `CATALOG` (unknown action/missing field → refused with reason), **host allowlist** (`CONNECTOR_HOSTS`, SSRF guard), **secrets only at execute-time** (drafts carry a `{{secret:<target>_token}}` handle, never a raw token — SecretBroker resolves behind approval), `build_connector_request` → one concrete HTTP request each, `draft_task_payload` → ask-tier approval-queue task, `catalog()` inspectable surface. `tests/test_writeback_connectors.py` (+15). **✅ Wired 2026-09-06 (`live-rails-behind-flags`) → the live half is production-verification:** `WriteBackBroker.request/execute` accept the 0.66 catalog (kind `writeback.<target>.<action>`, ask-tier, `{{secret:<target>_token}}` handles; Trello also `trello_api_key`), credentials resolved via SecretBroker at execute time (`writeback_connectors.resolve_credentials`), live HTTP send only behind `JARVIS_WRITEBACK_LIVE` with the `CONNECTOR_HOSTS` allowlist. +29 tests (`tests/test_live_rails_flags.py`). **Delivered-not-proven against real provider APIs** (no owner credentials in CI); owner OAuth/token setup per provider stays an owner task (`docs/OWNER_TASKS.md`), and the executor registration of `create_task`/`task.create` is an integrator edit (I1). | — |
...
| H30.8 ✅ | **Ambient light bridge (assistant state → LAN strip)** — the last open item from the 2026-08-06 "J.A.R.V.I.S. in the room" guide (`docs/design/JARVIS_PRESENCE_GAP.md`): a default-off bridge that maps the SAME voice/assistant state the orb renders onto a LAN light controller, so the strip and the sphere can never disagree. WLED first (plain HTTP JSON on the local network, no cloud account, strict-local by construction); Hue/Govee behind their own opt-in since they reach a vendor cloud. Must go through the existing governed device path, stay silent (not guess) when the device is unreachable, and ship with the light OFF by default. **Delivered 2026-09-06 (`house-hestia-wled`); not proven on hardware:** `agents/core/house/wled.py` `WLEDBridge` — `JARVIS_WLED_URL` unset → `wled_not_configured`; strict-local origin (LAN literal / localhost / `.local` / allowlist + LAN-only resolution, the HA rule set reused); `WLED_SCENES` mirrors the six orb states of `frontend/src/orb.tsx`; **every `set_scene` crosses the Action Kernel** under the existing `house.control` kind via `CapabilityActionAPI` (DENY → nothing sent, QUEUE → `approval_required`); echo-verified writes (`"v": true`), `wled_unreachable` without guessing, unchanged-scene no-op, per-minute write budget, lazy httpx (`dependency_unavailable:httpx`). `HestiaBridge.ambient(state)` is the forwarding seam. +22 tests (`tests/test_wled_bridge.py`) over a fake transport — **no real strip exercised**, and the voice-runtime call-site for the hand-off is still a follow-up. | 3 | P3 | H30.4 | NERVA_VISION §7 |
...
| # | Item | S | P | Dep | Sursă |
|---|------|---|---|-----|-------|
| H10.5 ✅ | **MCP Server Mode** — Jarvis expune agenți ca tool-uri MCP *guvernate*; orice client MCP (Claude Desktop, Cursor, alt Jarvis) poate apela agenți Jarvis ca tool-uri. **Done 2026-06-03:** `core/mcp/server.py` `JarvisMCPServer` — core JSON-RPC 2.0 transport-agnostic (initialize/tools/list/tools/call/ping), un tool `ask_<agent>` per agent, allowlist + LAN-only by default, rutează prin orchestrator (guardrails+gate); endpoints `GET /api/mcp/server` (status+tools) + `POST /api/mcp/server/rpc` (gated pe `mcp.server_enabled`, default off). +13 teste offline. **✅ stdio loop delivered 2026-09-06 (`mcp-transports`); bridge-vs-live-hub is production-verification:** `JarvisMCPServer.serve_stdio(reader, writer, identity=)` over the reusable `run_stdio_loop()` (newline-delimited JSON-RPC, batch arrays, -32700/-32600/-32603 carrying only the exception type, 4 MiB frame cap, identity threaded into `tools/call`) and `open_stdio_streams()` (event-loop pipe transports, thread fallback for consoles the loop cannot attach to). `scripts/nerva_mcp_stdio.py` bridges a stdio MCP client (Claude Desktop, Cursor) to a running hub's `POST /api/mcp/server/rpc` through the same guarded transport: token from an env var (never argv), client ids re-stamped, stderr-only logging, **no hub gate widened**. Tested over in-memory frames, a real `os.pipe` on Linux, and a fake hub route; the bridge has not been exercised against a live hub and the Windows console fallback is untested on Windows. | 8 | P1 | H4.7 | Langflow |
...
| # | Item | S | P | Dep | Sursă |
|---|------|---|---|-----|-------|
| H11.1 ✅ | **Desktop App (Tauri)** — native wrapper over the existing HUD, no new backend. **2026-09-15 H178 bounded floating chat:** executable tray and main controls; frameless/resizable always-on-top window where supported; saved physical geometry with DPI scaling and monitor recovery; hide/reset/handoff to normal Chat; same authenticated transport/session. Native macOS compile/tests and UI validation; Windows/Linux packaging and physical monitor hotplug unverified. No autostart/wake-word daemon, capture or input automation. Full Hermes overlay remains partial (no frost, click-through, game overlay or global shortcut). See `desktop/README.md`. | 13 | P3 | — | OpenJarvis (Tauri) |
...
| H12.21 ✅ | **Acțiuni guvernate pe social** (X/Twitter post/reply/DM) — fiecare *write* prin coada de aprobare; auth OAuth/secret‑broker (nu cookie‑uri brute). **Done 2026-06-09:** `core/social.py` `SocialBroker` — paralel cu write-back (H10.30): request → validare allowlist (x: post/reply/dm) + sanitizare → **task ask-tier** (`kind=social.x.<action>`, tier extern); **nimic nu se postează la request**. Pe aprobare, executor prefix `social` → `SocialBroker.execute` rezolvă tokenul OAuth/bearer **la momentul acțiunii, în spatele aprobării** (SecretBroker — handle `{{secret:x_api_token}}`, niciodată cookie-uri) și apelează client injectabil (`NullSocialClient` offline default; `HttpSocialClient` = rail live prin `build_social_request` pur → X API v2 `/2/tweets`, reply, `/2/dm_conversations`). Endpoints `GET/POST /api/integrations/social`. +15 teste offline (catalog, validare, build_request post/reply/dm, execute behind-approval, e2e prin coadă+worker). **Live rail now flag-gated (2026-09-06, `live-rails-behind-flags`):** `JARVIS_SOCIAL_LIVE=1` constructs `HttpSocialClient`; unset → Null client. Refuses `credential_not_configured` when `x_api_token` is absent; the `api.twitter.com` allowlist is enforced; the Postiz path is unchanged. **Real X posting unproven without an owner token.** | 5 | P3 | H6.2 | OpenClaw TweetClaw/Bird |
| H12.22 ✅ | **Voce outbound / call‑back** — agentul sună la prag + persona vocală izolată (Twilio/Telnyx), gated prin interrupt‑budget. **Done 2026-06-09:** `core/autonomy/call_broker.py` `CallBroker` — apel outbound gated **dublu**: coada de aprobare (`kind=call.outbound`, ask‑tier extern) ȘI **bugetul zilnic de întreruperi** (un apel e o întrerupere → consumă din ≤4/zi). Pe aprobare rezolvă tokenul telephony **în spatele aprobării** (SecretBroker — handle `{{secret:…}}`) și sună prin client injectabil (`NullCallClient` offline default; `HttpCallClient` = rail Twilio/Telnyx prin `build_call_request` pur — Twilio form+basic‑auth, Telnyx JSON+bearer). Endpoint `POST /api/autonomy/call`. +15 teste offline (validare, buget epuizat, build per provider, execute behind‑approval, e2e prin coadă+worker). **Live rail now flag-gated (2026-09-06, `live-rails-behind-flags`):** `JARVIS_CALL_LIVE=1` constructs `HttpCallClient`; unset → Null client. A flag-armed dial refuses `credential_not_configured` or `call_config_missing:<keys>` (pure `missing_config()`) **before** spending an interrupt-budget slot; `JARVIS_CALL_CONFIG` must carry the provider block (`account_sid` or `connection_id`, plus `from`). **Real Twilio/Telnyx dial unproven without an owner account.** | 8 | P3 | H6.2 | OpenClaw SuperCall |
...
## ORIZONT 19 — WorldView (4D OSINT) — Standalone + Integrare JARVIS — 35 rânduri: 35 ✅ (cod livrat; pentru 33 dintre ele runtime proof-ul e production-verification, nu dovadă CI — marker change 2026-09-07; recounted 2026-09-02)
...
> the Nerva 1.x critical path; the `H19.x` rows read ✅ **delivered** since the 2026-09-07 marker change,
> and "code delivered, scale unproven" stays true of them — the scale proof is a production-verification
> item, not CI evidence. The live-source hops (ADS-B/AIS/TLE egress + local Kafka via the worldview docker-compose) stay
...
| H22.8 ❌ | **Trial codebase-memory-mcp — închis pe propria regulă (2026-09-12).** Scaffold-ul (`.mcp.json.example`, `docs/dev/codebase-memory-mcp.md`, intrarea doc-pinned din `.github/third-party-manifest.json`) a fost **șters**; `.mcp.json` real n-a existat niciodată, deci serverul n-a rulat și nu se schimbă niciun runtime. Documentul lui scria regula de ieșire — *„if it doesn't earn its keep over `docs/AI_CONTEXT.md` bundles, drop the `.mcp.json` and close H22.8"* — iar în trei luni jumătatea de host (install binar + `index_repository`) n-a fost făcută. Motivul de fond: costul de context aici e **proza** (BACKLOG ~176K tokeni, ledger Hermes ~355K), pe care `backlog.py`/`ledger.py` o taie deja și pe care un index de cod n-o atinge; iar documentul însuși nota că embeddings-urile lui sunt mai slabe decât stiva Nervei. Decizie owner 2026-09-12, varianta A. Golul real rămas — un graf de apeluri interogabil, fiindcă §3 din `ARCHITECTURE.md` e index întreținut manual care poate deriva — se ia, dacă se ia, ca script stdlib + snapshot fixat de test (tiparul `gen_api_sweep.py`), fără dependență. Context: `docs/decisions/2026-09-11-external-tooling-shortlist.md`. | 2 | P2 | — | ❌ închis: scaffold șters, manifest curat |
