...
| **0.15.0 ⚠️** | Operability & distribution | health/readiness endpoint ✅ (H23.11, `/healthz`+`/readyz`) + signal handlers/graceful shutdown ✅ + log rotation ✅ (H23.11); graceful **local-LLM-down** everywhere ✅ (H23.12, split-timeout + clean degraded reply); systemd/service templates ✅ (H23.15, `deploy/`); **release artifacts** (tar/zip, optional PyPI + Docker publish); **semver compatibility contract** ✅ + supported-versions matrix ✅ + deprecation policy ✅ + platform matrix ✅ (H23.14, `docs/COMPATIBILITY.md`) |
