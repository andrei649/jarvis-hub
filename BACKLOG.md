...
> ✅ = provable in CI · 🔨 = delivered, remaining proof needs owner hardware/credentials/network ·
> 🟡 = partial with the remainder named. Rows that already existed elsewhere in this file were
...
- [ ] 🔨 **OP-TERM-1 — governed local terminal transport.** `LocalHostTransport` (argv only,
...
- [ ] 🔨 **OP-TERM-3 — kernel kind `terminal.exec` + `TERMINAL_EXEC_CONTRACT`.** Fingerprinted argv,
...
- [ ] 🔨 **OP-VISUAL — visual grounding fallback (screen_locator + presets).**
...
- [x] ✅ **install-doctor — `scripts/doctor.py`.** `./start.sh --doctor`, `START.bat doctor`:
  ...
  by test.
- [ ] 🔨 **docker-quickstart-loopback — `docker-compose.quickstart.yml`.** One container, host
...
- [ ] 🔨 **MS-1 — hardware-tiered local-model recommendation.** cpu-only → `qwen2.5:3b`, 8GB →
...
- [ ] 🔨 **MS-2 — governed Ollama pull.** `action:model.pull` (reversible, rollback =
...
- [ ] 🔨 **B4 / #778 M1 item 5 — `nerva.ledger.v1` cognitive-ledger records.**
  ...
  `BUILDING`.
- [ ] 🔨 **E1.3 — cognitive-ledger contracts delivered.** [`docs/nerva2/CORTEX_E1_3.md`](docs/nerva2/CORTEX_E1_3.md);
  rollback = delete module + test + doc. Not program-accepted.
- [ ] 🔨 **B3 / #731 — Continuity Core evaluation suite.** Packaged `evaluation_only` on the accepted
...
- [ ] 🔨 **SEC-B4 — SSRF IP-pinning coverage.** **✅ hermetic / 🔨 live-browser (2026-09-06).** **🟡 Recounted 2026-08-29 (`DRA-09`) — the row below was
...
| 0.66 SaaS Connector Breadth | 🔨 **builders ✅ + executor rail delivered behind `JARVIS_WRITEBACK_LIVE`; real provider writes unproven** | ~20 integrations skewed messaging/IoT; the white-collar suite was missing. **NEW `agents/core/writeback_connectors.py`** adds pure, offline request builders for **Linear · Asana · Trello · Todoist · ClickUp · Google Sheets · Microsoft 365 (Outlook draft)**, same discipline as H10.30 write-back: validated `CATALOG` (unknown action/missing field → refused with reason), **host allowlist** (`CONNECTOR_HOSTS`, SSRF guard), **secrets only at execute-time** (drafts carry a `{{secret:<target>_token}}` handle, never a raw token — SecretBroker resolves behind approval), `build_connector_request` → one concrete HTTP request each, `draft_task_payload` → ask-tier approval-queue task, `catalog()` inspectable surface. `tests/test_writeback_connectors.py` (+15). **✅ Wired 2026-09-06 (`live-rails-behind-flags`) → row now 🔨 on the live half:** `WriteBackBroker.request/execute` accept the 0.66 catalog (kind `writeback.<target>.<action>`, ask-tier, `{{secret:<target>_token}}` handles; Trello also `trello_api_key`), credentials resolved via SecretBroker at execute time (`writeback_connectors.resolve_credentials`), live HTTP send only behind `JARVIS_WRITEBACK_LIVE` with the `CONNECTOR_HOSTS` allowlist. +29 tests (`tests/test_live_rails_flags.py`). **Delivered-not-proven against real provider APIs** (no owner credentials in CI); owner OAuth/token setup per provider stays an owner task (`docs/OWNER_TASKS.md`), and the executor registration of `create_task`/`task.create` is an integrator edit (I1). | — |
