...
python -m pytest tests/ -v          # ~2,707 passed, 6 skipped
```
...
| AUD-5 | 🟢 **in PR** — **Session path-traversal** — shared `validation.is_valid_session_id` enforced in `sessions.py` (route → 400) and at the `memory/persistence.py` boundary (F7). | 1 | **P0** | ✅ `session_id=../../x` → 400; no file escapes the data root; `tests/test_session_traversal.py` |

**Phase 1 — next sprint (correctness + auth lifecycle + CI gates)**

| # | Item | S | P | AC |
|---|------|---|---|----|
| AUD-6 | 🟢 **in PR** — **Token lifecycle** — issued admin tokens on top of the static env token: `TokenStore` (`security/token_store.py`) mints `secrets.token_urlsafe(32)`, stores only its SHA-256 (raw token returned once), with an optional TTL; `verify`/`has_scope` reject expired tokens; `rotate` revokes a scope's prior tokens. Guards (`_admin_credential_ok`/`_admin_guard`) accept a valid issued token *in addition to* the env token (additive, no-lockout: localhost dev fallback unchanged). `POST /api/admin/rotate-tokens` (admin-guarded, returns the fresh token once, audited without the value). *Deferred (F19 tail): httpOnly cookie over `localStorage`, user-token issuance + read/write split.* | 3 | P1 | ✅ an expired/rotated token is rejected; raw tokens never hit disk (only the SHA-256); `tests/test_token_lifecycle.py` |
