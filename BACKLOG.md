...
**În afara totalului H1–H11:** ORIZONT 12 (acum în scope-ul v1.0 per [#52]) — **2/15 livrate** (H12.1 ✅ P0 securitate, H12.10 ✅); restul deschis. **Bugs & Hot Fixes** — 3 done (BUG-1, BUG-2, BUG-4) + deschise (BUG-3, HF-1/2, BUG-2b, TASK-1, CLN-1, NTH-1).
...
## ORIZONT 12 — Categoria Reală: Asistent Personal Privat & Proactiv (P0–P3) — 2/15
...
| H12.15 | **Backup & restore date personale** — script (`scripts/backup-data.sh` + `.ps1`) care arhivează `agents/data/` + `memory_logs/` (memoria H8, sesiuni, workflow-uri create, corpus ingerat) și restaurează din arhivă; opțional schedule. Păstrează local-first: backup local/extern, opt-in cloud. Aceste foldere sunt **singura stare cu date reale și sunt git-ignored** → fără asta, pierdere totală la orice `clean`/reinstalare (vezi incidentul 2026-06-02). | 3 | P2 | H8.2 | durabilitate local-first |
