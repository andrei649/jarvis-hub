...
python -m pytest tests/ -v          # 2156 passed, 1 skipped
```
...
| **CLN-2** | **Spargere god-object `Orchestrator`** (`agents/core/orchestrator.py`) — un singur obiect gestionează agenți + pluginuri + memorie + canale + autonomie + checkpoints + learning. **Început în #118 (audit A2 — `ComponentRegistry`)**, care a redus fișierul 1620→1537 LOC; rămâne de extras manageri dedicați (AgentManager/PluginManager/ChannelManager) și de continuat decuplarea. **→ 🟡 Avans:** `ChannelManager` (`core/channels/manager.py`) extras — deține registry-ul de canale + start/stop/send; Orchestrator delegă via proprietatea `channels` (getter+setter, zero schimbare de comportament). Rămân AgentManager/PluginManager. **Plan complet:** [`docs/superpowers/specs/2026-06-13-cln2-cln3-refactor-plan.md`](docs/superpowers/specs/2026-06-13-cln2-cln3-refactor-plan.md) (Faza 2: 4 manageri facade-preserving; sesiunea/ContextVar BUG-5 rămâne pe orchestrator — nu se extrage). | 🟡 În progres · P3 · **post-1.0** (decizie owner 2026-06-10: după testarea manuală) | 5 | Continuă #118 (A2) | Audit cod 2026-06-04 |
| **CLN-3** | **Spargere `web.py`** (~4636 LOC, 233 rute, singletons globale `orch`/`gateway`) — split în routere FastAPI per-domeniu (`APIRouter`); reduce/elimină mutarea state-ului global. **→ 🟡 Avans:** pachet `core/routers/` + **8 domenii extrase** (wyoming/onboarding/webhooks/a2a/pairing/canvas/browser/capture) cu wrappere lazy de auth-guard (`_deps.py`, fără ciclu de import); web.py **5417→4967 LOC**. Restul rutelor continuă același pattern incremental (sigur înainte de testarea manuală). **Plan complet** (294 rute live; 255 inline; topologie 3-straturi `web_helpers`/`app_state`/`_deps` cu `get_orch()` late-binding; ordine de extragere pe tier-uri + plasă de siguranță parity-guard): [`docs/superpowers/specs/2026-06-13-cln2-cln3-refactor-plan.md`](docs/superpowers/specs/2026-06-13-cln2-cln3-refactor-plan.md). | 🟡 În progres · P3 · **post-1.0** (decizie owner 2026-06-10: după testarea manuală) | 8 | — | Audit cod 2026-06-04 |
...
| H22.5 📐 | **Model-manager LRU fast↔deep** (Fooocus/ComfyUI `free_memory`) — track modele rezidente + unload LRU cu headroom înainte de load deep; anti-OOM-thrash în `hybrid_router`. Distinct de H21.C. **Spec livrat:** `docs/superpowers/specs/2026-06-20-h22.5-model-manager-design.md`. *Build în spatele kill-switch + validare GPU (nemăsurabil în CI).* | 5 | P3 | H22.2, H22.4 | swap fast↔deep fără OOM; evict LRU narat |
...
| H22.9 📐 | **Rute guvernate via MCP server** (BuilderIO/agent-native, *pattern*) — manifest acțiuni din OpenAPI; `mcp/server.py` expune rute allow-listed ca tool-uri MCP lângă agenți, reutilizând permission gate. **Post-1.0. Spec livrat:** `docs/superpowers/specs/2026-06-20-h22.9-agent-native-routes-design.md`. | 5 | P3 | — | un client MCP poate conduce hub-ul prin rute guvernate |
...
> - **Livrate cod+teste:** H22.1–3 (PR #264) · **H22.10** (securitate) · **H22.6** (workflow bound). 6/10.
> - **Repo-side done, acțiune host rămasă (🟡):** H22.4 (runbook → validare GPU), H22.7 (skills →
>   install plugin), H22.8 (scaffold → install binar + trial). 3/10.
> - **Spec'd, ready-to-build (📐):** H22.5 (build + validare GPU), H22.9 (post-1.0). 2/10. *(H22.5 e și 🟡.)*
