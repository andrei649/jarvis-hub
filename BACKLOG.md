...
| H5.16 🟡 | **Sentence-level TTS & Audio Barge-in** — edge-tts integration + server-side play/stop există și sunt testate; **dar** (audit 2026-06-05, cod citit) sentence-level splitting și barge-in NU sunt implementate, iar bucla vocală e *server-side* (Howard, mic pe host). **Browser voice loop** în HUD livrat separat (mic `getUserMedia` → STT local `/api/voice/stt` → chat → TTS playback, hands-free, fallback `speechSynthesis`); barge-in din browser rămâne TODO (necesită tuning echo-cancellation on-device). | 8 | H1.1, H5.5 | 0.8 🟡 |
