...
| BUG-2 🚧 | Frontend test infrastructure missing — 0% coverage on React HUD (~6 000 LOC: `app.js`, `admin.js`, `systems.js`, `workflows.js`, etc). No test runner (jest/vitest), no React Testing Library, no E2E (Playwright/Cypress). **In progress 2026-06-02:** Vitest + JSDOM harness added (`tests/frontend/`, `package.json`, `vitest.config.js`) that loads the real shipped global scripts (vendored React 18 UMD + static files) — no bundler/build step needed. First 23 tests green (`components.js` helpers + `Bracket`/`StatusDot`, `i18n.js`). Wired into CI (`frontend` job). Remaining: panels (`systems`/`workflows`/`admin`/`network`/`observability`/`cognition`), `data.js`, `app.js`, and E2E. See `tests/frontend/README.md`. | MEDIUM | Identified in test coverage audit 2026-06-02; backend gap closed (121 tests added on branch `claude/hud-human-interface-testing-r8IQS`) |
