...
| # | Item | S | P | Dep | Sursă |
|---|------|---|---|-----|-------|
| H12.2 ✅ | **Onboarding "drop folder → chat privat cu documentele"** — un singur pas: alegi un folder, Jarvis îl indexează local (PDF/MD/docx) și poți discuta cu el offline. **Done 2026-06-03:** `core/local_docs.py` `LocalDocsIndexer` (walk recursiv, extract md/txt/rst nativ + pdf/docx best-effort cu skip grațios, chunking word-window cu overlap → `memory.remember` local, fără cloud); endpoints `POST /api/local-docs/index` + `GET /api/local-docs` (ultimul sumar). +5 teste offline. | 3 | P1 | H8.3 | GPT4All LocalDocs, Khoj |
