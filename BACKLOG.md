...
- 2026-09-15 H022 dependency audit (partial): `nerva security audit` enumerates this interpreter's distributions plus the exact Python pins of any `--extension` descriptor, asks OSV.dev through the governed egress client (pinned DNS, owner TLS anchors; only name+version pairs leave, disclosed on stderr), merges the GHSA/PYSEC mirrors of one CVE into one finding carrying the best *stated* severity, and exits 0 clean · 1 at/above `--fail-on` · 5 database not consulted. Two rules are load-bearing and red-proofed: an unrated advisory fails every threshold (no CVSS scoring is attempted), and an unreachable database is exit 5 with nothing claimed — never "clean". MCP is reported as an empty surface with its reason (HTTP transport, no installed server packages). Live from a transparent-proxy sandbox with its CA in `SSL_CERT_FILE`: 48 distributions, 0 findings; a scratch pin of `requests 2.19.0` surfaced 5 findings (1 high, 4 moderate) once 5 PYSEC duplicates merged — 10 before merging, which would have failed `--fail-on high` on duplicates of moderate advisories. Remaining, named in the row: the acquired extension catalog needs the running hub and is not read offline; vector-only advisories stay unrated; no operator proxy knob (H281); the exit ladder is Nerva's 0/1/2/5, not Hermes's 0/1/2.
