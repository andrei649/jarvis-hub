...
| H23.15 | systemd/service templates (Linux/Windows) | ✅ **done** — `deploy/systemd/jarvis-hub.service` (hardened unit: `ProtectSystem=strict`/`NoNewPrivileges`/restricted address families; `KillSignal=SIGTERM` + `TimeoutStopSec` margin over `JARVIS_SHUTDOWN_TIMEOUT` → the H23.11 bounded graceful drain) + `jarvis-hub.env` + README; `deploy/windows/install-service.ps1` (NSSM, Ctrl-C graceful stop) + README; `deploy/README.md` index wiring the `/healthz`·`/readyz` probes. Both consume the H23.11 env knobs; guarded by `tests/test_compatibility.py`. **Always-On runtime operationalized (2026-08-18):** the headless engine (H6.1 coordinator + heartbeat + H6.6 night-shift, no HTTP) now has its own supervised entrypoint — `agents/core/autonomy/runtime_coordinator.py` (boots the same `Orchestrator` minus channels), `scripts/runtime_supervisor.py` (portable crash-recovery wrapper, restart-with-backoff, no systemd required — `make runtime-up`/`runtime-down`/`runtime-status`), and `deploy/systemd/jarvis-runtime-coordinator.service` for the systemd-native equivalent. A structured run-log (`logs/runtime.jsonl`, one JSON line/cycle via `runtime_log.py`) and a crash-safe `runtime_state.py` (atomic-write cycle/boot counters) make restarts observable and provable: verified live — `kill -9` on the coordinator recovered in ~3.5s with the cycle counter resuming (not reset) across a `boot_id` increment, 5 consecutive `clean` cycles logged straddling the restart. `tests/test_runtime_log.py` + `test_runtime_state.py` + `test_runtime_coordinator.py` + `test_runtime_supervisor.py` (+29). Deliberately non-invasive: zero changes to `queue.py`/`worker.py`/`autonomy_coordinator.py` (B7 mediation untouched) — the recorder only reads existing `TaskQueue.stats()`/`HeartbeatScheduler.get_status()`. Run this **instead of**, never alongside, `jarvis-hub.service` against the same `JARVIS_HOME` (both would drive `AutonomyCoordinator.loop()` against the same `autonomy.db`). | 0.15 |
