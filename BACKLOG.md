...
| BUG-5 | **Race pe `self.session_id`** — handler-ul de canal salvează/restaurează `self.session_id` pe instanța *partajată* a orchestratorului (`agents/core/orchestrator.py:714-719`) în jurul unui `await handle_input`. Două cereri concurente pe canale diferite pot suprascrie reciproc `session_id` înainte de blocul `finally` → **un răspuns poate ajunge în conversația greșită**. Fix: context per-cerere (nu mutație pe instanța partajată). Cel mai impactant bug găsit la audit. | HIGH (sub concurență; LOW single-user) | Audit cod 2026-06-04 |
| BUG-6 | **Reload non-atomic `_runtime_settings`** — loop-ul de fundal (la 30s) reconstruiește dict-ul `flat` apoi îl atribuie (`agents/core/orchestrator.py:380`) fără lock/atomic-swap; un reader concurent poate vedea stare parțială. H7.4 a acoperit SQLite, nu acest dict in-memory. | LOW | Audit cod 2026-06-04 |
| BUG-7 | **Leak `httpx.AsyncClient`** — `LMStudioBackend`/`OllamaBackend` creează clientul în `__init__` (`agents/core/llm/base.py:154`, `:248`) și nu apelează niciodată `aclose()` (fără context manager/`__del__`) → connection pools rămase deschise în timp. Fix: shutdown hook în orchestrator sau async context manager. | MEDIUM | Audit cod 2026-06-04 |
| BUG-8 | **Parsing fragil în `_detect_handoff`/`_detect_skill`** — dacă lipsește `]` după prefix, `end = len(resp)` extrage până la EOF fără limită (`agents/core/orchestrator.py:1129-1130`); `_detect_skill` (`:1141`) folosește `resp.index("]")` care **aruncă `ValueError`** dacă `]` lipsește. Fix: bound de lungime + `.find()` cu guard. | LOW | Audit cod 2026-06-04 |
| BUG-9 | **Path-traversal în `promote_bench_agent`** — scrie `SOUL.md` dintr-un `bench_id` nevalidat (`agents/core/orchestrator.py:1338`); fără `Path.resolve()`/containment, un id cu `../` ar putea scrie în afara `agents/`. Fix: whitelist `[a-z0-9_]` + verificare containment. | MEDIUM | Audit cod 2026-06-04 |
...
| **HF-3** | **Hardening scanner Secret/PII** — pattern OpenAI prea laxat (`sk-…{20,}`, real ≥40 chars → false positives); `db_connection_string` (`scanner.py:82`) prea larg (orice 10+ chars după `://`); `password_assignment` (`:81`) prinde doar valori *între ghilimele* (ratează `password=secret` neîncadrat); **lipsesc** JWT (`eyJ…`), service-account JSON GCP/Azure, Bearer tokens, material PEM, heuristică entropie. | 🔒 Hardening · P2 | 3 | Se cuplează cu HF-2 (security review) | Audit cod 2026-06-04 · `agents/core/security/scanner.py:76-87` |
| **HF-4** | **SSRF: DNS-rebinding / TOCTOU** — `check_ssrf` rezolvă IP-ul la momentul check-ului, dar fetch-ul real (`plugins/websearch.py`) e ulterior; un domeniu controlat de atacator poate întoarce IP public la check și `127.0.0.1` la fetch. Mitigare: pin pe IP-ul validat la conectare / re-validare / blocare redirect-uri spre IP-uri private. | 🔒 Hardening · P2 | 3 | După HF-2 | Audit cod 2026-06-04 · `agents/core/security/ssrf.py` |
| **HF-5** | **Separare cheie HMAC audit** — cheia de semnare stă lângă log (`memory_logs/security/*.key`) și „transparency log"-ul e tot un JSON local; acces de scriere pe FS = rescrierea întregului lanț Merkle + recalcul hash + cheie într-un singur pas. Mutare cheie în store criptat / anchoring extern (timestamp authority). | 🔒 Hardening · P3 | 3 | — | Audit cod 2026-06-04 · `agents/core/security/audit.py`, `anchor.py` |
