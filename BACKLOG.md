...
| H19.3.2 | **Tool-uri MCP write/async** (`watch_aoi`, `reconstruct_event`) + **auth capability-token** (reutilizează `CapabilityBroker`, H17.3). **AC:** `watch_aoi` creează o regulă sub token scoped; apelurile neautorizate respinse + auditat. | 5 | P1 | H19.3.1, H17.3 | JARVIS |
...
| H19.4.3 | **Provenance/chain-of-custody** (`source`+`ingested_at`+bitemporal `valid_*` în UI/API). **AC:** orice datum/insight se trasează la sursă; UI arată provenance. | 5 | P2 | — | Both |
