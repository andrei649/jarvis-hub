...
| BUG-2 ✅ | ~~Frontend test infrastructure missing — 0% coverage on React HUD (~6 000 LOC).~~ **Done 2026-06-02:** Vitest + JSDOM harness (`tests/frontend/`, `package.json`, `vitest.config.js`) that loads the real shipped global scripts (vendored React 18 UMD + static files) — no bundler/build step. **141 tests across 17 spec files**, wired into CI (`frontend` job). Covers `components.js` (helpers + all building blocks: Bracket/StatusDot/Badge/Sys*/InputBar/Weather/Calendar/Message/TopBar/AgentList/AgentsGrid/HeartbeatFeed), `i18n.js`, `data.js` (`loadJarvisData` resilience), `admin.js` (form rows + `renderRow`), `systems.js` (tabs incl. Memory/Plugins/Resilience + FusedRecall), `cognition.js`, `observability.js`, `workflows.js`, `enhancements.js`, `dossier-modal.js`, `network.js`, and a full-app mount smoke (`app.js`). **Caught a real shipped bug on first run:** `systems.js` `ResilienceTab` missing closing brace → the entire Systems panel failed to parse/load in the browser (present on `main`); fixed + regression-guarded (`resilience.test.js`). **Remaining (optional follow-up):** browser E2E (Playwright) — needs a running server + real browser; tracked separately. See `tests/frontend/README.md`. | ~~MEDIUM~~ | Identified in test coverage audit 2026-06-02; backend gap closed (121 tests added on branch `claude/hud-human-interface-testing-r8IQS`) |
