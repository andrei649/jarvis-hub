...
prediction only, not Jarvis's continuity identity. **Recounted 2026-08-28: still true** —
whether to open a new issue, fold it into #762, or explicitly decline is an owner-scoping call
the reconciliation doc deliberately didn't make unilaterally; packet in `docs/OWNER_TASKS.md`.
...
(+3); route/OpenAPI/route-auth snapshots reseeded. ***Still open (corrected 2026-08-28 — an earlier
note here overstated this as fully closed):*** the row says "into per-agent **digests**", and the
morning-brief/digest builder still does not consume any of this — `/api/signals/brief/{domain}` is
declared `UNCALLED_BACKLOG` in the HUD-parity gate for exactly that reason. The routing layer and
its HTTP/HUD surface are live; wiring `autonomy/digest.py` to pull per-domain briefs is the
remaining slice. | — |
...
> **Total ORIZONT 34:** ~22 SP — all delivered (H34.1–H34.2 + H34.6 delivered 2026-07-24; H34.4
> delivered 2026-08-10; H34.3 delivered 2026-08-28).
