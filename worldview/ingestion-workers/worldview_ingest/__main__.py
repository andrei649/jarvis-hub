...
Workers: adsb | ais | tle | ew | context | recon | cep. Each fetches its source, normalizes to
the canonical envelope, and publishes to its Kafka topic. The dark-vessel detector runs as part
of the AIS pipeline (see worldview_ingest.darkwatch). The recon worker is the Layer-C insight
predictor — it owns its OWN producer/topic (osint.recon), so it gets no shared producer. The cep
worker is the insight engine — it owns BOTH its consumer (osint.recon) and producer (osint.events).
"""
