...
// AOI plus how long to dwell there. `tourSteps()` yields the transition for each leg (the camera
// pose to fly to + the transition + dwell durations), so the UI layer only has to apply
// each step with a camera flight and a timer — no animation logic leaks into this module, which
...
// can be overridden via VITE_TOUR_AOIS (read at call time so tests can stub it), falling
...
/** A camera-pose target for a tour leg (pitch/bearing optional). */
export interface TourViewState {
  ...
  pitch?: number;
  bearing?: number;
  /** Transition duration (ms) for flying into this waypoint. */
  transitionDuration: number;
}

/** A single tour step: which waypoint, the camera pose to fly to, and the timings. */
...
/**
 * Parse VITE_TOUR_AOIS into waypoints, or return the built-in default tour. The env
...
export function defaultTourWaypoints(): Waypoint[] {
  const raw = env("VITE_TOUR_AOIS").trim();
 ...
 * output. Each step carries the camera pose to fly to (with transitionDuration) plus the
