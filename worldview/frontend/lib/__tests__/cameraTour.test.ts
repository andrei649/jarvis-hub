...
test("tourSteps maps each waypoint to a deck viewState with transition + dwell (no loop)", () => {
...
test("defaultTourWaypoints returns the built-in demo AOIs when env is unset", () => {
  vi.stubEnv("NEXT_PUBLIC_TOUR_AOIS", "");
...
test("defaultTourWaypoints parses NEXT_PUBLIC_TOUR_AOIS and skips malformed entries", () => {
  vi.stubEnv("NEXT_PUBLIC_TOUR_AOIS", "Taiwan,121,24,7,5000; ,1,2; Bad,notnum,3; Suez,32.3,30.5");
  const wps = defaultTourWaypoints();
...
test("defaultTourWaypoints falls back to defaults when env has no valid entries", () => {
  vi.stubEnv("NEXT_PUBLIC_TOUR_AOIS", "garbage;,,");
