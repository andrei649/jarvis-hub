...
```bash
python scripts/verify_change.py --mode draft
python scripts/verify_change.py --mode ready
python -m pytest tests/<targeted_test>.py -q
python scripts/check_ai_workflow_policy.py
python scripts/code_health.py
```

`verify_change.py` consumes the same change-risk manifest as CI and writes an exact-source JSON and
Markdown receipt under the gitignored `.artifacts/` directory. Draft mode stops at the first red;
ready mode collects the complete selected integration evidence. Run broader suites only when the
change or merge gate requires them. After JS/CSS changes, verify
