...
The harness composes the existing offline EvalHarness, keeps task/routing/model/
provider/host evidence separate, preserves failed and negative results, and has
no path to production routing or privileged-action authority.
"""
...
def _exception_type(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) > 255 or _EXCEPTION_TYPE_RE.fullmatch(value) is None:
...
def _direct_child(base: Path, child: str) -> Path:
    ...
    if not candidate.startswith(os.path.join(resolved_base, "")):
...
class Measurement:
    ...
    def __post_init__(self) -> None:
        if self.status not in {"measured", "not_measured", "not_applicable", "failed"}:
        ...
        if self.status == "measured":
            ...
            if isinstance(self.value, (int, float)):
                _finite(self.value, "measurement", maximum=1 if self.unit == "ratio" else None)
    ...
    @classmethod
    def from_dict(cls, raw: Mapping[str, Any], *, label: str = "measurement") -> Measurement:
...
class BenchmarkCriterion:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.expected, str) or not self.expected or len(self.expected) > 10_000:
...
class BenchmarkCase:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.input_text, str) or not self.input_text or len(self.input_text) > 100_000:
        ...
        if self.privacy_class == "owner_private_local" and self.allowed_lanes != ("local",):
            raise ValueError("owner-private cases are local-only")
        if self.criterion is not None and not isinstance(self.criterion, BenchmarkCriterion):
    ...
    def enforce_lane(self, lane: BenchmarkLane) -> None:
        if lane not in self.allowed_lanes:
            raise PermissionError(
                f"case {self.case_id!r} with privacy class {self.privacy_class!r} "
                f"cannot run in {lane!r}"
            )

    def to_dict(self, *, lane: BenchmarkLane | None = None) -> dict[str, Any]:
        if self.privacy_class == "owner_private_local" and lane is None:
            raise PermissionError("owner-private cases require an explicit local serialization lane")
        if lane is not None:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> BenchmarkCase:
        ...
        if raw.get("schema") != "nerva.benchmark.v1" or raw.get("kind") != "case":
            raise ValueError("unsupported benchmark case schema")
        criterion_raw = raw.get("criterion")
        criterion = BenchmarkCriterion.from_dict(criterion_raw) if criterion_raw is not None else None
        case = cls(
            case_id=raw["case_id"],
        ...
        if raw.get("input_digest") != case.input_digest:
...
class BenchmarkObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.resources, tuple):
            raise ValueError("resource measurements must be a tuple")
        if any(not isinstance(item, ResourceMeasurement) for item in self.resources):
    ...
    def retained_evidence(self) -> BenchmarkEvidence:
        return BenchmarkEvidence(
            ...
            response_digest=hashlib.sha256(self.response.encode("utf-8")).hexdigest(),
...
class BenchmarkEvidence(Mapping[str, Any]):
    """Strict retained evidence; raw responses cannot be inserted into a run."""
    ...
    def __post_init__(self) -> None:
        ...
        for label, value in (
            ...
            if value is not None:
                _identifier(value, label)
        if not isinstance(self.response_digest, str) or _SHA256_RE.fullmatch(self.response_digest) is None:
...
def _validate_ratio(measurement: Measurement, label: str, *, statuses: set[str]) -> None:
...
def _validate_privacy(measurement: Measurement, *, statuses: set[str]) -> None:
    if not isinstance(measurement, Measurement):
...
class BenchmarkResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate is not None and not isinstance(self.candidate, BenchmarkEvidence):
            raise ValueError("candidate evidence must use the strict retained schema")
        if self.baseline is not None and not isinstance(self.baseline, BenchmarkEvidence):
        ...
        if self.baseline_error_type is not None:
            ...
            if self.baseline is not None or self.baseline_quality.status != "failed":
                raise ValueError("baseline errors require absent evidence and failed quality")
        if self.baseline_quality.status == "failed" and self.baseline_error_type is None:
            raise ValueError("failed baseline quality requires a sanitized exception type")
        if self.baseline is not None and self.baseline_error_type is not None:
            raise ValueError("baseline evidence and baseline error are mutually exclusive")
        ...
        if self.baseline is None:
            ...
            _validate_ratio(self.baseline_quality, "baseline quality", statuses={"measured"})
        elif self.status == "unscored":
            _validate_ratio(self.baseline_quality, "baseline quality", statuses={"not_measured"})
        else:
            raise ValueError("error results cannot retain baseline evidence")

        if self.status == "error":
            _validate_numeric_measurement(self.latency, "latency", "ms", statuses={"failed"})
            _validate_numeric_measurement(
                self.cost, "cost", "usd", statuses={"not_measured", "not_applicable"}
            )
            _validate_ratio(
                self.reliability,
                "reliability",
            ...
            _validate_privacy(self.privacy, statuses={"failed", "not_measured"})
        else:
            _validate_numeric_measurement(self.latency, "latency", "ms", statuses={"measured"})
            _validate_numeric_measurement(
                self.cost, "cost", "usd", statuses={"measured", "not_measured"}
            )
            _validate_ratio(
                self.reliability,
                "reliability",
            ...
            resources.append(
                ResourceMeasurement(
                    ...
                    measurement=Measurement.from_dict(
                        item["measurement"], label="resource measurement"
                    ),
                )
            )
        return cls(
...
class BenchmarkRun:
    ...
    def summary(self) -> dict[str, Any]:
        candidate = [float(r.quality.value) for r in self.results if r.quality.status == "measured"]
        baseline = [
            float(r.baseline_quality.value)
            for r in self.results
            if r.baseline_quality.status == "measured"
        ]
        return {
            ...
            "unscored": sum(r.status == "unscored" for r in self.results),
            "errors": sum(r.status == "error" for r in self.results),
            "quality_mean": round(sum(candidate) / len(candidate), 6) if candidate else None,
    ...
    def from_json(cls, payload: str) -> BenchmarkRun:
        ...
        run = cls(
            ...
            results=tuple(BenchmarkResult.from_dict(item) for item in raw["results"]),
            artifact_refs=tuple(raw["artifact_refs"]),
        )
            ...
            "results": [
                {
                    ...
                    "candidate_fields": sorted((r.candidate or {}).keys()),
                    "baseline_fields": sorted((r.baseline or {}).keys()),
                    "measurement_fields": [
                        "quality",
                        "baseline_quality",
                        "latency",
                    ...
                    "resources": [item.name for item in r.resources],
                }
                for r in self.results
            ],
        }
...
class BenchmarkStore:
    """Versioned suites plus append-only positive, negative and failed run evidence."""
    ...
    def _version_file(self, name: str, version: int) -> Path:
        return _direct_child(self._suite_dir(name), f"v{_version(version)}.jsonl")
    ...
    def save_suite(
        self, name: str, cases: Sequence[BenchmarkCase], *, lane: BenchmarkLane
    ) -> int:
        ...
        with self._version_file(name, version).open("x", encoding="utf-8") as handle:
            for case in cases:
                handle.write(
                    json.dumps(
    ...
    def record_run(self, run: BenchmarkRun) -> None:
        ...
        for case_id, result in actual.items():
            ...
            if result.task_type != case.task_type or result.privacy_class != case.privacy_class:
                raise ValueError("run result metadata must match the stored suite case")
        directory = self._suite_dir(run.suite_name)
        directory.mkdir(parents=True, exist_ok=True)
        with self._runs_file(run.suite_name).open("a", encoding="utf-8") as handle:
            handle.write(run.to_json() + "\n")

    def runs(self, name: str, *, last_n: int = 20) -> tuple[BenchmarkRun, ...]:
...
class BenchmarkHarness:
    def __init__(
        ...
        *,
        ...
        self.baseline_id = _identifier(baseline_id, "baseline id") if baseline_id else None
    ...
    async def _run_case(self, case: BenchmarkCase) -> BenchmarkResult:
        try:
            ...
            return BenchmarkResult(
                ...
                None,
                None,
                Measurement("failed", source="candidate.runner"),
                Measurement("not_measured"),
                Measurement("failed", source="candidate.runner"),
                Measurement("not_measured"),
                Measurement("measured", 0.0, "ratio", "candidate.runner"),
                Measurement("failed", source="candidate.runner"),
        ...
        if self.baseline is not None:
            try:
                baseline = await self._observe(self.baseline, case.input_text)
            except Exception as exc:
                baseline_error = type(exc).__name__

        quality, passed, status = await _quality(case, candidate.response)
        if baseline is not None:
            baseline_quality, _, _ = await _quality(case, baseline.response)
        elif self.baseline is None:
            baseline_quality = Measurement("not_applicable")
        else:
            baseline_quality = Measurement("failed", source="baseline.runner")
        ...
        return BenchmarkResult(
            ...
            baseline.retained_evidence() if baseline else None,
            quality,
            baseline_quality,
            _measure(candidate.latency_ms, "ms", "benchmark.harness"),
            _measure(candidate.cost_usd, "usd", "candidate.runner"),
            _measure(candidate.reliability, "ratio", "candidate.runner"),
            Measurement("measured", candidate.privacy_effect, "classification", "candidate.runner"),
            candidate.resources,
            baseline_error_type=baseline_error,
        )

    @staticmethod
    async def _observe(runner: BenchmarkRunner, prompt: str) -> BenchmarkObservation:
        started = time.perf_counter()
        observation = await runner(prompt)
        if not isinstance(observation, BenchmarkObservation):
            raise TypeError("benchmark runners must return BenchmarkObservation")
        if observation.latency_ms is None:
            observation = replace(
                observation, latency_ms=(time.perf_counter() - started) * 1_000
            )
        return observation


async def _quality(
    case: BenchmarkCase, response: str
    ...
    if case.criterion.kind == "contains":
        eval_case = EvalCase(case.case_id, case.input_text, expect_contains=case.criterion.expected)
    else:
        ...
        eval_case = EvalCase(case.case_id, case.input_text, scorer=exact)
    row = (await EvalHarness(observed).run([eval_case]))["results"][0]
    passed = bool(row["passed"])
    return (
        Measurement(
            "measured", float(row["score"]), "ratio", "observability.eval.EvalHarness"
        ),
        passed,
...
def _measure(value: float | None, unit: str, source: str) -> Measurement:
    if value is None:
        return Measurement("not_measured")
    return Measurement("measured", _finite(value, unit), unit, source)


def current_router_runner(
    router: Any, agents: Mapping[str, Any], *, host_id: str = "in-process"
) -> BenchmarkRunner:
    """Observe the existing router without changing the returned Intent or live routing."""
...
class KeywordRouteBaseline:
    """Transparent exact-token/phrase baseline for current-router comparisons."""

    def __init__(self, rules: Mapping[str, str], *, fallback_route: str = "jarvis") -> None:
        if not rules:
            raise ValueError("keyword baseline requires at least one rule")
        self.rules = tuple(
            (phrase.casefold().strip(), _identifier(route, "baseline route"))
        ...
        self.fallback_route = _identifier(fallback_route, "fallback route")
