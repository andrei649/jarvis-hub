...
ACTION_CAPABILITY_MANIFESTS: dict[str, CapabilityManifest] = {
    ...
    "channel.reply": _action(
        "channel.reply",
        "Send a governed reply to a live channel inbox thread after approval.",
        required=("thread_id", "text"),
        risk="sensitive",
        supports=("reply", "send"),
        rollback=RollbackContract(
            mode="cancel",
            description="Reject or cancel the queued draft before the approved send.",
            limitations="A delivered message cannot be recalled from the channel.",
        ),
        implementation="agents.core.channel_reply:ChannelReplyBroker.request",
        contract_ref="agents.core.channel_reply:CHANNEL_REPLY_CONTRACT",
    ),
    "skill.install": _action(
        "skill.install",
        "Install a verified acquired capability package after permanent owner approval.",
        required=("name", "package_hash", "receipt_hash"),
        risk="irreversible_or_money",
        supports=("propose", "install"),
        rollback=RollbackContract(
            mode="restore",
            description="Roll the acquired package back to its previous installed version.",
            limitations="A first install has no previous version; the package is disabled instead.",
        ),
        implementation="agents.core.acquisition.promotion:PromotionBroker.propose",
        contract_ref="agents.core.skills.marketplace:SKILL_INSTALL_CONTRACT",
    ),
    ...
    "terminal.exec": _action(
        "terminal.exec",
        "Run one governed shell command on the host inside an approved working root.",
        required=("argv", "cwd", "approved_task_id"),
        risk="irreversible_or_money",
        supports=("execute",),
        rollback=RollbackContract(
            mode="none",
            description="A completed process cannot be undone; recovery is the owner's, per command.",
            limitations="The hardline denylist and the durable approval floor are the only guards before execution.",
        ),
        implementation="agents.core.environments.execution:GovernedTargetRunner.run",
        contract_ref="agents.core.environments.terminal_contract:TERMINAL_EXEC_CONTRACT",
    ),
    "file.write": _action(
        "file.write",
        "Write or delete one file inside the owner's declared file roots, under a byte cap.",
        required=("path", "op"),
        risk="sensitive",
        supports=("write", "delete"),
        rollback=RollbackContract(
            mode="restore",
            description="Restore the pre-write bytes from the snapshot taken before the write.",
            automatic=False,
            handler_ref="agents.core.file_tools:restore_snapshot",
            limitations="A snapshot older than its retention window is pruned; a create has no previous bytes to restore.",
        ),
        implementation="agents.core.file_tools:FileTools.write_file",
        contract_ref="agents.core.file_tools:FILE_WRITE_CONTRACT",
    ),
    "model.pull": _action(
        "model.pull",
        "Pull a hardware-tiered local model into the loopback Ollama store, under a size cap.",
        required=("model",),
        risk="reversible",
        supports=("execute",),
        rollback=RollbackContract(
            ...
            limitations="Bandwidth already spent is not recoverable; a partial blob is removed by Ollama's own cleanup.",
        ),
    ...
    "report.export": _action(
        "report.export",
        "Write the redacted day report to the owner's reports directory as JSON or HTML.",
        required=("date", "format", "path"),
        risk="sensitive",
        supports=("json", "html"),
        rollback=RollbackContract(
            mode="restore",
            description="Delete the exported file; the report itself is rebuilt on demand from the intent log.",
            limitations="A copy the owner moved or shared before deletion is outside Nerva's reach.",
        ),
