...
through its twin, ``frontend/src/lib/todo-tree.ts``.
...
* an item whose parent is missing, itself, or not an id at all renders at depth 0 in its
  place (when two items share an id, the first one holds it);
...
def tree(
    ...
    """``[(item, depth), ...]`` in depth-first order; every item exactly once."""
    rows = list(items)
    first: dict[Any, int] = {}
