...
EpisodeOperation = Literal['open', 'settle', 'consolidate', 'correct', 'merge', 'split', 'tombstone', 'migrate']
...
_FORBIDDEN_RAW_KEYS = {'content', 'messages', 'raw', 'raw_text', 'raw_transcript', 'transcript', 'turns'}
...
class EpisodeReference:
    ...
    def __post_init__(self) -> None:
        for value, name in ((self.reference_id, 'reference_id'), (self.source_id, 'source_id'), (self.record_id, 'record_id'), (self.source_kind, 'source_kind'), (self.source_schema, 'source_schema'), (self.deletion_root_id, 'deletion_root_id')):
    ...
    def from_atlas(cls, observation: AtlasObservation, *, role: EpisodeReferenceRole='source') -> EpisodeReference:
        ...
        return cls.build(role=role, source_id=observation.source.source_id, record_id=observation.source.record_id, source_kind=observation.source.source_kind, source_schema=observation.schema, privacy_class=observation.privacy_class, integrity_sha256=observation.integrity_sha256, occurred_at=observation.valid_from, deletion_root_id=observation.lineage.source_record_id, confidence=observation.confidence, projection_id=observation.observation_id)

    @property
    def expected_reference_id(self) -> str:
        material = {'role': self.role, 'source_id': self.source_id, 'record_id': self.record_id, 'source_kind': self.source_kind, 'source_schema': self.source_schema, 'projection_id': self.projection_id}
...
class EpisodeAssertion:
    ...
    def __post_init__(self) -> None:
        ...
        refs = _validated_string_tuple(self.evidence_reference_ids, 'assertion evidence references', allow_empty=False)
    ...
    def build(cls, *, kind: EpisodeAssertionKind, text: str, evidence_reference_ids: tuple[str, ...], confidence: AtlasConfidence) -> EpisodeAssertion:
        ...
        return cls(assertion_id=assertion_id, kind=kind, text=text, evidence_reference_ids=refs, confidence=confidence)

    @property
    def expected_assertion_id(self) -> str:
        material = {'kind': self.kind, 'text': self.text, 'evidence_reference_ids': self.evidence_reference_ids, 'confidence': asdict(self.confidence)}
        return 'episode:assertion:' + _sha256(material)[:24]

    def eligible_for_settlement(self, floor: float=_SETTLEMENT_CONFIDENCE_FLOOR) -> bool:
        if self.kind == 'direct':
            return True
        return self.confidence.status == 'measured' and self.confidence.value is not None and (float(self.confidence.value) >= floor)
...
class EpisodeRecord:
    ...
    def __post_init__(self) -> None:
        ...
        participants = _validated_string_tuple(self.participants, 'participants', allow_empty=False)
        ...
        for assertion in (self.goal, self.summary, self.significance):
            ...
            tombstoned_ids = {reference.reference_id for reference in self.references if reference.tombstoned}
            ...
            if self.state in {'settled', 'consolidated'} and (not assertion.eligible_for_settlement()):
                raise ValueError('Low-confidence inference cannot be promoted to settled history')
        parents = _validated_string_tuple(self.parent_episode_ids, 'parent_episode_ids', allow_empty=True)
        object.__setattr__(self, 'parent_episode_ids', parents)
        successors = _validated_string_tuple(self.superseded_by_episode_ids, 'superseded_by_episode_ids', allow_empty=True)
    ...
    def build(cls, *, state: EpisodeState, participants: tuple[str, ...], started_at: float, ended_at: float | None, references: tuple[EpisodeReference, ...], goal: EpisodeAssertion | None, summary: EpisodeAssertion | None, significance: EpisodeAssertion | None, created_at: float, updated_at: float, episode_id: str | None=None, revision: int=1, parent_episode_ids: tuple[str, ...]=(), supersedes_record_id: str | None=None, superseded_by_episode_ids: tuple[str, ...]=()) -> EpisodeRecord:
        ...
        return cls(episode_id=episode_id, record_id=record_id, revision=revision, state=state, participants=participants, started_at=float(started_at), ended_at=None if ended_at is None else float(ended_at), references=references, goal=goal, summary=summary, significance=significance, parent_episode_ids=parent_episode_ids, supersedes_record_id=supersedes_record_id, superseded_by_episode_ids=superseded_by_episode_ids, created_at=float(created_at), updated_at=float(updated_at), integrity_sha256=integrity_sha256)
    ...
    def record_material(self) -> dict[str, Any]:
        return _record_material(episode_id=self.episode_id, revision=self.revision, state=self.state, participants=self.participants, started_at=self.started_at, ended_at=self.ended_at, references=self.references, goal=self.goal, summary=self.summary, significance=self.significance, parent_episode_ids=self.parent_episode_ids, supersedes_record_id=self.supersedes_record_id, superseded_by_episode_ids=self.superseded_by_episode_ids, created_at=self.created_at, updated_at=self.updated_at)

    def canonical_payload(self, *, include_integrity: bool=True) -> dict[str, Any]:
        payload = {**self.record_material(), 'record_id': self.record_id}
        payload.update({'schema': self.schema, 'authority': self.authority, 'can_authorize': self.can_authorize, 'can_execute': self.can_execute, 'can_mark_complete': self.can_mark_complete})
        if include_integrity:
            payload['integrity_sha256'] = self.integrity_sha256
        return payload

    def verify_integrity(self) -> bool:
        return self.integrity_sha256 == _sha256({**self.record_material(), 'record_id': self.record_id})
    ...
    def from_payload(cls, payload: dict[str, Any]) -> EpisodeRecord:
        ...
        return cls(episode_id=payload['episode_id'], record_id=payload['record_id'], revision=payload['revision'], state=payload['state'], participants=tuple(payload['participants']), started_at=payload['started_at'], ended_at=payload.get('ended_at'), references=references, goal=goal, summary=summary, significance=significance, parent_episode_ids=tuple(payload.get('parent_episode_ids', ())), supersedes_record_id=payload.get('supersedes_record_id'), superseded_by_episode_ids=tuple(payload.get('superseded_by_episode_ids', ())), created_at=payload['created_at'], updated_at=payload['updated_at'], integrity_sha256=payload['integrity_sha256'])
...
def _reference_from_payload(payload: dict[str, Any]) -> EpisodeReference:
    ...
    return EpisodeReference(reference_id=payload['reference_id'], role=payload['role'], source_id=payload['source_id'], record_id=payload['record_id'], source_kind=payload['source_kind'], source_schema=payload['source_schema'], privacy_class=payload['privacy_class'], integrity_sha256=payload['integrity_sha256'], occurred_at=payload['occurred_at'], deletion_root_id=payload['deletion_root_id'], confidence=AtlasConfidence(confidence['status'], confidence.get('value'), confidence.get('source')), projection_id=payload.get('projection_id'), tombstoned=payload.get('tombstoned', False), deleted_at=payload.get('deleted_at'))

def _assertion_from_payload(payload: dict[str, Any] | None) -> EpisodeAssertion | None:
    if payload is None:
        return None
    if payload.get('schema') != 'nerva.episode.assertion.v1':
        raise ValueError('Unsupported episode assertion schema')
    confidence = payload['confidence']
    return EpisodeAssertion(assertion_id=payload['assertion_id'], kind=payload['kind'], text=payload['text'], evidence_reference_ids=tuple(payload['evidence_reference_ids']), confidence=AtlasConfidence(confidence['status'], confidence.get('value'), confidence.get('source')))

def _legacy_reference_from_payload(payload: dict[str, Any]) -> EpisodeReference:
    confidence = payload['confidence']
    return EpisodeReference.build(role=payload['role'], source_id=payload['source_id'], record_id=payload['record_id'], source_kind=payload['source_kind'], source_schema=payload['source_schema'], privacy_class=payload['privacy_class'], integrity_sha256=payload['integrity_sha256'], occurred_at=payload['occurred_at'], deletion_root_id=payload['deletion_root_id'], confidence=AtlasConfidence(confidence['status'], confidence.get('value'), confidence.get('source')), projection_id=payload.get('projection_id'))

def _legacy_assertion_from_payload(payload: dict[str, Any] | None, *, reference_aliases: dict[str, str]) -> EpisodeAssertion | None:
    if payload is None:
        return None
    confidence = payload['confidence']
    try:
...
def _merge_references(existing: tuple[EpisodeReference, ...], additional: tuple[EpisodeReference, ...]) -> tuple[EpisodeReference, ...]:
...
def _require_current_state(record: EpisodeRecord, allowed: set[str], operation: str) -> None:
...
def _validated_string_tuple(value: Any, name: str, *, allow_empty: bool) -> tuple[str, ...]:
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            if str(key).casefold() in _FORBIDDEN_RAW_KEYS:
                raise ValueError(f'Legacy episode migration rejects raw content at {path}.{key}')
