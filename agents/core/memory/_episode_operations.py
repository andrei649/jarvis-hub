...
class EpisodeAuditEvent:
    """Deterministic integrity record for one manual episode mutation."""
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            ...
            normalized = _validated_string_tuple(
                getattr(self, name),
                name,
                allow_empty=True,
            )
    ...
    def build(
        ...
        *,
        ...
        affected_reference_ids = _validated_string_tuple(
            affected_reference_ids,
            "affected_reference_ids",
            allow_empty=True,
        )
        material = _audit_material(
            ...
            input_episode_ids=input_episode_ids,
            output_episode_ids=output_episode_ids,
    ...
    def audit_material(self) -> dict[str, Any]:
        return _audit_material(
            ...
            input_episode_ids=self.input_episode_ids,
            output_episode_ids=self.output_episode_ids,
            affected_reference_ids=self.affected_reference_ids,
        )
    ...
    def verify_integrity(self) -> bool:
        return self.integrity_sha256 == _sha256(
            {**self.audit_material(), "audit_id": self.audit_id}
        )
...
class EpisodeMutation:
    ...
    def __post_init__(self) -> None:
        ...
        actual_input_record_ids = tuple(
            sorted(record.record_id for record in self.before)
        )
        actual_output_record_ids = tuple(
            sorted(record.record_id for record in self.after)
        )
        if actual_input_record_ids != self.audit.input_record_ids:
            raise ValueError("Episode mutation before records do not match audit")
        if actual_output_record_ids != self.audit.output_record_ids:
            raise ValueError("Episode mutation after records do not match audit")

        actual_input_episode_ids = tuple(
            sorted(record.episode_id for record in self.before)
        )
        actual_output_episode_ids = tuple(
            sorted(record.episode_id for record in self.after)
        )
        if actual_input_episode_ids != self.audit.input_episode_ids:
            raise ValueError("Episode mutation input episode IDs do not match audit")
        if actual_output_episode_ids != self.audit.output_episode_ids:
        ...
        if set(self.audit.affected_reference_ids) - mutation_reference_ids:
            raise ValueError(
...
class EpisodeQuery:
    ...
    def __post_init__(self) -> None:
        terms = _validated_string_tuple(
            tuple(
                _normalize_text(term)
                for term in self.situation_terms
        ...
        if not (
            self.situation_terms or self.outcome_record_ids or self.participants
        ):
...
def open_episode(
    *,
    ...
    audit = EpisodeAuditEvent.build(
        ...
        before=(),
        after=after,
        affected_reference_ids=tuple(ref.reference_id for ref in references),
    )
...
def settle_episode(
    ...
    *,
    ...
    return _mutation(
        ...
        (record,),
        (settled,),
        tuple(ref.reference_id for ref in additional_references),
    )
...
def consolidate_episode(
    ...
    *,
    ...
    consolidated = EpisodeRecord.build(
        episode_id=record.episode_id,
        revision=record.revision + 1,
        state="consolidated",
        participants=record.participants,
...
def correct_episode(
    ...
    *,
    ...
    occurred_at: float,
    reason: str,
    participants: tuple[str, ...] | object = _KEEP,
    started_at: float | object = _KEEP,
    ended_at: float | None | object = _KEEP,
    references: tuple[EpisodeReference, ...] | object = _KEEP,
    goal: EpisodeAssertion | None | object = _KEEP,
    summary: EpisodeAssertion | None | object = _KEEP,
    significance: EpisodeAssertion | None | object = _KEEP,
) -> EpisodeMutation:
    _require_current_state(
        record,
        {"open", "settled", "consolidated"},
        "correct",
    )
    _require_operation_time((record,), occurred_at, "correct")
    corrected = EpisodeRecord.build(
        episode_id=record.episode_id,
        revision=record.revision + 1,
        state=record.state,
        participants=record.participants if participants is _KEEP else participants,
        started_at=record.started_at if started_at is _KEEP else started_at,
        ended_at=record.ended_at if ended_at is _KEEP else ended_at,
        references=record.references if references is _KEEP else references,
        goal=record.goal if goal is _KEEP else goal,
        summary=record.summary if summary is _KEEP else summary,
        significance=record.significance
        if significance is _KEEP
        else significance,
        parent_episode_ids=record.parent_episode_ids,
        supersedes_record_id=record.record_id,
        created_at=record.created_at,
...
def merge_episodes(
    ...
    *,
    ...
    merged = EpisodeRecord.build(
        state="settled" if all_closed else "open",
        participants=participants,
        ...
        summary=summary,
        significance=significance,
        parent_episode_ids=tuple(record.episode_id for record in records),
        created_at=occurred_at,
        updated_at=occurred_at,
    )
    superseded = tuple(
        _superseded_revision(
            record,
            successor_episode_ids=(merged.episode_id,),
            occurred_at=occurred_at,
        )
        for record in records
    )
...
def split_episode(
    record: EpisodeRecord,
    partitions: tuple[EpisodePartition, ...],
    *,
    actor_id: str,
    occurred_at: float,
    reason: str,
    ...
    all_reference_ids = {
        reference.reference_id for reference in record.references
    }
    partition_ids = [
        reference_id
        for partition in partitions
        for reference_id in partition.reference_ids
    ]
    if len(partition_ids) != len(set(partition_ids)):
        raise ValueError("Episode split partitions must be disjoint")
    if set(partition_ids) != all_reference_ids:
        raise ValueError(
            "Episode split partitions must cover every reference exactly once"
        )
    by_id = {
        reference.reference_id: reference for reference in record.references
    }
    children = tuple(
        EpisodeRecord.build(
            state="open" if partition.ended_at is None else "settled",
            participants=partition.participants,
            started_at=partition.started_at,
            ended_at=partition.ended_at,
            references=tuple(
                by_id[reference_id]
                for reference_id in partition.reference_ids
            ),
            ...
            parent_episode_ids=(record.episode_id,),
            created_at=occurred_at,
            updated_at=occurred_at,
        )
        for partition in partitions
    ...
    superseded = _superseded_revision(
        record,
        successor_episode_ids=tuple(child.episode_id for child in children),
        occurred_at=occurred_at,
    )
...
def tombstone_sources(
    record: EpisodeRecord,
    *,
    deletion_root_ids: tuple[str, ...],
    deleted_at: float,
    actor_id: str,
    occurred_at: float,
    reason: str,
    ...
    affected_references = tuple(
        reference
        for reference in record.references
        if reference.deletion_root_id in root_set
    )
    if not affected_references:
        raise ValueError("Episode tombstone did not match any source lineage")
    if any(deleted_at < reference.occurred_at for reference in affected_references):
        raise ValueError("Episode deletion timestamp precedes source occurrence")
    if any(
        reference.deleted_at is not None and deleted_at < reference.deleted_at
        for reference in affected_references
    ):
        raise ValueError("Episode deletion timestamp reverses an existing tombstone")

    affected = tuple(
        reference.reference_id for reference in affected_references
    )
    ...
    tombstoned = EpisodeRecord.build(
        episode_id=record.episode_id,
        revision=record.revision + 1,
        state=record.state,
        participants=record.participants,
        ...
        ended_at=record.ended_at,
        references=references,
        goal=scrub(record.goal),
        summary=scrub(record.summary),
        significance=scrub(record.significance),
        parent_episode_ids=record.parent_episode_ids,
        supersedes_record_id=record.record_id,
        created_at=record.created_at,
        updated_at=occurred_at,
...
def trace_source_derivatives(
    ...
    reference_ids = tuple(
        reference.reference_id
        for reference in record.references
        if reference.deletion_root_id == deletion_root_id
    )
    ...
    assertion_ids = tuple(
        ...
        and set(assertion.evidence_reference_ids) & reference_set
    )
    tombstoned = all(
        reference.tombstoned
        for reference in record.references
        if reference.reference_id in reference_set
    )
    return EpisodeDerivativeTrace(
        deletion_root_id=deletion_root_id,
        episode_id=record.episode_id,
        episode_record_id=record.record_id,
...
def retrieve_episodes(
    ...
    for record in records:
        ...
        if wanted_outcomes:
            outcome_ids = {
                reference.record_id
                for reference in record.outcome_references
        ...
        if query.situation_terms:
            ...
            text = _normalize_text(active_text)
            term_hits = sum(
                1 for term in query.situation_terms if term in text
            )
            if not term_hits:
                continue
...
def migrate_manual_episode_v0(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("Legacy episode payload is malformed") from exc
    ...
    assertions = {
        name: _legacy_assertion_from_payload(
            payload.get(name),
            reference_aliases=reference_aliases,
        )
        for name in ("goal", "summary", "significance")
    }
    record = EpisodeRecord.build(
        state=state,
        participants=participants,
        ...
        references=references,
        goal=assertions["goal"],
        summary=assertions["summary"],
        significance=assertions["significance"],
        created_at=payload.get("created_at", occurred_at),
        updated_at=occurred_at,
...
def _require_operation_time(
    ...
    if any(occurred_at < record.updated_at for record in records):
        raise ValueError(
            f"Episode {operation} cannot precede the current revision"
        )
...
def _superseded_revision(
    record: EpisodeRecord,
    *,
    successor_episode_ids: tuple[str, ...],
    occurred_at: float,
) -> EpisodeRecord:
    return EpisodeRecord.build(
        episode_id=record.episode_id,
        revision=record.revision + 1,
        state="superseded",
        participants=record.participants,
