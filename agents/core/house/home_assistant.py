...
class HomeAssistantAdapter:
    ...
    async def snapshot(self) -> HouseSnapshot:
        ...
        try:
            # _runtime_endpoint re-resolves the HA origin via socket.getaddrinfo;
            # inline it blocked the event loop on every snapshot poll. The HTTP
            # transport below is already async — keep DNS in a worker thread too.
            origin, pinned_ip, host, port = await asyncio.to_thread(
