...
def _build_mcp_mutating_route_tools():
    ...
    even with both kill-switches on. The transport (``_mcp_handle_message``)
    extracts the credential from the request headers and passes it to the server.
    Residual gap: a leaked token alone drives the write surface, and the
    unset-token posture trusts the in-process call unconditionally — see the
    caveat block in ``agents/core/mcp/route_tools.py``.
    """
