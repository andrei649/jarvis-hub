...
it('says the first run fires now when the hub queued one', () => {
  expect(armNote({job, first_run: {status: 'queued'}, confirmation: 'first run now, then every weekday at 8:00 (0 8 * * 1-5)'}))
    .toBe('armed · first run now, then every weekday at 8:00 (0 8 * * 1-5)');
});
