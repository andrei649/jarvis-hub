...
import { imageStatus, proposeImage, imageTask, imageBlob, type ImageTask } from '../api/images';
...
export function ImagesPanel() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (submitting.current || submitted !== null || !configuration?.configured || !prompt.trim()) return;
    ...
    try {
      const id = await proposeImage(prompt, controller.signal);
  ...
  return <Card title="IMAGES" sub="Local generation · owner instance" live={asLive(configuration)}>
    ...
    {submitted === null ? <><form onSubmit={submit}>
      ...
      <button className="tool-btn" type="submit" disabled={!configuration?.configured || !prompt.trim()}>Propose image</button>
    </form>
      ...
      {imageUrl && task?.artifact && <figure style={{ margin: '12px 0' }}>
        ...
        <figcaption><a href={imageUrl} download={'nerva-image-' + task.artifact.id + '.png'} style={{ color: 'var(--accent-light)' }}>Download PNG</a></figcaption>
      </figure>}
