...
export function useLiveModes(): number {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      await apiGet('/api/security/audit/intent').then((a: any) => {
        ...
        if (acts && acts.length) set('AUDIT_CHAIN', acts.slice(0, 6).map((r: any, i: number) => ({ verb: String(r.action || r.verb || 'ACTION').toUpperCase().slice(0, 6), x: r.why || r.detail || r.action || '', hash: String(r.hash || '').slice(0, 4) || String(i), prev: String(r.prev || '').slice(0, 4) || '0000', t: r.t || r.ts || '' })));
      }).catch(() => {});
      await apiGet('/api/payments').then((p: any) => {
        const list = arr(p, 'payments');
        if (list && list.length) set('PAYMENTS', list.map((x: any) => ({ pcap: x.mandate_id || x.payee || '', desc: x.memo || x.desc || '', amt: String(x.currency || '') + (x.amount != null ? x.amount : ''), state: x.state || x.status || 'pending' })));
      }).catch(() => {});

      // ADMIN — local models + plugin registry
      await apiGet('/api/models/local').then((m: any) => {
        const models = arr(m, 'models');
        if (models && models.length) set('ADMIN', { ...V2.ADMIN, models: models.map((x: any) => ({ name: x.name || x.id, type: x.type || (x.local ? 'local' : 'cloud'), backend: x.backend || x.provider || '', ctx: x.ctx || '—', status: x.status || (x.active ? 'loaded' : 'ready'), use: x.use || '' })) });
      }).catch(() => {});
      await apiGet('/plugins').then((p: any) => {
        const plugins = arr(p, 'plugins');
        if (plugins && plugins.length) set('ADMIN', { ...V2.ADMIN, plugins: plugins.map((x: any) => ({ name: x.name || x.id, scope: (x.allowed_domains && x.allowed_domains[0]) || x.scope || x.network_access || '', net: String(x.network_access || x.net || '').toLowerCase(), on: x.enabled !== false })) });
      }).catch(() => {});
