...
**Status:** 15 domains extracted into per-domain routers under `core/routers/` (each its own commit,
route-parity + full suite green throughout): rooms, notes, actions, arena, review, quality, security, skills,
data_spaces, secrets, mesh, autonomy, models_llm, oauth, **memory_kg** (`/api/kg/*` + `/api/memory/*`). All use
the Phase-1 shared kernel — zero static `from agents import web`. **web.py: 5,037 → 3,001 LOC** so far.
