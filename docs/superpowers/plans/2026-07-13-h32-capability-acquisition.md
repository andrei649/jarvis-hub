...
- [ ] Rebase on current main and confirm Claude's stale Agent Runtime worktree is abandoned or
  integrated before touching `agent_runtime.py`, ToolRPC, orchestrator, or settings.
- [ ] Require `Sandbox.active_backend()` to be `docker` or `wasm`. `disabled`, host subprocess,
  and mocks cannot satisfy H32.4/H32.7.
- [ ] Build a dedicated hostile-code acquisition profile: non-root uid/gid, all capabilities
...
- [ ] Run a real hostile smoke receipt and bind the actual OCI digest/runtime attestation, never a
  mutable tag string.
- [ ] If Docker/WASM is unavailable, continue request/reuse/research code only; mark acquisition
...
- [ ] Red tests: only an explicit bounded tool/capability miss creates a request; normal unanswered
...
- [ ] Add registry state/source for `missing` without weakening SEAM/WIRED/VERIFIED/GA readiness.
- [ ] Add an injected Agent Runtime gap callback after honest registry/tool refusal; capture a
  redacted goal and hash under runtime data.
- [ ] Encrypt request goals/projections at rest; cap at 1,000 requests/16 MiB and 30 days unless an
...
- [ ] Provide read/status APIs only after auth/parity classifications are designed.
- [ ] Run Agent Runtime/registry/store tests, Ruff/Bandit, review, and commit.
...
- [ ] Red tests for exact/semantic goal ranking, deterministic ties, disabled/untrusted/quarantined
...
- [ ] Persist decision provenance and terminal outcome. Define reuse rate as
  `reused / (reused + generated)`; blocked/abandoned requests remain visible but do not inflate it.
- [ ] Verify reused capabilities execute through their existing ToolRPC/kernel/sandbox boundary.
- [ ] Run skill/marketplace/registry tests, review, and commit.
...
- [ ] Red tests: default-off/no backend, explicit owner consent for any network/cloud hop, local
...
- [ ] Stream every fetch under a hard received-byte cap independent of `Content-Length`; abort
...
- [ ] Do not implicitly fall back to DuckDuckGo. Draft with a strict-local model seam.
- [ ] Wrap `ground_plan` with a non-empty references/steps hard gate; every implementation step
  cites a fetched source id/hash and phantom citations fail.
- [ ] Store only bounded source extracts and hashes; research content remains tainted and can never
  auto-authorize installation or execution.
- [ ] Encrypt extracts at rest, cap each source/plan and the aggregate store, expire research after
  7 days, and purge it with the capability request unless retained as hash-only audit evidence.
- [ ] Run websearch/egress/grounding/taint tests, review, and commit.
...
- [ ] Red tests: strict-local generator route, runtime quarantine location, no repository `skills/`
...
- [ ] H32 v1 generated code is stdlib-only. Imports outside an explicit stdlib subset fail; no
  runtime package installation or network dependency resolution is permitted.
- [ ] Run generated tests in Docker/WASM with read-only source, explicit scratch mount, no network,
  caps, timeout/cancellation, bounded output, and cleanup.
- [ ] Do not trust model-generated tests alone. Add system-owned goal/entrypoint contract tests and
...
- [ ] Produce an immutable receipt binding source, plan, model route, code, test, sandbox image/
  runtime, config, output, exit status, and timestamps by hashes.
- [ ] A failing/missing/timed-out/tampered test never creates an approval proposal.
- [ ] Encrypt quarantine metadata/artifacts at rest, cap each package and total store, expire
  rejected/abandoned artifacts in 7 days, and support immediate purge/cancel on disable/revoke.
- [ ] Run sandbox/quarantine/trust tests and one real isolated smoke, review, and commit.
...
- [ ] Add a kernel-mediated `skill.install` action with permanent approval hard floor. Earned
  confidence cannot auto-install generated code.
- [ ] Persist a bounded canonical proposal. At approved execution, revalidate task status and all
  receipt/artifact/source hashes to prevent TOCTOU, then sign and install through the reviewed
  marketplace path atomically.
- [ ] Fail closed without a managed signing key. Sign a canonical manifest covering every package
...
- [ ] Install into a runtime acquired-package store outside repository `skills/`, with persistent
...
- [ ] Register a concrete ToolRPC -> acquisition sandbox runner only after promotion. Every call
...
- [ ] Signed means integrity/provenance, never permission for in-process import.
- [ ] Register skill/tool identity at low confidence and generalize outcome projection beyond
  action records without manufacturing evidence.
- [ ] Use a durable staged transaction journal across SQLite/package store/ToolRPC registration:
...
- [ ] Run kernel/action-auth/signing/marketplace/rollback tests, review, and commit.
...
- [ ] Record request/reuse/research/generation/sandbox/approval/signature/install/registry/
  execution/revocation/rollback events with hashes and actor/task ids, not raw secrets or sources.
- [ ] Encrypt any non-hash ledger projections, cap the ledger at 100,000 rows/64 MiB with 90-day
  detailed retention plus hash-chain summaries, and define owner purge/export semantics.
- [ ] Admin mutations and user-readable status use separate guarded endpoints; expose honest
  disabled/blocked/quarantined/installed/revoked/degraded states plus reuse rate.
- [ ] Add revoke/rollback controls that cannot bypass kernel/receipt checks.
- [ ] Update route/OpenAPI/auth/HUD/mobile parity and generated types in the same slice.
- [ ] Run backend/frontend/mobile tests, review, and commit.

## Task 7 — H32.7 S2 acquisition reality proof and truth sync

- [ ] Under a real Docker/WASM backend, prove a net-new hermetic skill request: miss -> no reuse ->
...
- [ ] Negative cases prove no host execution, no install before approval, hash tamper refusal,
  kernel halt, revoke/uninstall, upgrade rollback, and no network from generated code.
- [ ] Report reuse metric honestly and keep optional live web research separately gated.
- [ ] Run all H32 plus registry/ToolRPC/sandbox/marketplace/kernel/reality/parity, full repository,
  frontend/mobile, Ruff/Bandit/diff-check/status-sync.
- [ ] Fresh final review, truth sync, draft PR, CI monitoring, merge.
