...
| S-003 | 2026-09-16 | electric-gale | `nerva send --list` when no channel is ready prints one honest line under the list — "none ready yet — Nerva has things to say and nowhere to say them; connect one above". It is the first empty state a stranger meets on the way to their first automation, and it now sounds like someone is home. One string, one test, text output only (`--json` untouched), deletable in one commit. | `agents/cli/nerva.py` (`cmd_send`, `--list`) | live |
