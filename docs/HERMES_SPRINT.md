...
47 tests; every fix red-proofed by neutering it and naming the test that went red, then restoring byte-identically.
...
Two ways this control can quietly cover less than it claims are named in the row rather than smoothed over: the `loggerDict` walk falls back to root-only if it raises, and `_cover` swallows a per-handler failure. In both cases the outcome is the pre-slice status quo rather than a regression — but a security control that can silently degrade deserves to say so, and now the record does.
