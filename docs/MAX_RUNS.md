...
| 001 | 2026-08-11 | nimble-beacon | ADV-087 closed: the action-capability reality probe now resolves `manifest.implementation` to the real actuator (fail-closed) and records it as evidence — all nine `qa_audit_probes.py` claims CLOSED; ⭐B0/A1 chapter-15 prep | #894 | — | H23 tail is owner-gated (B0/RTX); next agent-side leverage per §3.1b: a `PARTIAL` from the audit follow-ups — SEC-B5 taint-by-dataflow tail or SEC-B6 read-route auth-matrix hardening |
