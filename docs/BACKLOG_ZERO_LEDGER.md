...
| AUD-0 | Hardening audit, line 1965 | OWNER-DECISION | S | Same underlying ask as A2/H23.23 ("record AUD-0"); not separately actionable. **Recorded 2026-09-01** — the owner declined to narrow (expanded six-pillar 1.0 gate, 2026-07-11); the ~44 Null-railed modules stay reachable by choice | **CLOSED** (see the `BACKLOG.md` AUD-0 row AC cell) |
...
| GAP-0 | Nerva vs Hermes gap analysis, line 604 | OWNER-DECISION | — | "Distribution is the binding constraint" — feeds directly into A7; no code fix, a GTM/positioning call. **Decided 2026-09-01:** first-value path = one-step partner bootstrap (🚧1) + first-30-min local loop (🚧4), metric = activation rate; H23.30 sequenced after as reach | **DECIDED 2026-09-01** (see the `BACKLOG.md` GAP-0 bullet) |
...
| SEC-B6 | Governance-rails audit, line 770 | R3-REVIEW-GATED | S | Code landed via #896 with green CI, but zero independent review was ever submitted; needs a fresh exact-head R3 review only, no new code. **Owner decision 2026-09-01:** the #894 directive is amended — an independent post-merge review of the merged artifact suffices; PASS/HOLD: pending | SHIPPED (after review) |
...
| SEC-B4 | Governance-rails audit, line 723 | R3-REVIEW-GATED | M | SSRF IP-pinning for the Playwright path + central `PluginHTTPClient`; needs a live network/browser host to demonstrate (chapter 15 ADV-142). **R3 post-merge review commissioned 2026-09-01 — PASS/HOLD: pending** | SHIPPED (after R3 review) |
...
| B7-TASK-MEDIATION | Nerva 2.0 program control, ~line 225–241 | OWNER-DECISION | — | PR #918 merged under an integrator HOLD that still stands; #818 remains `owner_hold`; owner must record a retain/revert decision + reconcile the ledgers (#757/#778/#818). **Owner decision 2026-09-01:** #918 RETAINED under a bounded default-off exception; ledgers reconciled; B7 still not program-accepted pending #906 | **DECIDED 2026-09-01** |
| GOVERNANCE-WAVE-HOLD | Nerva 2.0 program control, line 162 | OWNER-DECISION | — | #911 (SEC-B8) and #916 landed with green CI but each carries a recorded post-merge integration HOLD; owner retain/revert decision needed. **Owner decision 2026-09-01:** #911 RETAINED — post-merge SEC-B8 security audit commissioned, PASS/HOLD: pending; #916 RETAINED — attestation = the existing exact-head receipt (PR #916 comment 5308830474, head a2438d8), no new review | **DECIDED 2026-09-01** (not governance-complete until the #911 audit PASS is recorded) |
| E1/E6/E9-AUTHORITY-CEILING | Nerva 2.0 program control, line 151 | R3-REVIEW-GATED | M | #859/#860/#861/#864 merged before their #856 predecessor was validated; #913 now satisfies the predecessor but retained bytes still need fresh post-B2 acceptance decisions. **Owner decision 2026-09-01:** post-B2 acceptance reviews authorized, in order E6 #860 (`f62d8b5`) → E9-authority #861 (`9b7ac88`) → E9-totals #864 (`568de94`), GO/HOLD bound to those real merge commits; results pending; not dependency acceptance | SHIPPED (after R3 review) |
...
| BUG-2b.2 | Bugs & Hot Fixes, line 2206 | OWNER-DECISION | — | **Recounted 2026-08-28**: this was never an ambiguous test-coverage gap — it's a missing-feature gap. `H10.2`/`H10.7` (its ride-along deps) shipped backend-only; no drag-drop SVG canvas component exists anywhere in `frontend/src` (confirmed: `WorkflowsPanel` is a plain list/run/delete panel). Nothing to write pointer-event tests against until the owner decides build-vs-drop. Packet added to `docs/OWNER_TASKS.md` Parking lot; `BACKLOG.md` BUG-2b row corrected. **Correction 2026-09-01:** a v1 canvas does exist (legacy HUD `agents/web/static/workflows.js` `WorkflowCanvas`); it retires with the v1 HUD (AUD-15) | **RESCOPED** — dropped by owner decision 2026-09-01; the JSON-paste WORKFLOW BUILDER is the v2 surface of record; a visual canvas only as a fresh spec on demonstrated demand |
...
| id | section | bucket | size | dependencies | terminal-state target |
|----|---------|--------|------|---------------|------------------------|
| T-0.20 | Competitive-Gap, line 1197 | AI-EXEC | M | **CLOSED 2026-08-28** — router (`agents/core/routers/vault.py`, user-guarded) + `VaultPanel` (Console → Memory) + export wiring (`data_export._dump_vault`, decrypts ciphertext into the portable export). Forget needed no code change — the vault root was already outside `data_purge.KEEP_DIRS`, confirmed and pinned by a new regression test rather than assumed | **CLOSED** |
...
| T-0.29 | Competitive-Gap, line 1206 | AI-EXEC (PWA) + **OWNER-CONFIG** (signing) | M | **PWA half CLOSED 2026-08-28** — manifest + runtime-caching service worker now ship with the **v2** HUD (the actual default), root-scoped, with `/api/` responses provably never cached (`tests/test_pwa_v2.py`, +8). **Signed installers are genuinely owner-gated**, not deferred: they need owner-provisioned code-signing certificates (Apple Developer ID, Windows OV/EV) + CI secrets. Packet filed in `docs/OWNER_TASKS.md` | **CLOSED** (PWA) + OWNER-PACKET (signing) |
| T-0.37 | Competitive-Gap, line 1214 | AI-EXEC | M | **PARTIAL 2026-08-28 — provenance half CLOSED, ontology half open (deliberately).** `_record_derived_provenance()` now records the knowledge + embed phases with `parent_id` lineage back to the source message, closing the gap `provenance.py`'s own docstring had already designed for. `tests/test_ingestion_derived_provenance.py` (+8). **Ontology + cross-agent sharing not attempted:** it's a design question (what the shared entity schema *is*, and what cross-agent read authority means under the plugin/data-space gate), not a wiring gap — improvising it inside a provenance slice would have been the wrong call | SHIPPED (provenance) · ontology needs its own scoping pass |
| T-0.41 | Competitive-Gap, line 1218 | AI-EXEC | S | **CLOSED 2026-08-28** (after one honest correction mid-run: first marked CLOSED prematurely, corrected to PARTIAL when the HUD-parity caller gate showed the digest consumed nothing, then genuinely closed). Routing layer + 3 user-guarded routes + `SignalRoutingPanel`, **and** the morning brief now renders a per-domain `🌍 Semnale externe` section via `scheduler_service._signal_briefs_or_none()` (one sidecar read routed into all domains; honest silence when there's no sidecar or nothing to say). `tests/test_signals_router.py` (+11), panel test (+3), `tests/test_digest_signal_briefs.py` (+10) | **CLOSED** |
...
| T-0.50 | Competitive-Gap, line 1235 | AI-EXEC (surface) + **OWNER-CONFIG** (executor) | M | **Surface CLOSED 2026-08-28** — `POST /api/creative/publish/{checklist,package}` + `PublishReadinessPanel` expose the previously-unwired `creative/publishing.py`. Never publishes: `release_payload` withheld until all checks pass, no `/publish`/`/upload` endpoint exists (asserted by test), panel states "still not published". **Platform executor genuinely owner-gated** — per-platform OAuth + MOONSHOT §5 approval-held publication. `tests/test_creative_publish_routes.py` (+12), panel test (+6) | **CLOSED** (surface) + OWNER-PACKET (executor) |
...
| T-0.53 | Competitive-Gap, line 1238 | AI-EXEC (route/HUD) + OWNER-CONFIG (Figma sync) | S | **CLOSED (AI-EXEC half) 2026-08-28** — `GET /api/design-manifest` + `DesignManifestPanel` (Console → Observe) shipped; route/OpenAPI/route-auth snapshots reseeded. Figma token sync needs an owner-provisioned API token and is genuinely owner-gated, not deferred laziness | **CLOSED** + OWNER-PACKET (Figma sync, low priority, no packet filed — trivial to request when wanted) |
| T-0.55 | Competitive-Gap, line 1240 | OWNER-DECISION | S | SLA definition — a doc/owner artifact, not code | OWNER-PACKET |
| T-0.58 | Competitive-Gap, line 1243 | AI-EXEC | M | **CLOSED 2026-08-28** — the content/domain pack type already existed (`knowledge_packs.py`) with zero callers; new `agents/core/routers/packs.py` + `PacksPanel` unify it with skill packs under one typed inventory + per-pack verification. `model` packs are declared **unsupported with a reason** (Nerva distributes no model weights) rather than stubbed as an empty type. `tests/test_packs_router.py` (+11), panel test (+5) | **CLOSED** |
...
| H5.16 | ORIZONT 5, line 2250 | AI-EXEC | S | **CLOSED (synthesize-while-streaming) 2026-08-28** — new `frontend/src/sentences.ts` (contract-matching TS port of `sentence_stream.py`) + a streaming-speak session in `voice.ts` fed by `app.tsx`'s live token deltas; fail-safe fallback to the whole-reply path if any sentence fails. `sentences.test.ts` (+41, incl. an arbitrary-chunk round-trip property that caught 2 real bugs) + `voice-stream-speak.test.tsx` (+3). Browser wake-word remains open under the same row but is separate scope | **CLOSED** |
| H18.24 | ORIZONT 18 mobile, line 2574 | AI-EXEC (contract) + **DEVICE-GATED** (renderer) | M | **PARTIAL 2026-08-28 — contract ported and proven, renderer honestly deferred.** The row asks to "port `orb.tsx`'s **state→visual contract**", and that contract (`orbVisual`) is pure — now at `mobile/src/voice/orbVisual.ts`, proven identical to the browser's against **80 shared vectors** (`tests/_fixtures/orb_visual_vectors.json`, asserted by both suites — the repo's existing WorldView cross-language-vector pattern). **Not ported:** the particle renderer (RN has no canvas → needs react-native-svg/Skia + on-device validation) and `listening`/`transcribing` states (no mobile mic-capture pipeline exists at all, only TTS playback). Shipping an unverifiable animated visual would have been the exact "looks done, isn't proven" failure this ledger exists to catch | SHIPPED (contract) · renderer device-gated |
| H18.25 | ORIZONT 18 mobile, line 2575 | AI-EXEC (contract) + **DEVICE-GATED** (chrome + PTT) | M | **PARTIAL 2026-08-28 — same treatment as H18.24.** `wallState` (the word + tone the wall announces) ported to `mobile/src/voice/wallState.ts`, proven identical against **500 shared vectors**. Ordering is the load-bearing part and is pinned: a voice error outranks everything, and "offline" never masks running work. **Not ported:** the neural-field canvas (needs a graphics dep) and **hold-to-talk, which cannot exist at all** until mobile has a mic-capture pipeline — it currently has only TTS playback | SHIPPED (contract) · chrome + PTT device-gated |
...
| WV-170 | ORIZONT 19 WorldView, line 2593 | AI-EXEC | S | **CLOSED (code+CI) 2026-08-28, live run unverified this session** — `tests/test_neo4j_live_property_search.py` (gated `JARVIS_NEO4J_LIVE=1`) exercises the real property-scanning Cypher against an actual Neo4j server; new `neo4j-live` job in `.github/workflows/reality.yml` provides one via a `neo4j:5` service container, mirroring `worldview.yml`'s proven TimescaleDB/Redis pattern. Docker Desktop would not start in this sandbox after repeated attempts, so the test's first real execution will be on GitHub Actions, not here — this is disclosed, not claimed as verified | **CLOSED** (pending first live CI run) |
...
| TASK-3 | Bugs & Hot Fixes, line 2209 | OWNER-DECISION | M | Residual defense-in-depth: wrap email/web-webhook input in `TaintedValue` at the channel boundary + gate irreversible tool calls through `QuarantinePolicy.check_step` — explicitly an "owner architecture call" (distinct from the already-shipped H23.6 channel-ingress taint). **Owner decision 2026-09-01:** stays OPEN, rescoped to exactly this bounded slice; full through-LLM taint propagation is not commissioned; the "sufficient?" judgment stays with `RISKS.md` SEC-05 | SHIPPED (bounded slice) — decided 2026-09-01, still open |
...
| E731-CONTINUITY-IDENTITY | B3/Continuity Core mapping, line 250 | **OWNER-DECISION** (reclassified 2026-08-28, was AI-EXEC) | M | **Recounted 2026-08-28, confirmed STILL OPEN, and genuinely owner-shaped, not AI-buildable** — `docs/nerva2/CONTINUITY_CORE_RECONCILIATION.md` itself states creating a destination issue for Jarvis's own Identity Manifest "is an owner-scoping decision..., not a documentation call this pass should make unilaterally." An unrelated persona-traits commit (#911xx-adjacent, all 17 SOULs) does not touch this. Packet added to `docs/OWNER_TASKS.md`. **Decided 2026-09-01:** new issue #1008 owns Jarvis's own Identity Manifest; #762 stays Howard-only | **CLOSED** (decided 2026-09-01 → #1008) |
| ACTION-KERNEL-FLIP-CRITERIA | Docs-vs-code accuracy pass, line 571 | OWNER-DECISION | S | Define the flip-on criteria for `JARVIS_ACTION_KERNEL` + `JARVIS_UNIFIED_ACTION_API` — when does the kernel become the default rail? **Decided 2026-09-01:** criteria (4 weeks opt-in dogfood + zero kernel-caused false DENYs/ungoverned actions + one 72h PASS soak with both flags on; A8 not a precondition) recorded in `docs/decisions/2026-09-01-action-kernel-default-rail.md` | **DECIDED 2026-09-01** (criteria recorded; the flip PR stays criteria-gated) |
...
| LVP-PAYMENT-RAIL | Live-vs-Plumbing, line 964 | OWNER-DECISION | XL | Real payment adapter (AP2/ACP/x402) at `payments.settle()` — explicitly "moves money", owner decision required. **Ratified 2026-09-01:** no real payment rail for 1.0; reopen condition recorded in `BACKLOG.md` DRA-20 | **DECIDED 2026-09-01** (no rail) |
...
| AUD-18 | Hardening audit, line 1993 | AI-EXEC | M | **PARTIAL 2026-08-28 — F30 CORS validation CLOSED.** `normalize_cors_origins()` + `web.py` wiring: malformed origins (missing scheme, trailing slash, path) and `*`-with-credentials are rejected **with reasons logged**, and a set-but-unusable config warns loudly instead of leaving CORS believed-on-but-inert. `tests/test_cors_policy.py` (+11), verified by booting the app with a mixed list. **Still open:** Qdrant-by-default at scale, lazy plugin instantiation, Vite code-split, F31 loader retry/feedback — each a real perf/UX change needing its own slice (Qdrant-by-default in particular is a runtime default flip that wants a running Qdrant to validate, not a code-only change) | SHIPPED (F30) · rest still open |
| AUD-14 | Hardening audit, line 1989 | AI-EXEC | S | **CLOSED-AS-RATCHETED 2026-08-28.** The recount found it running *backwards* (104 → 145) because "migrate opportunistically, don't sweep" had nothing preventing regrowth. Fixed the mechanism, not just the symptom: migrated `plugin_manager.py`'s 30 reads (145 → 115) **and** added `test_raw_env_reads_do_not_grow`, a count ratchet that lets the number fall but never rise, plus a companion test stopping the cap drifting above reality. Red-proved with three planted reads. The remaining tail stays cosmetic-by-design but can now only shrink | **CLOSED** (ratcheted; residual tail bounded) |
...
- **2026-09-01 (owner decision packet):** the bulk-approved owner decisions were recorded against the
  ...
  LVP-PAYMENT-RAIL, WV-SCALE-PROOF). Commissioned reviews are recorded as *pending*, never as passed.
  Left open by the owner, untouched here: T-0.55 (SLA wording), MOBILE-PHONE-SURFACE.
