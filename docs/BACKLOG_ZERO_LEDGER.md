...
| T-0.41 | Competitive-Gap, line 1218 | AI-EXEC | S | **CLOSED 2026-08-28** — new `agents/core/routers/signals.py` (3 user-guarded routes: routed feed, per-agent slice, per-domain brief) gives `signal_routing.py` its first real caller; `SignalRoutingPanel` (Console → Interop) is the read surface. `tests/test_signals_router.py` (+11) + panel test (+3) | **CLOSED** |
