...
| T-0.41 | Competitive-Gap, line 1218 | AI-EXEC | S | **PARTIAL 2026-08-28 (corrected — first marked CLOSED, which overstated it).** New `agents/core/routers/signals.py` (3 user-guarded routes) + `SignalRoutingPanel` (Console → Interop) give `signal_routing.py` its first real callers, and the panel calls the per-agent slice endpoint directly. **But the row asks for per-agent _digests_, and `autonomy/digest.py` still consumes none of it** — the HUD-parity caller gate caught this by flagging `/api/signals/brief/{domain}` as uncalled. Wiring the digest builder is the honest remainder | SHIPPED (routing+HUD) · digest consumer still open |
