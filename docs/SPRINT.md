...
**Data:** 2026-07-06 (AUD-14 webhook-channels env-json active)
...
**Branch:** `codex-aud14-webhook-channels-env-json`
...
**Active AUD-14 webhook-channels scope:** `codex-aud14-webhook-channels-env-json` routes `JARVIS_WEBHOOK_CHANNELS` through shared `env_json_object()` parsing so malformed webhook adapter config falls back to default-off instead of using a local JSON parser. Red/green guard plus focused env-config/lifespan checks are green locally.
...
| AUD-14 / Webhook Channels Env-Json | `codex-aud14-webhook-channels-env-json` | — | 🟡 active | Codex | `JARVIS_WEBHOOK_CHANNELS` uses shared `env_json_object`; focused local checks green |
...
Active PR lock: `codex-aud14-webhook-channels-env-json` touches `agents/web.py`, `tests/test_o26_p2_env_config.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
