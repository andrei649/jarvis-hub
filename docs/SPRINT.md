...
**Data:** 2026-07-06 (AUD-14 LLM model config active/local-green)
...
**Branch:** `codex-aud14-llm-model-config`
...
**Active AUD-14 scope:** `codex-aud14-llm-model-config` centralizes LLM model-name defaults and the `JARVIS_DEEP_MODEL` override in `agents/core/llm/model_config.py`. `HybridRouter` consumes that shared module for deep-slot routing while preserving the public constants and the LOCAL_ONLY floor. Red/green guard plus hybrid-router/config adjacent suites are green locally.
...
| AUD-14 / LLM Model Config | `codex-aud14-llm-model-config` | pending | 🟢 local-green | Codex | Model-name defaults + `JARVIS_DEEP_MODEL` moved behind shared `llm/model_config.py`; focused/adjacent checks green |
...
Pending PR / `codex-aud14-llm-model-config`: `agents/core/llm/model_config.py`, `agents/core/llm/hybrid_router.py`, `tests/test_llm_model_config.py`, `BACKLOG.md`, `STATUS.md`, `docs/SPRINT.md`.
