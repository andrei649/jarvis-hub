...
**Data:** 2026-07-06 (AUD-14 task-budget env-float in progress)
...
**Branch:** `codex-aud14-task-budget-env-float`
...
**Active AUD-14 task-budget scope:** #598 (`codex-aud14-task-budget-env-float`) routes the K3 per-task wall-time env knob `JARVIS_TASK_MAX_SECONDS` through shared `env_float(..., minimum=0.0)` instead of local parsing in `AutonomyCoordinator.build_executor`. Red/green guard plus focused executor/env-config checks are green locally.
...
| AUD-14 / Task Budget Env-Float | `codex-aud14-task-budget-env-float` | #598 | 🟡 active | Codex | `JARVIS_TASK_MAX_SECONDS` uses shared `env_float` parser; focused local checks green |
...
Active PR lock: `codex-aud14-task-budget-env-float` touches `agents/core/autonomy_coordinator.py`, `agents/core/env_config.py`, `tests/test_executor_budget.py`, `tests/test_o26_p2_env_config.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
