...
**Data:** 2026-07-06 (AUD-14 log rotation env-int in progress)
...
**Branch:** `codex-aud14-log-rotation-env-int`
...
**Active AUD-14 log-rotation scope:** #604 (`codex-aud14-log-rotation-env-int`) routes `JARVIS_LOG_MAX_MB` and `JARVIS_LOG_BACKUPS` through shared `env_int()` while preserving the settings-DB fallback chain before hardcoded defaults. Red/green guard plus focused operability/env-config checks are green locally.
...
| AUD-14 / Log Rotation Env-Int | `codex-aud14-log-rotation-env-int` | #604 | 🟡 active | Codex | `JARVIS_LOG_MAX_MB` / `JARVIS_LOG_BACKUPS` use shared `env_int`; focused local checks green |
...
Active PR lock: `codex-aud14-log-rotation-env-int` touches `agents/core/log.py`, `tests/test_h2311_operability.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
