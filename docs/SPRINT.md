...
**Data:** 2026-07-06 (AUD-14 channel-rates env-map active)
...
**Branch:** `codex-aud14-channel-rates-env-map`
...
**Active AUD-14 channel-rates scope:** `codex-aud14-channel-rates-env-map` routes `JARVIS_CHANNEL_SEND_RATES` through a shared `env_int_map()` helper so comma-separated integer maps use the same never-raise config layer. Red/green guard plus focused env-config/send-rate checks are green locally.
...
| AUD-14 / Channel Rates Env-Map | `codex-aud14-channel-rates-env-map` | — | 🟡 active | Codex | `JARVIS_CHANNEL_SEND_RATES` uses shared `env_int_map`; focused local checks green |
...
Active PR lock: `codex-aud14-channel-rates-env-map` touches `agents/core/env_config.py`, `agents/core/channels/send_rate_limit.py`, `tests/test_o26_p2_env_config.py`, `tests/test_channel_send_rate_limit.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
