...
**Data:** 2026-07-06 (M3.5 WorldView MCP write transport active/local-green)
...
**Branch:** `codex-m35-worldview-mcp-write-transport`
...
**Active M3.5 scope:** `codex-m35-worldview-mcp-write-transport` wires JARVIS to WorldView's MCP write tools without changing the read-only HTTP bridge: `WorldViewMCPWriteClient` gates `watch_aoi`/`reconstruct_event` through plugin-gate, Action Kernel, per-agent `plugin:worldview` broker capability, and scoped HMAC token before stdio MCP tool calls. Local red/green, focused MCP/capability/write/Argus tests, adjacent WorldView/MCP sweeps, ruff, and py_compile are green; PR CI/merge pending.
...
| M3.5 / WorldView MCP Write Transport | `codex-m35-worldview-mcp-write-transport` | pending | 🟢 local-green | Codex | Argus reaches `watch_aoi`/`reconstruct_event` only through plugin-gate + Action Kernel + scoped HMAC MCP token |
...
PR pending / `codex-m35-worldview-mcp-write-transport`: `agents/core/mcp/client.py`, `agents/core/mcp/worldview_write.py`, `agents/core/argus.py`, `agents/core/plugins/worldview.py`, `agents/argus/SOUL.md`, `tests/test_mcp_client.py`, `tests/test_worldview_mcp_write_transport.py`, `tests/test_argus_interface.py`, `docs/superpowers/{plans,specs}/2026-07-06-m35-worldview-mcp-write-transport*`, `BACKLOG.md`, `STATUS.md`, `docs/SPRINT.md`.
