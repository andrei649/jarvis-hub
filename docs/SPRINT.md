...
**Data:** 2026-07-06 (AUD-14 skill-history env-flag active)
...
**Branch:** `codex-aud14-skill-history-env-flag`
...
**Active AUD-14 skill-history scope:** `codex-aud14-skill-history-env-flag` routes `JARVIS_SKILL_HISTORY` through shared `env_flag()` parsing so `0` disables the opt-in marketplace history ledger and `on` enables it. Red/green guard plus focused marketplace history checks are green locally.
...
| AUD-14 / Skill History Env-Flag | `codex-aud14-skill-history-env-flag` | — | 🟡 active | Codex | `JARVIS_SKILL_HISTORY` uses shared `env_flag`; focused local checks green |
...
Active PR lock: `codex-aud14-skill-history-env-flag` touches `agents/core/orchestrator.py`, `tests/test_marketplace_history.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
