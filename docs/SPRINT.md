...
**Data:** 2026-07-06 (AUD-14 CORS-origins env-list active)
...
**Branch:** `codex-aud14-cors-origins-env-list`
...
**Active AUD-14 CORS-origins scope:** `codex-aud14-cors-origins-env-list` adds shared `env_list()` parsing and routes `JARVIS_CORS_ORIGINS` through it so comma-separated env lists have one trim/blank-skip convention. Red/green guard plus focused env-config/lifespan checks are green locally.
...
| AUD-14 / CORS Origins Env-List | `codex-aud14-cors-origins-env-list` | — | 🟡 active | Codex | `JARVIS_CORS_ORIGINS` uses shared `env_list`; focused local checks green |
...
Active PR lock: `codex-aud14-cors-origins-env-list` touches `agents/core/env_config.py`, `agents/web.py`, `tests/test_o26_p2_env_config.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
