...
**Data:** 2026-07-06 (AUD-14 plugin-grants env-list active)
...
**Branch:** `codex-aud14-plugin-grants-env-list`
...
**Active AUD-14 plugin-grants scope:** `codex-aud14-plugin-grants-env-list` routes `JARVIS_PLUGIN_GRANTS` through shared `env_list()` parsing so plugin hardening grants use the same comma-list convention as CORS. Red/green guard plus focused plugin-gate checks are green locally.
...
| AUD-14 / Plugin Grants Env-List | `codex-aud14-plugin-grants-env-list` | — | 🟡 active | Codex | `JARVIS_PLUGIN_GRANTS` uses shared `env_list`; focused local checks green |
...
Active PR lock: `codex-aud14-plugin-grants-env-list` touches `agents/core/plugin_gate.py`, `tests/test_cdx11_least_privilege_plugins.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
