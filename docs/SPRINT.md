...
**Data:** 2026-07-06 (AUD-14 call-config env-json active)
...
**Branch:** `codex-aud14-call-config-env-json`
...
**Active AUD-14 call-config scope:** `codex-aud14-call-config-env-json` routes `JARVIS_CALL_CONFIG` through a shared `env_json_object()` helper so structured call-provider config follows the same never-raise env-config rule as the numeric AUD-14 slices. Red/green guard plus focused env-config/executor/call-broker checks are green locally.
...
| AUD-14 / Call Config Env-Json | `codex-aud14-call-config-env-json` | — | 🟡 active | Codex | `JARVIS_CALL_CONFIG` uses shared `env_json_object`; focused local checks green |
...
Active PR lock: `codex-aud14-call-config-env-json` touches `agents/core/env_config.py`, `agents/core/autonomy_coordinator.py`, `tests/test_o26_p2_env_config.py`, `tests/test_executor_budget.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
