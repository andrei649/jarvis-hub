...
**Data:** 2026-07-05 (TASK-3 channel-ingress taint active/local-green)
...
**Branch:** `codex-task3-channel-ingress-taint`
...
**Active TASK-3 scope:** #590 (`codex-task3-channel-ingress-taint`) marks untrusted inbound channel messages at `Gateway.route()` with private `_inbound_meta`, persists only public taint fields in `ChannelInboxStore`, and strips private gateway metadata in `Orchestrator.channel_handler()` before adapter sends. Local red/green and adjacent channel/taint sweeps are green; PR CI/merge pending.
...
| TASK-3 / Channel Ingress Taint | `codex-task3-channel-ingress-taint` | #590 | 🟡 draft PR | Codex | Gateway marks inbound channel metadata, inbox persists public taint fields, outbound sends strip private metadata; local targeted checks green |
...
PR #590 / `codex-task3-channel-ingress-taint`: `agents/core/channels/gateway.py`, `agents/core/channel_inbox.py`, `agents/core/orchestrator.py`, `tests/test_task3_channel_ingress_taint.py`, `docs/superpowers/{plans,specs}/2026-07-05-task3-channel-ingress-taint*`, `BACKLOG.md`, `STATUS.md`, `docs/SPRINT.md`.
