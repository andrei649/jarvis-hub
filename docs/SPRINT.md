...
**Data:** 2026-07-06 (AUD-14 email-port env-int active)
...
**Branch:** `codex-aud14-email-port-env-int`
...
**Active AUD-14 email-port scope:** `codex-aud14-email-port-env-int` routes `SMTP_PORT` and `IMAP_PORT` through shared `env_int()` parsing so malformed email-channel port env values fall back to the existing defaults instead of crashing startup. Red/green guard plus focused env-config checks are green locally.
...
| AUD-14 / Email Port Env-Int | `codex-aud14-email-port-env-int` | — | 🟡 active | Codex | `SMTP_PORT` / `IMAP_PORT` use shared `env_int`; focused local checks green |
...
Active PR lock: `codex-aud14-email-port-env-int` touches `agents/web.py`, `tests/test_o26_p2_env_config.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
