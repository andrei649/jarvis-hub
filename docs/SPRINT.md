...
**Data:** 2026-07-06 (AUD-14 vector-dimension env-int active)
...
**Branch:** `codex-aud14-vector-dimension-env-int`
...
**Active AUD-14 vector-dimension scope:** `codex-aud14-vector-dimension-env-int` routes `VECTOR_DIMENSION` through shared `env_int(..., minimum=1)` parsing so malformed Qdrant vector dimensions fall back to the existing 768 default instead of crashing memory setup. Red/green guard plus focused memory embedding checks are green locally.
...
| AUD-14 / Vector Dimension Env-Int | `codex-aud14-vector-dimension-env-int` | — | 🟡 active | Codex | `VECTOR_DIMENSION` uses shared `env_int`; focused local checks green |
...
Active PR lock: `codex-aud14-vector-dimension-env-int` touches `agents/core/memory/manager.py`, `tests/test_memory_embeddings.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
