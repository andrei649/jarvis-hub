...
**Data:** 2026-07-06 (AUD-14 STT beam-size env-int in progress)
...
**Branch:** `codex-aud14-stt-beam-env-int`
...
**Active AUD-14 STT scope:** #602 (`codex-aud14-stt-beam-env-int`) routes `JARVIS_STT_BEAM_SIZE` through shared `env_int(..., minimum=1)` so malformed or non-positive beam values fall back to the greedy live-loop default. Red/green guard plus focused STT/env-config checks are green locally.
...
| AUD-14 / STT Beam-Size Env-Int | `codex-aud14-stt-beam-env-int` | #602 | 🟡 active | Codex | `JARVIS_STT_BEAM_SIZE` uses shared `env_int` parser; focused local checks green |
...
Active PR lock: `codex-aud14-stt-beam-env-int` touches `agents/core/voice/stt.py`, `tests/test_stt_config.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
