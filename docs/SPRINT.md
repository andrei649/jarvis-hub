...
**Active R3-B3 scope:** `codex-r3-b3-a2a-escalation-contracts` adds `A2A_INBOUND_CONTRACT` before signed peer tasks enter the pending inbox and `ESCALATION_CONTRACT` before autonomy broadcasts call channel adapters. Local red/green and adjacent contract sweeps are green; PR CI/merge still pending.
...
| R3-B3 / A2A + Escalation Contracts | `codex-r3-b3-a2a-escalation-contracts` | TBD | ⏳ in progress | Codex | A2A inbound contract + escalation broadcast contract; local focused/adjacent sweep green |
