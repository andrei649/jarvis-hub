...
**Branch:** `codex-aud14-trust-env-flags`
...
**Merged AUD-14 analytics scope:** #600 routes `JARVIS_ANALYTICS_MAX_EVENTS` through shared `env_int(..., minimum=0)` so a malformed public-analytics retention cap cannot crash `analytics_store` at import time. Red/green guard, focused analytics/env-config checks, and full PR CI are green.

**Merged AUD-14 STT scope:** #602 routes `JARVIS_STT_BEAM_SIZE` through shared `env_int(..., minimum=1)` so malformed or non-positive beam values fall back to the greedy live-loop default. Red/green guard, focused STT/env-config checks, and full PR CI are green.

**Merged AUD-14 log-rotation scope:** #604 routes `JARVIS_LOG_MAX_MB` and `JARVIS_LOG_BACKUPS` through shared `env_int()` while preserving the settings-DB fallback chain before hardcoded defaults. Red/green guard, focused operability/env-config checks, and full PR CI are green.

**Merged AUD-14 call-config scope:** #606 routes `JARVIS_CALL_CONFIG` through a shared `env_json_object()` helper so structured call-provider config follows the same never-raise env-config rule as the numeric AUD-14 slices. Red/green guard, focused env-config/executor/call-broker checks, and full PR CI are green.

**Merged AUD-14 channel-rates scope:** #608 routes `JARVIS_CHANNEL_SEND_RATES` through a shared `env_int_map()` helper so comma-separated integer maps use the same never-raise config layer. Red/green guard, focused env-config/send-rate checks, and full PR CI are green.

**Merged AUD-14 email-port scope:** #610 routes `SMTP_PORT` and `IMAP_PORT` through shared `env_int()` parsing so malformed email-channel port env values fall back to the existing defaults instead of crashing startup. Red/green guard, focused env-config/lifespan checks, and full PR CI are green.

**Merged AUD-14 vector-dimension scope:** #612 routes `VECTOR_DIMENSION` through shared `env_int(..., minimum=1)` parsing so malformed Qdrant vector dimensions fall back to the existing 768 default instead of crashing memory setup. Red/green guard, focused memory embedding checks, and full PR CI are green.

**Merged AUD-14 skill-history scope:** #614 routes `JARVIS_SKILL_HISTORY` through shared `env_flag()` parsing so `0` disables the opt-in marketplace history ledger and `on` enables it. Red/green guard, focused marketplace history checks, and full PR CI are green.

**Merged AUD-14 webhook-channels scope:** #616 routes `JARVIS_WEBHOOK_CHANNELS` through shared `env_json_object()` parsing so malformed webhook adapter config falls back to default-off instead of using a local JSON parser. Red/green guard, focused env-config/lifespan checks, and full PR CI are green.

**Merged AUD-14 CORS-origins scope:** #618 adds shared `env_list()` parsing and routes `JARVIS_CORS_ORIGINS` through it so comma-separated env lists have one trim/blank-skip convention. Red/green guard, focused env-config/lifespan checks, and full PR CI are green.

**Merged AUD-14 plugin-grants scope:** #620 routes `JARVIS_PLUGIN_GRANTS` through shared `env_list()` parsing so plugin hardening grants use the same comma-list convention as CORS. Red/green guard, focused plugin-gate checks, and full PR CI are green.

**Active AUD-14 trust-env scope:** `codex-aud14-trust-env-flags` routes `JARVIS_MIC_MUTED` and `JARVIS_STRICT_LOCAL` through shared `env_flag()` parsing while preserving the `_env_truthy` compatibility import. Red/green guard plus focused trust/env-config checks are green locally.
...
| AUD-14 / Analytics Max-Events Env-Int | `codex-aud14-analytics-max-events-env-int` | #600 | ✅ merged | Codex | `JARVIS_ANALYTICS_MAX_EVENTS` uses shared `env_int` parser; full PR CI green |
| AUD-14 / STT Beam-Size Env-Int | `codex-aud14-stt-beam-env-int` | #602 | ✅ merged | Codex | `JARVIS_STT_BEAM_SIZE` uses shared `env_int` parser; full PR CI green |
| AUD-14 / Log Rotation Env-Int | `codex-aud14-log-rotation-env-int` | #604 | ✅ merged | Codex | `JARVIS_LOG_MAX_MB` / `JARVIS_LOG_BACKUPS` use shared `env_int`; full PR CI green |
| AUD-14 / Call Config Env-Json | `codex-aud14-call-config-env-json` | #606 | ✅ merged | Codex | `JARVIS_CALL_CONFIG` uses shared `env_json_object`; full PR CI green |
...
| AUD-14 / Email Port Env-Int | `codex-aud14-email-port-env-int` | #610 | ✅ merged | Codex | `SMTP_PORT` / `IMAP_PORT` use shared `env_int`; full PR CI green |
| AUD-14 / Vector Dimension Env-Int | `codex-aud14-vector-dimension-env-int` | #612 | ✅ merged | Codex | `VECTOR_DIMENSION` uses shared `env_int`; full PR CI green |
| AUD-14 / Skill History Env-Flag | `codex-aud14-skill-history-env-flag` | #614 | ✅ merged | Codex | `JARVIS_SKILL_HISTORY` uses shared `env_flag`; full PR CI green |
| AUD-14 / Webhook Channels Env-Json | `codex-aud14-webhook-channels-env-json` | #616 | ✅ merged | Codex | `JARVIS_WEBHOOK_CHANNELS` uses shared `env_json_object`; full PR CI green |
| AUD-14 / CORS Origins Env-List | `codex-aud14-cors-origins-env-list` | #618 | ✅ merged | Codex | `JARVIS_CORS_ORIGINS` uses shared `env_list`; full PR CI green |
| AUD-14 / Plugin Grants Env-List | `codex-aud14-plugin-grants-env-list` | #620 | ✅ merged | Codex | `JARVIS_PLUGIN_GRANTS` uses shared `env_list`; full PR CI green |
| AUD-14 / Trust Env-Flags | `codex-aud14-trust-env-flags` | — | 🟡 active | Codex | Trust-status mic/strict-local reads use shared `env_flag`; focused local checks green |
...
Active PR lock: `codex-aud14-trust-env-flags` touches `agents/core/routers/oauth.py`, `tests/test_o26_p2_env_config.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
