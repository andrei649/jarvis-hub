...
**Data:** 2026-07-06 (AUD-14 analytics max-events env-int in progress)
...
**Branch:** `codex-aud14-analytics-max-events-env-int`
...
**Active AUD-14 analytics scope:** #600 (`codex-aud14-analytics-max-events-env-int`) routes `JARVIS_ANALYTICS_MAX_EVENTS` through shared `env_int(..., minimum=0)` so a malformed public-analytics retention cap cannot crash `analytics_store` at import time. Red/green guard plus focused analytics/env-config checks are green locally.
...
| AUD-14 / Analytics Max-Events Env-Int | `codex-aud14-analytics-max-events-env-int` | #600 | 🟡 active | Codex | `JARVIS_ANALYTICS_MAX_EVENTS` uses shared `env_int` parser; focused local checks green |
...
Active PR lock: `codex-aud14-analytics-max-events-env-int` touches `agents/core/analytics_store.py`, `tests/test_analytics_local.py`, `BACKLOG.md`, `STATUS.md`, and `docs/SPRINT.md`.
