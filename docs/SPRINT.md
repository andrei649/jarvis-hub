...
**Data:** 2026-07-05 (AUD-14 channel send-rate env-int active/local-green)
...
**Branch:** `codex-aud14-channel-rate-env-int`
...
**Active AUD-14 scope:** #592 (`codex-aud14-channel-rate-env-int`) routes the global outbound channel send-rate cap through `env_int()` and pins malformed/negative `JARVIS_CHANNEL_SEND_RATE` as unlimited. Local red/green, focused send-rate, and adjacent env-config checks are green; PR CI/merge pending.
...
| AUD-14 / Channel Send-Rate Env-Int | `codex-aud14-channel-rate-env-int` | #592 | 🟡 draft PR | Codex | `JARVIS_CHANNEL_SEND_RATE` global cap uses `env_int`; local targeted checks green |
...
PR #592 / `codex-aud14-channel-rate-env-int`: `agents/core/channels/send_rate_limit.py`, `tests/test_channel_send_rate_limit.py`, `docs/superpowers/{plans,specs}/2026-07-05-aud14-channel-send-rate-env-int*`, `BACKLOG.md`, `STATUS.md`, `docs/SPRINT.md`.
