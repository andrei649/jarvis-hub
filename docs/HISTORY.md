...
| 2026-09-02 | **D1 — CI posture: partial re-gate, bounded to ≤ +3 min on the PR path (measured ≈ +0 wall-clock, the new jobs run in parallel with `test`); the 2026-08-29 de-gate otherwise intact** *(CTO — owner ratifies by merge)*: (a) the `hud-v2-build` bundle-staleness check runs on pull requests too; (b) the two cheap lanes from the #986 archive come back on PRs — security-scans (~1–1.5 min) and lockfile-drift (~15 s); (c) push-to-`main` runs are no longer cancelled by newer pushes (`cancel-in-progress` false for push, true for PRs); (d) the Windows matrix stays post-merge; (e) after the PR pytest run the executed backend test count is compared with `project-status.json` `tests.backend` and drift fails the job. Blocking only once the owner marks the checks required (A4). [Decision doc](decisions/2026-09-02-cto-ci-posture-and-1.0-freeze.md). | 4 of the last 15 `main` pushes were red after merge — 2 stale HUD bundles (deterministic, caught by (a)) + 2 flaky Windows reds (probabilistic, fixed in D2 instead of re-gated) — and 4 more were cancelled and never verified. 14/14 human PRs merged this week with zero reviews, so pre-merge checks are the only automated brake; the tracked test count had drifted 11 tests (7,244 tracked vs 7,255 live) unseen because the in-suite gate reuses counts. |
| 2026-09-02 | **D2 — Ship the strictly-monotonic `_now()` fix in `agents/core/notes_store.py`** for the `docs.updated_at` writers (`create_doc`, `add_block`, `update`, `move`, `delete_block`) with a regression test that forces identical wall-clock reads; block ordering keys untouched *(CTO — owner ratifies by merge)*. | `test_list_docs_returns_docs_most_recently_updated_first` failed twice on `windows-latest` (#999 shipped both the store method and the test): `datetime.now()` has ~15 ms resolution on the CI Python, so two writes share an `updated_at` and the tie breaks on a random UUID. Block `ordering` keys are unique fractional keys, not timestamps — not in the tie class. |
| 2026-09-02 | **D3 — Fix the scheduled Reality Harness crash** (`AttributeError: module agents.core.skills has no attribute discover`, `agents/core/observability/reality_evidence.py:162`, introduced by #980) and add a test exercising the evidence recorder's skills-discovery path *(CTO — owner ratifies by merge)*. | Red four nights running on the nightly `reality.yml` lane with no test covering the path, so it could regress silently. |
| 2026-09-02 | **D4 — Ledger truth** *(CTO — owner ratifies by merge)*: (a) `scripts/status_sync.py` gains a third horizon state — 🔨 rows count as *delivered, runtime proof pending* (neither done nor open) and the roll-up/snippets show done / delivered-pending / open; (b) `open_release_gates()` and the horizon classifier read the status cell's *leading* marker, never a ✅ quoted in prose; (c) BACKLOG corrections — H19 headers 33/33 → the real 35-row table (2 ✅ + 33 🔨), DRA ledger 54 ticked / 8 open of 62, the stale DRA-08 "Phase 5 deliberately not built" paragraph struck (Phase 5 landed in #1000), SEC-B5 rescoped to the HTTP-recall bind/reset hardening (its recall→action leg shipped in #983), A3 / OWNER_TASKS Dependabot figures replaced by the 2026-09-02 offline `npm audit` / `pip-audit` measurements (frontend 0; root 5 incl. 2 high; worldview 3 high; worldview/mcp 5 incl. 2 high; mobile 21–22 incl. 10–11 high; Python clean) with GitHub's own count still an owner read. | The status artifact reported 46 open/blocked items of which 33 were H19 rows marked 🔨 with "Livrat:" prose — real work invisible to a ✅-only parser; A4 (open: owner must fix required-check names) read as closed because of a ✅ inside its explanatory prose; the DRA tally said 53 + 8 of 62; the 19/4 Dependabot figure was a 2026-07-07 copy, not a measurement. |
| 2026-09-02 | **D5 — Release truth: 1.0 definition of done / freeze** *(CTO — owner ratifies by merge)*: `main` is **feature-frozen for 1.0** from the merge of the 2026-09-02 CTO PR; between now and the tag only (1) red-`main` fixes, (2) the dependency audit wave, (3) the A5 relicense PR land; the tag (`git tag v1.0.0 && git push origin v1.0.0` on `main`) is cut immediately after A5 merges; before tagging the owner folds CHANGELOG `[Unreleased]` into `[1.0.0]` and sets its date; the A1 §0 run proves the tagged build and its findings are 1.0.1; everything else (8 open DRA rows, SEC-B4's `browser_run` capability residual, H23.30 public demo, GAP-0, ORIZONT 27–33) is 1.x. Every surface still carrying the July "expanded gate" wording (BACKLOG version line + roadmap row, GO_LIVE_PLAN header / checklist / roadmap summary, MOONSHOT §4, OWNER_TASKS A8) is marked superseded by the 2026-08-28 directive; `release.yml` has never run on GitHub — a `workflow_dispatch` `dry_run` was triggered today. [Decision doc](decisions/2026-09-02-cto-ci-posture-and-1.0-freeze.md) · GO_LIVE_PLAN → "1.0 definition of done / freeze". | `__version__` has read 1.0.0 since 2026-08-28 and the tag has waited 4–5 days on one owner PR plus one tag push, while four documents still described a gate the owner removed. A written freeze stops the 1.0 scope from re-expanding by accretion (an audit added 62 ledger rows one day after release prep merged) and puts the proof where the owner ordered it: after the tag. |
| 2026-09-02 | **D6 — Review governance: keep the de-gated posture, make the docs consistent** *(CTO — owner ratifies by merge)*: `docs/AGENT_WORKFLOW.md` R2/R3 rows and `AGENTS.md` say independent review is *recommended*; R3 runtime/security changes get a recorded post-merge attestation in `BACKLOG.md` (the SEC-B4 / SEC-B6 / #911 model from #1009); both note that `pr-auto-merge.yml` merges any non-draft CLEAN PR hourly with no review and that the re-gated PR checks block only once the owner lists them as required (A4). | The R2/R3 table still read "independent review" / "separated roles" as requirements while the header called the tiers advisory and 14/14 human PRs merged this week unreviewed; a rule nobody enforces is worse than a recorded convention. |
| 2026-09-02 | **D7 — Dependency audit wave (engineering, now)** *(CTO — owner ratifies by merge)*: `npm audit fix` in `worldview/`, `worldview/mcp/` and the repo-root HUD-test tree, each followed by that tree's own tests/build; `frontend/` (already 0) and `mobile/` (the owner's device-gated Expo tail) are not touched. | Offline `npm audit` on 2026-09-02: worldview 3 high, worldview/mcp 5 (2 high), root 5 (2 high) — all `fixAvailable`; no PR-path scan existed to catch them until D1(b). |
| 2026-09-06 | **The 1.x → 2.0 roadmap and the one-PR backlog delivery** *(CTO — owner ratifies by merge)*: `docs/NERVA_2_ROADMAP.md` fixes the release sequence 1.1.0 (hands on three OSes, company mode as the E5.0 candidate, the activation path, the program contracts that had no code, the DRA/SEC/debt residuals) → 1.2.0 proven hands → 1.3.0 Night Shift E5.1 → 1.4.0 fast adoption → 1.5.0 continuity → 1.6.0 reach → 2.0.0-rc hardening → 2.0.0 program acceptance, each with an exit gate; AUD-13/15, credential.fill, the consent-ledger completion, T-0.21, T-0.49, H18.24/25 renderers and the reach items are deferred by name to their milestone. `docs/prompts/BACKLOG_DRIVER.md` is the standing prompt for an unattended developer (pick order: red main → current milestone → 1.0.1 defects → DRA/SEC residuals → blocking debt). The 1.1.0 delivery ships as one PR built from file-disjoint slices, each with an independent adversarial review. | The owner asked (2026-09-05) for the roadmap of the rest of the backlog, everything AI-executable delivered in one PR, and Nerva made a full computer operator with a 24/7 company loop and iPhone-grade adoption. The 2026-09-06 recount found 110 AI-executable open rows against 101 owner-gated; the research pass (`docs/research/2026-09-06-hermes-and-field-gap-matrix.md`) found Nerva ahead of Hermes on memory, approval, audit, local models and house and behind on every actuation, scheduling and install axis — so the plan spends unique engineering on hands and the evidence loop, and copies nothing from the do-not-copy list. |
