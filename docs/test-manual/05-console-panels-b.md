...
- **Expected:** 1) only matching rows remain, across every category, with `N matches`; 2) `no setting matches "zzz"`. 3) nothing is sent on the first click (focus moves to the confirmation, which names how many settings); the second resets only that category (`reset N`, or `already the defaults`), keeps its secrets (`kept N secrets` on PLUGINS) and names a setting the product posture still forces; unsaved edits of that category are dropped, other categories keep their values. 4) one row `settings.system reset to defaults: [...]`. 5) `curl -X POST -H 'Content-Type: text/plain' …/api/admin/settings/system/reset` answers **415** and resets nothing.
