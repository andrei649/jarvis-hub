...
| PNB-170 | LOGS never shows a secret, never reads another file (H145) | Write a line holding a token into a rotation by hand (`echo "2026-01-01 00:00:00  INFO  x  token=ghp_…" >> <data>/logs/jarvis.log.1`), view it; `curl -s -H "X-Admin-Token: $JARVIS_ADMIN_TOKEN" "localhost:8080/api/admin/logs?file=../../.env"`; append a 50 MB log and time the panel | The token is masked (redaction runs again at read time, rotations included); the path is refused with **400** `file must be one of the listed log files` (only `jarvis.log` and its numbered rotations are listed); the read stays fast (at most 4 MiB from the end, and the panel says only the last part was read) | **BLOCKER** if a secret or another file's content is shown | ✅tests/test_log_tail.py |
