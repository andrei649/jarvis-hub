...
| PNB-169 | LOGS: the hub log from the Console (H145) | With file logging off (the default), open Console → Observe → **Logs**; then `nerva config set system.log_to_file on`, restart, make an error happen (e.g. a bad provider key), ↻; switch Level to WARNING, pick a Component, Lines 500, a rotation under File; tick **auto-refresh 5 s** | Off: an amber note says file logging is off and how to turn it on (`system.log_to_file` or `JARVIS_LOG_FILE`), never an empty log. On: records newest first, ERROR/CRITICAL red, WARNING amber, a traceback shown with its error; each filter changes the list (`GET /api/admin/logs?level=…&component=…&lines=…&file=…`); at most 500 records; auto-refresh shows **LIVE** and re-reads every 5 s until unticked; without the admin token the panel says it needs one | MAJOR | ✅tests/test_log_tail.py, frontend/src/test/logs-panel.test.tsx |
| PNB-170 | LOGS never shows a secret, never reads another file (H145) | Write a line holding a token into a rotation by hand (`echo "2026-01-01 00:00:00  INFO  x  token=ghp_…" >> <data>/logs/jarvis.log.1`), view it; `curl -s -H "X-Admin-Token: $JARVIS_ADMIN_TOKEN" "localhost:8080/api/admin/logs?file=../../.env"`; make `jarvis.log.2` a link to another file (`ln -s ~/.env <data>/logs/jarvis.log.2`); log a line holding the value of a secret-named variable (e.g. `SMTP_PASS`); append a 50 MB log with no errors, then pick Level **ERROR** | The token is masked (redaction runs on each whole line before anything is cut, rotations included), and so is the secret-named variable's value; the path is refused with **400** `file must be one of the listed log files`; the linked `jarvis.log.2` is not listed at all (only regular files named `jarvis.log` and `jarvis.log.<n>`); the ERROR read stays fast and the panel says only the last 4096 KiB of the file were read | **BLOCKER** if a secret or another file's content is shown | ✅tests/test_log_tail.py |
...
| PNB-167 | Sandbox files live under the data root, not `/tmp` (H667) | Run one `execute_code` (or SANDBOX **execute**), then `ls <data root>/cache/exec` and `ls /tmp`; take a backup (`nerva backup`) | A `nerva-sandbox-*` directory (mode `0700`, in a `0700` root) holds the run under `cache/exec`, its lock beside it in `cache/exec/.locks/` (`host pid flock=1 boot=… start=…`); nothing new in `/tmp`; the backup has no `cache/exec` entry, nor any `nerva-sandbox-*` directory under a root chosen inside the data root. The same holds for a run made by `execute_code` after `rm -rf` of its run directory and lock (the directory comes back `0700`, its lock taken again). With `JARVIS_EXEC_TEMP_DIR=/srv/nerva-exec` (or Admin → `security.sandbox_temp_dir`, which **needs a restart** and refuses a relative or `~user` path) runs go there instead; such a root is the owner's and is never pruned. To keep a pruned cache on a bigger disk, move the data root (`JARVIS_HOME`) or bind-mount `cache/exec` (a `cache/exec` replaced by a linked directory is never pruned, and the sweep shows as failed). Naming the managed cache itself, through a link or a `..`, is still the managed cache. On Docker Desktop for macOS the data root must be under a shared path (`/Users`, `/tmp`, `/private`, `/var/folders`) for the bind mount | MAJOR — a tmpfs `/tmp` fills overnight and execution fails silently | ✅tests/test_exec_cache.py, tests/test_h667c_exec_cache_review.py |
| PNB-168 | The managed cache is pruned; a chosen root never is (H667) | In `cache/exec`, `touch -d '-4 days'` a stopped run directory and everything in it; set `security.sandbox_temp_max_age_hours` to `72` (1–8760); wait for the hourly `exec-cache-prune` job (or restart and wait an hour). Repeat with one fresh file deep inside it, with the live hub's own run directory aged, with a root set by `JARVIS_EXEC_TEMP_DIR`, and with a file dated 100 years ahead | Only the old, stopped directory is removed (and its lock). One fresh file keeps its whole directory; the live hub's directory stays (its flock is held; where flock does not work, its record's pid is alive with the same start time — asked of the process table on Windows); a crashed run's directory goes even when its pid is in use again (the hub is pid 1 in a container), since its free flock is the answer on the same kernel; another host's record is kept until ten times the limit; nothing under a chosen root is deleted, and the managed cache is still pruned while one is chosen; entries not named `nerva-sandbox-*` are never touched; a future date does not keep a directory forever. A managed root that is a link, not yours, or writable by others is not swept, and the scheduler shows the sweep as failed | **BLOCKER** if a live run's files or anything under an owner-chosen directory is deleted | ✅tests/test_exec_cache.py, tests/test_h667c_exec_cache_review.py |
...
- **Expected:** 1) only matching rows remain, across every category, with `N matches`; 2) `no setting matches "zzz"`. 3) the first click resets nothing: it asks the hub for a dry run and shows what would change (H259, PNB-173), and focus moves to the confirmation, which names how many settings; the second resets only that category (`reset N`, or `already the defaults`), keeps its secrets (`kept N secrets` on PLUGINS) and names a setting the product posture still forces; unsaved edits of that category are dropped, other categories keep their values. 4) one row `settings.system reset to defaults: [...]`. 5) `curl -X POST -H 'Content-Type: text/plain' …/api/admin/settings/system/reset` answers **415** and resets nothing.
- **FAIL if:** reset runs on the first click, touches another category, or leaves no audit row → **MAJOR**.

#### PNB-173 — SETTINGS DB: what differs from the default, a previewed reset, and its undo (H259)
- **Surface:** Console → Admin → **SETTINGS DB** · **Tier:** admin (`POST /api/admin/settings/{category}/reset`, `POST /api/admin/settings/reseed`, `GET /api/admin/settings/resets`, `POST /api/admin/settings/undo`) · **Auto:** ✅tests/test_settings_defaults_undo.py, ✅frontend/src/test/settings-reset-undo.test.tsx
- **Steps:** 1) Set `system.log_level` to `DEBUG` and save; hover **changed** on its row, then press **↺** and **💾 save 1 change**. 2) Set it to `DEBUG` again, save, set a plugin secret (e.g. `plugins.tuya_secret`), click **reset** on SYSTEM and read the preview, then confirm. 3) Change `system.log_to_file`, then press **undo…** → **undo the reset**. 4) Click **reset every category…**, read the preview, confirm, then **undo…** again. 5) Read `GET /api/admin/audit`, then `curl -X POST -H "X-Admin-Token: $T" …/api/admin/settings/reseed` with no body.
- **Expected:** 1) **changed** shows only on a setting that differs from its declared default, its tooltip names the default (`default: INFO`), a secret never shows one; ↺ stages the default and the save writes it. 2) the preview lists `system.log_level: DEBUG → INFO` before anything is written; the reset answers `reset 1 · can be undone` and a line `last reset: system · 1 setting · <time>` appears. 3) the undo answers `restored 1` and names any setting changed since the reset as left as it was (`… (changed since the reset)`); the undo line disappears. 4) the preview counts every setting that would change across categories and says how many secrets are kept; after it, the plugin secret is still set, and undo puts every other value back. 5) one audit row per reset, reseed and undo (`settings reset to defaults in every category: N setting(s)`, `settings reset undone (…)`), naming settings, never secret values; the bare POST answers **415** and resets nothing.
- **FAIL if:** a reset or reseed removes a secret, writes before its second click, or cannot be undone → **MAJOR**; an undo overwrites a setting changed after the reset without naming it → **MAJOR**.
...
- **Expected:** 1) the file has `"format": "nerva-settings/1"` and no secret, no client id, no `mcp.servers`, no token-shaped value and no URL with a password (`http://u:pw@host` in `llm.lm_studio_url`); the panel names each left-out setting with its reason. 2) the preview lists each change `category.key: old → new` (a secret shows `(secret)`), nothing is written until **apply**, then `imported N settings`. 3) every reason is listed and nothing is written (all or nothing); `NaN` or `mcp.servers: [{}]` are refused too. 4) one `settings imported: N setting(s): [...]` row. 5) the same document posted as `text/plain` (what a cross-site page could send) answers **415**, and one marked `Sec-Fetch-Site: cross-site` answers **403**; nothing is written.
