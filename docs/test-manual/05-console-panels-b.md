...
| PNB-167 | Sandbox files live under the data root, not `/tmp` (H667) | Run one `execute_code` (or SANDBOX **execute**), then `ls <data root>/cache/exec` and `ls /tmp` | A `sandbox-*` directory (mode `0700`) holds the run under `cache/exec`; nothing new in `/tmp`. With `JARVIS_EXEC_TEMP_DIR=/srv/nerva-exec` (or Admin → `security.sandbox_temp_dir`) and a restart, runs go there instead | MAJOR — a tmpfs `/tmp` fills overnight and execution fails silently | ✅tests/test_exec_cache.py |
| PNB-168 | The managed cache is pruned; a chosen root never is (H667) | In `cache/exec`, `touch -d '-4 days'` a stopped run directory and everything in it; set `security.sandbox_temp_max_age_hours` to `72`; wait for the hourly `exec-cache-prune` job (or restart and wait an hour). Repeat with one fresh file deep inside it, with the live hub's own run directory aged, and with a root set by `JARVIS_EXEC_TEMP_DIR` | Only the old, stopped directory is removed. One fresh file keeps its whole directory; the live hub's directory stays (its lock is held); nothing under a chosen root is deleted, and entries not named `sandbox-*` are never touched | **BLOCKER** if a live run's files or anything under an owner-chosen directory is deleted | ✅tests/test_exec_cache.py |
