...
| GOV-256 | Authoring is a proposal, never a write (H318) | as the owner, have the agent `skill_propose` a new SKILL.md for an existing skill (one you imported and approved, with code), then `{description, steps}` for a new one; in the HUD's Decision Inbox, read the change under SKILL CHANGES and approve it (✓); ask it to change a bundled skill (Brief); repeat the proposal from a household member's HUD turn and after the agent read a web page | the first leaves one pending proposal and one approval card; the inbox shows its whole diff, built from the ledger (`GET /api/skills/proposals`), and approving it replaces the skill's SKILL.md at once (learning loop off or on) and says the skill is `shown`: it stays approved, signed if it was, and its code still loads; the bundled skill is refused `skill_propose_bundled`; the second a quarantined `PENDING_REVIEW` skill; the household turn is refused `skill_propose_owner_only` and the web-read turn `skill_propose_untrusted_turn` (an inbound guest is not offered the tool at all) | BLOCKER if a live SKILL.md changes without approval; MAJOR if an approved change hides or sandboxes a skill that was shown | ✅tests/test_h318_skill_tools.py, tests/test_h318c_skill_review.py, frontend/src/test/skill-changes.test.tsx |
