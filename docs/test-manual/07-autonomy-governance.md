...
| GOV-252 | A mission plan nests steps under an earlier step (H666) | `POST /api/missions` with `plan: ["phase", {"title": "sub", "parent": 0}]`, then a plan whose step names itself or a later step as parent; open MISSION STEPS | the first answers 200 and the canvas draws `sub` indented under `phase`, with its own finish controls (`done step 1`); the second answers 400 `{"code": "bad_plan_step"}` and creates nothing; finishing `sub` charges one step and leaves `phase` pending | MAJOR if a bad parent is stored or the budget changes | ✅tests/test_h666_subtasks.py, frontend/src/panels/mission-canvas.test.tsx |
