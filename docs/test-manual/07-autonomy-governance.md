...
| GOV-258 | Two processes share the kernels' data root; plan reads after scripts (H315 fourth review) | with `llm.execute_code_sessions` on, run the web hub and the coordinator (`jarvis-hub` and `jarvis-runtime`) on one `JARVIS_HOME`; open a session kernel from the HUD, then restart the coordinator; then in one turn have the agent run four scripts that each tick a plan item, reading the plan after each | the HUD's kernel keeps making tool calls after the coordinator restarts (its mailbox under `data/code_sessions/p<pid>-…` is untouched); every plan read after a script answers the plan, none is refused as a repeat, and the turn does not end early | MAJOR if a restart breaks another process's kernel, or a read after a script is refused | ✅tests/test_h315e_todo_review.py |
