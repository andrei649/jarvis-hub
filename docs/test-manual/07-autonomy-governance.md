...
| GOV-258 | Two processes share the kernels' data root; plan reads after scripts (H315 fourth review) | with `llm.execute_code_sessions` on, run the web hub and the coordinator (`jarvis-hub` and `jarvis-runtime`) on one `JARVIS_HOME`; open a session kernel from the HUD, then restart the coordinator; then in one turn have the agent run four scripts that each tick a plan item, reading the plan after each, and once a script and a plan read in the same step; then run the hub in its Docker image on a bind-mounted data root beside a host process on the same root, and restart the container twice | the HUD's kernel keeps making tool calls after the coordinator restarts (its mailbox under `data/code_sessions/p<pid>-…` is untouched: its process holds the directory's `.lock`); every plan read after a script answers the plan, the read after a script's own step included, none is refused as a repeat, and the turn does not end early; each container restart removes the previous container's directories (their locks are free, though the new hub is PID 1 too) and never the host process's | MAJOR if a restart breaks another process's kernel, leaves a dead process's directories, or a read after a script is refused | ✅tests/test_h315e_todo_review.py, tests/test_h315f_todo_review.py |
