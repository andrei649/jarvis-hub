...
| GOV-240 | A guest-steered plan says so | have a paired non-owner Telegram sender ask a multi-step question | that plan carries an amber `guest turn` tag; its text renders as inert text | MAJOR | ⚠️frontend/src/test/plans-in-flight.test.tsx |
| GOV-241 | The plan text never rides the tool trail | after GOV-239: `nerva tools -n 100 --json` and search it for a phrase from an item's content | no hit; each `todo_updated` row carries the session, ids, statuses, `current` and `total` only | **BLOCKER** if plan text is in the trail | ✅tests/test_todo_tool.py |
