...
| GOV-256 | Authoring is a proposal, never a write (H318) | as the owner, have the agent `skill_propose` a new SKILL.md for an existing skill, then `{description, steps}` for a new one; approve the first card in the Decision Inbox; repeat the proposal from a household member's HUD turn and after the agent read a web page | the first leaves one pending proposal and one approval card showing the diff, and approving it replaces the skill's SKILL.md at once (learning loop off or on); the second a quarantined `PENDING_REVIEW` skill; the household turn is refused `skill_propose_owner_only` and the web-read turn `skill_propose_untrusted_turn` (an inbound guest is not offered the tool at all) | BLOCKER if a live SKILL.md changes without approval | ✅tests/test_h318_skill_tools.py |
