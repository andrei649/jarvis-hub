...
| GOV-258 | Two processes share the kernels' data root; plan reads after scripts (H315 fourth review) | with `llm.execute_code_sessions` on, run the web hub and the coordinator (`jarvis-hub` and `jarvis-runtime`) on one `JARVIS_HOME`; open a session kernel from the HUD, then restart the coordinator; then in one turn have the agent run four scripts that each tick a plan item, reading the plan after each, and once a script and a plan read in the same step; then run the hub in its Docker image on a bind-mounted data root beside a host process on the same root, and restart the container twice; start the hub and the coordinator at the same moment (`systemctl restart jarvis-hub jarvis-runtime`) ten times; then have two chats each run a script at once (`import time; time.sleep(1); print("from chat A")` and `print("from chat B")`); with a kernel open, make its owner directory unreadable for a moment (`chmod 000 data/code_sessions/p<pid>-…`), open a second kernel, restore the mode (`chmod 700`) and restart the coordinator; finally run a second hub on the same root as another OS user | the HUD's kernel keeps making tool calls after the coordinator restarts (its mailbox under `data/code_sessions/p<pid>-…` is untouched: its process holds the directory's `.lock`); every plan read after a script answers the plan, the read after a script's own step included, none is refused as a repeat, and the turn does not end early; each container restart removes the previous container's directories (their locks are free, though the new hub is PID 1 too) and never the host process's (on Linux: Docker Desktop's bind mounts may not carry `flock` between host and VM); two starts at once never cost either its lock, and a kernel used every minute keeps its mailbox however long it has run; each chat gets its own script's output, never the other's; a lock that could not be read for a moment is kept, and both kernels keep their mailboxes after the restart; another user's directory, whose lock this hub cannot ask, reads as alive and is never removed | MAJOR if a restart breaks another process's kernel, leaves a dead process's directories, a read after a script is refused, or a chat receives another's output | ✅tests/test_h315e_todo_review.py, tests/test_h315f_todo_review.py, tests/test_h315g_todo_review.py, tests/test_h315h_todo_review.py |
