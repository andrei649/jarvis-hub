...
| GOV-257 | `/refine [focus]` reviews this conversation now (H465) | with a local model up, chat a few turns that state a preference ("keep replies short"), then send `/refine reply style` from the owner's channel; send it again from a guest's chat; send it while a long turn is still answering in the same conversation | the owner gets "Reviewed this conversation (focus: reply style):" with one line per memory or skill change (skill changes say they wait in the Decision Inbox) or "nothing worth keeping"; the conversation's history is unchanged; the guest is refused as for any owner command; the mid-turn send answers that a turn is still running; with no local model it says reviews never leave this machine | MAJOR if the review runs on a cloud model or writes into the conversation | ✅tests/test_h465_refine.py |
