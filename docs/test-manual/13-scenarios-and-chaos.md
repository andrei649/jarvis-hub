...
| JRN-064 | The known TASK-5 payload leak | From the phone: `GET /tasks` (user) and list the keys of each task | **Current known behaviour:** each task dict still contains `payload` and `result` (`agents/core/routers/dashboard.py:136-194`). Record as **TASK-5 still open**, MAJOR — not a new finding | **BLOCKER** only if a payload contains a *resolved secret value* rather than a `{{secret:NAME}}` handle | ❌ |
| JRN-065 | Mission Control is payload-free 👁 | Open `/mission-control` on the phone with the user token only | The approvals card shows counts plus the 7-field whitelist `id,title,agent,kind,risk_tier,status,created_at` (`agents/core/routers/swarm.py:140`) — never a body or result | **BLOCKER** for any payload/result on this page | ✅tests/test_swarm_summary.py |
...
| 13.5 JRN-4 family / LAN day | 14 | 🌐 🔑 | 11 of 14 | Includes the known TASK-5 leak; needs a second person |
