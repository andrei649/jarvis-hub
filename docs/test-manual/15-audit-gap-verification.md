...
- **Expected:** the instance structurally satisfies `ExternalOrchestratorBindings`; all 15 slots exist with explicit `None`; observed direct, qualified, imported-aliased, tuple/module-aliased, and literal-`getattr` binding API calls match the inventoried path/line/column callsites exactly in both directions; dynamic binding-API `getattr` fails closed; hostile typed, aliased, nested, `setattr`, and `object.__setattr__` receivers cannot bypass the lexical undeclared-write guard; unrelated or shadowed same-named assignments cannot satisfy the inventory; and consumers test availability rather than `hasattr` presence.
...
- **Evidence:** `tests/test_orchestrator_bindings.py` (27 hermetic cases); the explicit boot defaults in `Orchestrator.__init__`; all external writers routed through `bind_external_orchestrator_attribute`; production callsite map equals `EXTERNAL_BINDING_WRITERS` exactly by path, line and column.
