...
| SEC-055 | Offline recovery CLI (no lockout) | after revoking everything, `python scripts/token_recover.py rotate admin` on the box (it loads the hub's `.env` files first, so it writes the store the hub reads) | prints a fresh raw token; it works from `$B` | **BLOCKER** — otherwise the owner can be locked out of their own machine | ✅`tests/test_token_lifecycle.py` |
...
|  SEC-214 | The hub warns about a dangerous host (H501) | Start the hub as root (`sudo -E python serve.py`), then as your user; on a box whose sshd takes passwords (no `PasswordAuthentication no`, or `yes` in a drop-in read first); in Docker with `JARVIS_HOME` pointed at a path that is not a volume; read the log, `python scripts/doctor.py` and `GET /api/security/posture` (admin) | Each posture logs one `host posture: …` WARNING at start naming the check, what it saw and the fix; the doctor shows `host_not_root`, `sshd_no_passwords` and `container_storage` as `warn` (advisory — the verdict is not changed) and `skip` when it cannot tell; the posture route's `host` lists the same three with a `warnings` count; nothing is refused or changed | MAJOR if a dangerous posture is reported ok, or a check blocks the start · auto: ✅`tests/test_h501_host_posture.py`  |
|  SEC-215 | Broken TLS trust stops the start by name; verification is never quietly off (H504) | Start the hub with `SSL_CERT_FILE=/nonexistent.pem`, then with `REQUESTS_CA_BUNDLE` pointing at a text file, then `SSL_CERT_DIR` at a file; put the bad value in `~/Documents/Jarvis/.env` instead of the shell. Behind a TLS-inspecting proxy, set `JARVIS_CA_BUNDLE` to its root and ask a cloud model something. Point LM Studio at a self-signed HTTPS address and set `JARVIS_TLS_INSECURE_TARGETS=lm-studio`, then add `JARVIS_HARDENED=1` | Each bad variable refuses the start with `TLS trust is misconfigured…`, the variable, its value, what is wrong and a `fix:` command (and the `.env` file when it came from one) — never a bare `FileNotFoundError`; with the proxy root the model answers; the listed LM Studio client logs `TLS verification DISABLED for https://…` and connects; hardened, it logs an ERROR and refuses the certificate | MAJOR if any bad CA value leads to verification being off, or a model call ignores `JARVIS_CA_BUNDLE` · auto: ✅`tests/test_h504_tls_trust.py`  |
