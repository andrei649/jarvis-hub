...
#### SEC-065 — The posture endpoint reports a guardrail mode the engine is not running  🤖👁
- **Surface:** `GET /api/security/posture` (admin) vs the live `GuardrailsEngine` · **Auto:** ❌
...
- **Expected (per code):** posture and the panel now say **BLOCK**, but the turn **succeeds** with a normal 200 reply — because `self.security` is constructed once at boot (`agents/core/orchestrator.py:498-506`) and `load_runtime_settings` (`:760`) never rebuilds it. 6) restart, resend → *now* 403.
- **Also acceptable (honest degradation):** the posture payload carrying an explicit "pending restart" marker, or a 403 immediately (the setting genuinely applied live).
- **FAIL if:** the panel/endpoint claims `BLOCK` while the engine warns, with nothing telling the operator a restart is required → **MAJOR** (a false security-state display; same class as run 1's false kill-switch ENGAGED).
...
- **Surface:** audit DB, log file, transcripts · **Tier:** n/a · **Auto:** ⚠️`tests/test_audit_hardening.py` (covers `findings[].matched_text` only)
- **Why it matters:** `docs/THREAT_MODEL.md` T4 claims secrets never reach logs. Every turn writes an audit row whose `content_preview` is the **assistant reply's first 100 chars, unredacted** (`agents/core/orchestrator.py:1798-1806`; `agents/core/security/audit.py:112` redacts only `findings`).
...
- **FAIL if:** the plant appears in `audit.db` / `GET /api/admin/audit` because the model echoed it into `content_preview` → **MAJOR** (a durable plaintext secret in the tamper-evident log, outside retention control). If it appears in the log file → **BLOCKER**.
...
| SEC-137 | Supply-chain gates run and are blocking | `python scripts/check_thirdparty_drift.py --consistency`; `python scripts/gen_sbom.py requirements-beta.txt /tmp/sbom.json /tmp/NOTICE 0.0.0`; and read `.github/workflows/security.yml` | drift check exits 0 (manifest `.github/third-party-manifest.json` matches the vendored versions); the SBOM is valid CycloneDX JSON and deterministic (run twice, `diff` is empty); the four jobs — gitleaks, semgrep, pip-audit, bandit — are all **blocking** (no `continue-on-error`) | MAJOR | ✅`tests/test_release_gate.py` |
| SEC-138 | Secret-scan gate really blocks | on a scratch branch add a file containing a **fake but realistic** key (`AKIAQAFAKE0000000000`), run the same gitleaks command the workflow uses (`gitleaks dir . --config .gitleaks.toml --redact`) | non-zero exit, finding redacted in the output. Delete the file afterwards | MAJOR | ❌ |
