...
| CHN-168 | The receiver switch refuses every delivery (H153) | Console → Webhooks → **switch the receiver off** (or `PUT /api/admin/settings/webhooks` `{"values":{"receiver_enabled":false}}`, admin), then trigger any hook | `503 {"error":"the webhook receiver is off"}`, with or without a token; the panel says "The receiver is off"; list, create, switch and delete still work. Switch it back on: deliveries run again, no restart. | MAJOR | ✅tests/test_h153_webhook_subscriptions.py |
...
| CHN-170 | A prompt template decides what the agent reads (H153) | Create with `"prompt":"New {event} on {repository.full_name}"`; trigger with a GitHub-shaped body and `X-GitHub-Event: push` | The agent receives `New push on <owner>/<repo>`. `{a.b.0}` reads the payload by key and index, `{event}` is the event, `{payload}` the whole body; `{__class__}` and `{a[0]}` read nothing. The turn is still an inbound, untrusted one. | MAJOR | ✅tests/test_h153_webhook_subscriptions.py |
