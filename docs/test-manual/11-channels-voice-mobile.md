...
| CHN-171 | **Deliver to** sends the reply to your own channel (H153 review) | Create with `"deliver":"telegram"` (or pick *deliver to* in the Webhooks panel: log, telegram, voice, ntfy); trigger it by day with `X-GitHub-Event: push` | The turn runs, and the reply arrives in your Telegram as plain text: `Webhook <name> - push`, a blank line, then the text (markup stripped, a link shows its address, no link preview, no voice note; cut at 4,000 characters with `… (cut: the whole reply is in the session)`). ntfy gets the label as an ASCII title of at most 120 characters; voice speaks it. The response carries `"delivery":{"channel":"telegram","ok":true}`; the detail pane says `last delivery … · delivered to telegram`. With no owner chat configured: `ok:false` and the reason, never a silent drop and never a 500. | MAJOR | ✅tests/test_h153c_webhook_sends.py |
...
| CHN-177 | A hook pushes at most 30 times an hour (H153 third review) | Post 31 deliveries to a deliver-only `ntfy` hook within an hour | 30 arrive; the 31st is `ok:false`, `over this hook's push limit (30 an hour): not pushed; the text was not kept`. Another hook is unaffected. | MINOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-178 | A workflow delivers its result or nothing (H153 third review) | A workflow hook whose last step fails (a timeout, a guardrail) | Nothing is pushed: `the workflow run failed at <step>: nothing delivered`, never an earlier step's output. A missing workflow, an invalid one and a run that raises are each recorded on the hook. | MAJOR | ✅tests/test_h153c_webhook_sends.py |
