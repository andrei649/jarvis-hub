...
| CHN-170 | A prompt template decides what the agent reads (H153) | Create with `"prompt":"New {event} on {repository.full_name}"`; trigger with a GitHub-shaped body and `X-GitHub-Event: push` | The agent receives `New push on <owner>/<repo>`. `{a.b.0}` reads the payload by key and index, `{event}` is the event, `{payload}` the whole body; `{payload.x}` reads the payload's own `x` (a key named `event` included); `{__class__}` renders empty and `{a[0]}` stays as literal text. A template that renders nothing is skipped (`202`), not run. The turn is still an inbound, untrusted one. | MAJOR | ✅tests/test_h153_webhook_subscriptions.py |
| CHN-171 | **Deliver to** sends the reply to your own channel (H153 review) | Create with `"deliver":"telegram"` (or pick *deliver to* in the Webhooks panel); trigger it by day | The turn runs, and the reply arrives in your Telegram as `Webhook <name> · <event>`, then the text (cut to 4,000 characters with a note). The response carries `"delivery":{"channel":"telegram","ok":true}`; the detail pane says `last delivery … · delivered to telegram`. With no owner chat configured: `ok:false` and the reason, never a silent drop. | MAJOR | ✅tests/test_h153b_webhook_delivery.py |
| CHN-172 | Deliver only skips the agent (H153 review) | Create with `"deliver":"web","deliver_only":true,"prompt":"New {event} on {repository.full_name}"`; trigger a GitHub push | No turn runs; `New push on <owner>/<repo>` reaches the HUD, labelled with the hook. With `deliver: log` nothing is sent and nothing runs. | MAJOR | ✅tests/test_h153b_webhook_delivery.py |
| CHN-173 | Quiet hours keep a push off the phone (H153 review) | Set `ambient.quiet_hours_start/end` around now; trigger a hook that delivers to `telegram`, then one to `web` | The Telegram one is not pushed: `delivery.ok:false`, reason `quiet hours: not pushed; the reply is in the session`, shown in the detail pane. The `web` one arrives. | MINOR | ✅tests/test_h153b_webhook_delivery.py |
| CHN-174 | Destinations Nerva does not offer are refused by name (H153 review) | Create with `"deliver":"email"`, `"github_comment"`, `"discord"`, `"slack"`; and with `"skills":["x"]` | `422` naming why: email is never an outbound side effect of an inbound delivery; a GitHub comment goes through governed write-back; Discord and Slack are reply-only here. An unknown field is refused, never silently dropped. | MINOR | ✅tests/test_h153b_webhook_delivery.py |
