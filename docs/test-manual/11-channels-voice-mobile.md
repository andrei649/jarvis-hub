...
| CHN-060 | Widget messages bypass the gateway limiter — **FIXED 2026-08-02** | Issue a widget token (`POST /api/admin/widgets`, admin), then `for i in 1..20: curl -X POST localhost:8000/api/widget/$TOK/message -d '{"message":"hi"}'` (open) | The widget now routes through `Gateway.route`, so the per-channel window (`channels.rate_limit`, default 10/min) applies **in front of** the global per-IP 120/min — expect the burst to be throttled by the channel limiter first. No `sender=` is passed, so the fail-closed pairing gate stays out of the anonymous path. | MAJOR | ✅tests/test_q10_widget_ingress_taint.py |
| CHN-061 | Widget origin classification — **FIXED 2026-08-02** | Same as CHN-060, then inspect the resulting audit/kernel rows for `origin` | `widget` is no longer in `INTERNAL_TURN_CHANNELS`: a public embed's turns classify **`inbound`**, are taint-marked (`inbound:widget`) with `injection_flags`, and a GRANTed action from such a turn escalates to QUEUE (owner approval) instead of auto-executing. | MAJOR | ✅tests/test_q10_widget_ingress_taint.py |
...
| CHN-073 | Workflow target degrades honestly | Create a hook with `"target_type":"workflow"`, `"target":"nope"`, trigger it | Either `501 {"error":"workflow execution not available"}` (`routers/webhooks.py:382`) or a bounded engine error — never a fabricated "workflow ran". | MINOR | ✅tests/test_h10_8_webhooks.py |
...
| CHN-075 | Not-initialized path | Trigger a hook during the first second of startup | `503 {"error":"not initialized"}` (`routers/webhooks.py:307`) — honest, not a hang. | MINOR | ❌ |
| CHN-167 | A hook switched off keeps its token (H153) | `curl -s -X PATCH localhost:8000/api/webhooks/$HID -H "X-Admin-Token: $A" -H 'Content-Type: application/json' -d '{"enabled":false}'`, then trigger with the token | `200 {"ok":true,"webhook":{…"enabled":false…}}`, then `403 {"error":"webhook disabled"}`; without the token still `401`. `{"enabled":true}` and the same token works again. The Webhooks panel shows `off`. | MAJOR | ✅tests/test_h10_8_webhooks.py |
| CHN-168 | The receiver switch refuses every delivery (H153) | Console → Webhooks → **switch the receiver off** (or `PUT /api/admin/settings/webhooks` `{"values":{"receiver_enabled":false}}`, admin), then trigger any hook | `503 {"error":"the webhook receiver is off"}`, with or without a token; the panel says "The receiver is off"; list, create, switch and delete still work. Switch it back on: deliveries run again, no restart. The audit row ends `receiver_enabled=false` (then `=true`). Only a literal `true` is on: a hand-edited `"true"` or `1` reads as off, and the panel says the stored value is not true or false. | MAJOR | ✅tests/test_h153_webhook_subscriptions.py · tests/test_h153b_webhook_delivery.py |
| CHN-169 | An event list skips what it does not name (H153) | Create with `"events":["push"]`; trigger with `-H "X-GitHub-Event: ping"`, then with `push` | `202 {"ok":true,"skipped":"event 'ping' is not subscribed"}` and no turn, then `200`. The detail pane shows `1 skipped · last skipped: ping`. A delivery that names no event is skipped too. | MAJOR | ✅tests/test_h153_webhook_subscriptions.py |
| CHN-170 | A prompt template decides what the agent reads (H153) | Create with `"prompt":"New {event} on {repository.full_name}"`; trigger with a GitHub-shaped body and `X-GitHub-Event: push` | The agent receives `New push on <owner>/<repo>`. `{a.b.0}` reads the payload by key and index, `{event}` is the event, `{payload}` the whole body; `{payload.x}` reads the body's own `x` (a key named `event` included), so a sender that wraps its body in a top-level `payload` key (Vercel: `{"type", "payload": {…}}`) needs `{payload.payload.x}`. `{__class__}` renders empty and `{a[0]}` stays as literal text. A template that renders nothing is skipped (`202`), not run; so is a delivery with no text (`the delivery carries no text`). The turn is still an inbound, untrusted one. | MAJOR | ✅tests/test_h153_webhook_subscriptions.py |
| CHN-171 | **Deliver to** sends the reply to your own channel (H153 review) | Create with `"deliver":"telegram"` (or pick *deliver to* in the Webhooks panel: log, telegram, voice, ntfy); trigger it by day with `X-GitHub-Event: push` | The turn runs, and the reply arrives in your Telegram as it was written: `Webhook <name> - push`, a blank line, then the text (nothing rendered or rewritten, so `**x**` and `[a](url)` show their markers and a link its address; no link preview, no voice note; cut at 4,000 characters with `… (cut: the whole reply is in the session)`). ntfy gets the label as an ASCII title of at most 120 characters and the text unrendered; voice speaks it without the markup. The response carries `"delivery":{"channel":"telegram","ok":true}`; the detail pane says `last delivery … · delivered to telegram`. With no owner chat configured: `ok:false` and the reason, never a silent drop and never a 500. | MAJOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-172 | Deliver only skips the agent (H153 review) | Create with `"deliver":"ntfy","deliver_only":true,"prompt":"New {event} on {repository.full_name}"`; trigger a GitHub push. Then try `"deliver_only":true` with `"deliver":"log"` | No turn runs; `New push on <owner>/<repo>` reaches your ntfy topic, titled with the hook. Deliver only with the log is refused (`422 deliver only needs a channel`) on create and on PATCH, and the panel says so before sending. | MAJOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-173 | Quiet hours keep a push off the phone (H153 review) | Set `ambient.quiet_hours_start/end` around now and wait 30 s (the orchestrator reloads its runtime settings every 30 s; the settings PUT does not reload them); trigger a hook that delivers to `telegram`, then a `log` one | The Telegram one is not pushed: `delivery.ok:false`, reason `quiet hours: not pushed; the reply is in the session` (a deliver-only hook: `… the text was not kept`), shown in the detail pane. The push is dropped, not held for the morning. The `log` one is answered as usual. | MINOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-174 | Destinations Nerva does not offer are refused by name (H153 review) | Create with `"deliver":"email"`, `"github_comment"`, `"discord"`, `"slack"`, `"web"`, `"Telegram"`; and with `"skills":["x"]` | `422` naming why: email is never an outbound side effect of an inbound delivery; a GitHub comment goes through governed write-back; Discord and Slack are reply-only here; the HUD has no receiver for a pushed message. A name is matched exactly. An unknown field is refused, never silently dropped. | MINOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-175 | The receiver fails closed (H153 review) | Switch the receiver off; stop the hub, corrupt or lock `settings.db` (e.g. hold `PRAGMA locking_mode=EXCLUSIVE` in a sqlite shell), start it and trigger a hook | `503 {"error":"the webhook receiver's state cannot be read"}` on a fresh start, and `503 … is off` while the hub remembers the last state — never a delivery. A body over 5 MiB is refused with `413` before it is read whole. | BLOCKER if a delivery runs | ✅tests/test_h153b_webhook_delivery.py |
| CHN-176 | A change during the turn reaches the push (H153 third review) | Point a hook at `telegram`; while a slow turn runs, switch the hook off (or the receiver off, or delete the hook, or move it to `ntfy`) | Nothing is pushed to Telegram; the hook says why (`the hook was switched off before its delivery: not sent`, …). Moved to ntfy, the reply goes to ntfy. | MAJOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-177 | A hook makes at most 30 push attempts an hour (H153 third review) | Post 31 deliveries to a deliver-only `ntfy` hook within an hour | 30 arrive; the 31st is `ok:false`, `over this hook's push limit (30 an hour): not pushed; the text was not kept`. Another hook is unaffected. An attempt the channel refused counts (the panel says so); a push held by quiet hours does not. | MINOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-178 | A workflow delivers its result or nothing (H153 third review) | A workflow hook whose last step fails (a timeout, a guardrail) | Nothing is pushed: `the workflow run failed at <step>: nothing delivered`, never an earlier step's output. A missing workflow, an invalid one and a run that raises are each recorded on the hook. A run that stopped early (a `terminate_when` guard) pushes the last step that ran, which is the guard's own output: word the guard for the phone. The last step is the last to run, not the last listed. | MAJOR | ✅tests/test_h153c_webhook_sends.py |
| CHN-179 | A receiver with no stored row can be switched off (H153 third review) | Delete the `webhooks.receiver_enabled` row from `settings.db` (a restore from before H153); open Console → Webhooks | The receiver row says `on · not stored (the default)` and offers `switch the receiver off`, which creates the row. | MINOR | ✅frontend/src/test/webhooks-panel.test.tsx |
| CHN-180 | One long line cannot stall the hub (H153 fourth review) | To a deliver-only `telegram` hook, post a text/plain body of 1 MiB of `[` (or a GitHub issue whose body is 65,536 `[`); meanwhile poll `GET /healthz` | The delivery is answered at once (well under a second) and the health poll never stalls. The push is the first 4,000 characters as written, ending `… (cut: the rest was not kept)`. | BLOCKER if the hub stalls | ✅tests/test_h153d_webhook_review.py |
| CHN-181 | A sender's text arrives as written (H153 fourth review) | Deliver-only to `telegram` and to `ntfy` with the text `pkg/__init__.py https://x.example/*draft*/notes **kept** [a](https://b.example)` | Both show exactly that text: file names and addresses are never rewritten and no markup is rendered. Bidi overrides (U+202E), zero-width spaces and control characters are dropped; an emoji joined with ZWJ stays whole. | MAJOR | ✅tests/test_h153d_webhook_review.py |
| CHN-182 | A burst right after a start is answered (H153 fourth review) | Restart the hub and at once post 10 deliveries to a hook in parallel | All 10 run (200). Only a receiver whose first read has not landed within 2 s is refused, closed (`503 … cannot be read`). | MINOR | ✅tests/test_h153d_webhook_review.py |
| CHN-183 | A workflow hook's answer names its steps, never its context (H153 fourth review) | Trigger a workflow hook whose steps read your data; post a body with `\ud800` in its text | `200 {"ok":…, "target":…, "steps":[ids that ran], "delivery":…}` — no step output, no echo of the sender's text. A lone surrogate becomes U+FFFD and a NaN `null`; never a 500 after the push. | MAJOR | ✅tests/test_h153d_webhook_review.py |
| CHN-184 | `web` is never listed as ready while nothing receives it (H153 fourth review) | `nerva send --list`, or `GET /api/channels/targets` | `web` is connected but not ready: `web has no connected client: nothing would receive it`. | MINOR | ✅tests/test_h153d_webhook_review.py |
| CHN-185 | The hub says when it is listening (H222) | With the host voice pipeline on (wake word installed): `curl /api/voice/listening` idle; say the wake word, speak, wait for the reply; then with a Wyoming satellite paired, have it send `detection` and a transcript; try `POST /api/voice/listening` | Idle: `armed`, `mic_open: true`; during the utterance `listening`, then `thinking`, `speaking`, back to `armed`; the satellite shows as `satellite:<id>` `listening`, then `thinking`, and disappears once answered or disconnected; an unauthenticated satellite (auth on) changes nothing; the POST is 405 — nothing can set it over HTTP | MAJOR if the indicator stays on after a capture ends or a peer can set it | ✅`tests/test_h222_listening.py` |
| CHN-186 | A spoken reply sounds like speech (H526) | Ask for an answer with a code block, a bulleted list, a small table, a link, emoji and "50% -> done at 20°C", in Romanian and in English; hear it through the HUD voice loop (server voice, then the browser voice), `POST /tts`, the wake word and a Telegram voice note; then ask for a reply that is only a code block | Nothing of the code, the pipes, the bullets, the emoji or the URL is read; each list item and row is its own sentence, the link is read by its words ("link" for a bare URL), and the symbols are said as words in the reply's language ("la sută", "spre", "grade Celsius"). While the reply streams, no line inside a code block is spoken even when the ``` arrives split across chunks. The code-only reply is silence: `/tts` answers 204 with `X-Nerva-Speech: nothing_to_say`, the HUD plays nothing and does not fall back to the browser voice, and mobile does not show an error | MAJOR if any code or reasoning is spoken, or a nothing-to-say reply produces an error or an empty clip | ✅`tests/test_h526_speech_text.py`, `frontend/src/test/voice-speech-text.test.tsx` |
...
| 11.6 Rate limiters & taint | 9 (056–064) | ⏱ restarts | 8 ✅ / 1 ⚠️ | CHN-060/061 (the widget-bypass probes) FIXED 2026-08-02. |
...
4. ~~**The public widget message endpoint bypasses the governed gateway.**~~ **FIXED 2026-08-02** — the
   handler now calls `Gateway.route(message, channel="widget")` (via the new `app_state.get_gateway()`
   late-bind), so the per-channel rate limit and inbound taint apply. Deliberately still **no pairing gate**
   (no `sender=`): that gate fails closed and an anonymous visitor has no pairing to approve. Also still **no
   inbox record** — `SUPPORTED_INBOX_CHANNELS` is scoped to channels with a proven reply path, and there is
   no widget send adapter; recording a thread nobody can reply to would be the dishonest option. → CHN-060 ✅.
5. ~~**`widget` is classified as a *trusted internal* turn origin.**~~ **FIXED 2026-08-02** — `widget` was
   removed from `INTERNAL_TURN_CHANNELS`, so an anonymous visitor's message classifies `inbound`: the memory
   leg taint-marks it and the kernel escalates a GRANT to QUEUE for any action it induces. → CHN-061 ✅.
