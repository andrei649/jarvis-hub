...
| ENV-131 | Offline recovery from total token loss | Lose every token, then `python scripts/token_recover.py rotate admin` on the box (a packaged build: stop it and remove `memory/security/tokens.db` under its data home) | Mints a fresh admin token with no HTTP involved, in the store the hub reads — no lockout | MAJOR | ✅ same |
