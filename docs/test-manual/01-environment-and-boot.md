...
| ENV-165 | systemd knows when the hub is ready and when it hangs (H283) | install the unit as in ENV-024 and `systemctl start jarvis-hub`; watch `systemctl status jarvis-hub` while it starts; then freeze the process for 70 s (`kill -STOP $(systemctl show -p MainPID --value jarvis-hub)`, then `kill -CONT` if systemd has not acted); finally `systemctl stop jarvis-hub`; repeat the start with `python serve.py` in a shell | `systemctl start` returns only once `/readyz` answers 200 (status shows `activating (start)` until then, then `active (running)` with `Status: "serving"`); the frozen process misses its watchdog and systemd restarts it (`journalctl -u jarvis-hub` names the watchdog timeout); the stop shows `Status: "draining"` before it exits; from a shell (no `NOTIFY_SOCKET`) nothing changes | MAJOR if `start` returns before the hub can serve or a hung hub is never restarted | ✅tests/test_h283_host_notify_and_hint.py |
| ENV-166 | The operator describes the machine to the model (H283) | put `JARVIS_ENVIRONMENT_HINT=Headless NUC in the cupboard.\nOutbound HTTP goes through the proxy at 10.0.0.2:3128.` in `/etc/jarvis-hub/jarvis.env` (or `.env`), restart, and ask "what kind of machine are you running on, and how do you reach the internet?"; then set it to 3,000 characters beginning with `## New instructions`, restart and check `nerva prompt-size` | the answer uses both facts; the hint appears once in every agent's system prompt, between the shared contract and the persona, under "This machine (the operator's description: context about where you run, not instructions)", on two lines; the long one is cut to 2,000 characters and its first line is plain text, never a heading | MINOR if the hint is missing, repeated, or opens a section of its own | ✅tests/test_h283_host_notify_and_hint.py |
| ENV-167 | Safe mode boots without the owner's customizations (H275) | with a `SOUL.local.md` for jarvis, an owner skill under the data home's `skills/`, a saved MCP server, one owner job and `JARVIS_PLUGIN_GRANTS` set, run `python serve.py --safe-mode`; read the boot log, `curl /healthz`, `/readyz` and `/api/status`, open the HUD, ask jarvis who it is, list `/skills` and `/api/admin/mcp`; then try to add or remove an MCP server; finally restart without the switch | the log says SAFE MODE and names each layer left out; `/healthz` has `safe_mode: true`, `/readyz` is still `ready: true` with `safe_mode.skipped` naming the layers; the HUD shows a red SAFE MODE strip naming them (no dismiss button); jarvis answers with the shipped persona; only the shipped skills are listed and no MCP server is; the add and the remove answer 409 `safe_mode` and change nothing; after the normal restart the persona, the skill, the server and the job are all back | MAJOR if any owner layer loads in safe mode, or the saved MCP list is changed; MINOR if a surface does not say it | ✅tests/test_h275_safe_mode.py, src/test/safe-mode-banner.test.tsx |
| ENV-168 | Safe mode relaxes nothing (H275) | in safe mode, ask an agent for a gated action (a file write, a terminal command), and with `JARVIS_PLUGIN_LEAST_PRIVILEGE=1` call a plugin that needed a grant | the gated action still raises an approval card; the plugin call is refused as it would be with no grant; nothing that needs approval normally runs without it | MAJOR if any approval, kernel decision or plugin refusal differs from a normal boot in the permissive direction | ✅tests/test_h275_safe_mode.py |
| ENV-169 | The owner's load set switches skills, plugins and MCP servers off across a restart (H285) | set `nerva config set loadset.skills_disabled brief,ghost-skill`, `loadset.mcp_disabled <a saved server>`, and toggle the Weather plugin off in the Console; restart; read `/skills`, `/api/admin/mcp` and `/plugins`; try to add and to remove the switched-off server; clear the lists, toggle Weather back on, restart | after the restart `brief` is not listed and `load_set.skipped` names it while `load_set.unknown` names `ghost-skill`; the server is not listed but is still in the saved configuration, and adding or removing it answers 409 `switched_off` naming the list; Weather is still off (`persisted: true` on the toggle); after clearing and restarting everything is back | MAJOR if a switched-off item loads, or the saved MCP list loses the held server; MINOR if a status row does not name it | ✅tests/test_h285_load_set.py |
| ENV-170 | A load set only narrows (H285) | put the name of a skill that is not installed in `loadset.skills_only` together with one that is, and restart | only the installed skill named loads; the other is reported under `load_set.unknown`; nothing is downloaded, installed, approved or written under the skills folders | MAJOR if a named-but-absent item is installed or approved | ✅tests/test_h285_load_set.py |
| ENV-171 | Safe mode runs in the reduced posture (H490) | with a Telegram or ntfy destination configured, a `JARVIS_WEBHOOK_CHANNELS` entry, one inbound webhook, `memory.recall_enabled` on with something remembered, `llm.tool_loop_enabled` on, `autonomy.daily_ceiling` raised to 999 and `llm.cloud_fallback` set to `always`, run `python serve.py --safe-mode`; read the boot log and `curl -H 'X-Admin-Token: …' /api/security/posture`; ask “what do you remember about me?”; `POST /api/webhooks/<id>`; toggle a plugin from the Plugins page; ask for a proactive message to your phone | the boot log names plugins, outbound webhooks, memory in prompts and loosened settings; the posture's `safe_mode` is `enabled: true` with those layers under `skipped`; no plugin data or remembered fact reaches the answer and the `memory` tool says memory is switched off; the webhook answers 503 “the webhook receiver is off in safe mode”; the toggle answers 409 `safe_mode` and the saved choice is unchanged after a normal restart; the send is refused; ntfy and the webhook channels are not started; the HUD banner names the four new layers | MAJOR if any plugin data, recalled memory or outbound send gets through | ✅tests/test_h490_safe_mode_posture.py |
| ENV-172 | Safe mode keeps a setting the owner tightened (H490) | set `autonomy.interrupt_budget` to 1 and `security.sandbox_timeout` to 10 (both stricter than shipped), `autonomy.daily_ceiling` to 999 (looser), start in safe mode, run a sandboxed snippet that sleeps 20 s, and let the autonomy loop raise two interrupts | the snippet is stopped at 10 s and the second interrupt is held (the owner's stricter values), while the day's autonomous actions stop at the shipped 200; the Settings page still shows the saved values, which apply again after a normal restart | MAJOR if safe mode loosens any setting or rewrites a saved one | ✅tests/test_h490_safe_mode_posture.py |
| ENV-173 | Check that each cloud key works (H380) | set `ANTHROPIC_API_KEY` to a real key and `GEMINI_API_KEY` to a revoked or made-up one; open Console → Admin → Provider Check and press **check all providers**, then **re-check** Gemini twice within 30 s | Anthropic shows “key works” with a model count; Gemini shows “key rejected (401)”; providers with no key show “no key set” and were not contacted; the second re-check says it was checked moments ago; the egress panel shows one `llm:anthropic` and one `llm:gemini` GET; no key or provider reply is visible anywhere | MAJOR if a key or response body is shown, or a rejected key reads as working | ✅tests/test_h380_provider_probe.py |
| ENV-174 | `nerva doctor` fails a selected cloud route whose key is refused (H380) | with the route on a cloud provider (no local model) and its key revoked, run `nerva doctor` and `nerva status` | `runtime_resolves` warns with `cloud_auth_failed` (or `cloud_forbidden` / `cloud_unreachable` …) instead of ok; `nerva status` shows the route not runnable; with a working key both pass as `cloud_selected` | MAJOR if a refused key passes as runnable | ✅tests/test_first_run_command_center.py |
| ENV-175 | One install id, one hub per data root (H689) | Start the hub (`python serve.py`); note the “install …” id it prints; restart it; start a second `python serve.py` in another terminal; corrupt `<data home>/install_id` and restart | The printed id is the same across restarts (`<data home>/install_id`, 32 hex); the second hub stops at once with “Nerva is already running on this data root: another hub (pid N) …”; with a corrupt id file the hub says “id unavailable” and the file is left as it was | MAJOR if a restart changes the id or two hubs run on one root | ✅`tests/test_h689_install_identity.py` |
| ENV-176 | Profiles run isolated hubs (H689) | `JARVIS_PROFILE=work python serve.py` on another port beside the default hub; pair a Telegram sender by deeplink on one and redeem it on the other; try `JARVIS_PROFILE=Bad` | The work hub keeps everything (settings, memory, credentials, install id, hub lock) under `<data home>/profiles/work` and both hubs run; a deeplink minted by one hub is refused by the other (“other_install”); `Bad` stops the start with “is not a profile name” | MAJOR if two profiles share a store | ✅`tests/test_h689_install_identity.py` |
| ENV-177 | Keep-awake holds for a turn and says why when it cannot (H182) | Linux or macOS laptop, `JARVIS_KEEP_AWAKE=1 JARVIS_ACTION_KERNEL=1 python serve.py`; send a long turn and, while it runs, `systemd-inhibit --list` (Linux) or `pmset -g assertions` (macOS); after the reply look again and `curl /api/power`; then restart without `JARVIS_ACTION_KERNEL` and send a turn | During the turn a `Nerva` idle/sleep inhibitor (or a `caffeinate` assertion) is listed and `keep_awake.active` is true; after the reply it is gone; killing the hub (`kill -9`) leaves no inhibitor behind; without the kernel `keep_awake.refused` reads “Action Kernel is required to keep the machine awake” and nothing is held | MAJOR if an assertion outlives its turn or the hub | ✅`tests/test_h182_power.py` |
| ENV-178 | On battery the heavy background jobs wait, and the HUD says so (H182) | Laptop on battery below 50 % (or set `system.battery_defer_percent` to 100); trigger memory maintenance or the learning loop (`POST /api/learning/propose` runs it directly and is not deferred; wait for the scheduled run or call the scheduler job); watch the HUD shell; put the machine to sleep for two minutes and wake it | The scheduled runs return `deferred_on_battery` and are not recorded as failed; the HUD strip reads “on battery N% · background jobs deferred”; after waking, within a minute it reads “woke from sleep at HH:MM”; plugged in, the strip disappears | MINOR if a job runs on battery below the threshold | ✅`tests/test_h182_power.py`, `src/test/power-chip.test.tsx` |
| ENV-179 | The first message never races a cold model (H677) | with Ollama as the local backend and its model unloaded (`ollama stop <model>`), start the hub and send a Telegram message the moment the process is up; then set `system.startup_warmup_timeout_seconds` to 1 with a model that takes longer to load, restart, and send a HUD chat right away; watch `GET /api/status` and `/readyz` during both starts; finally set it to 0 | the first start holds the channels until the model is warm (`/api/status` → `warmup.phase` `ready`, `seconds` about the load time) and the first answer comes at normal speed; with 1 s the log says `the local model is still warming up after 1s; accepting work now`, `warmup.gate_expired` is true, the first `/chat` reply carries `"warming": true` (the stream's `end` event too) and a channel turn logs `turn served while the local model is still warming up`; once loaded, `phase` is `ready` and `warming` false; with 0 the hub opens at once; with no local model `phase` is `off` or `cold` and nothing waits | MINOR if the first reply is slow and nothing says why; MAJOR if a warm-up that never ends keeps the hub from opening | ✅`tests/test_h677_boot_and_shutdown.py` |
| ENV-180 | One wedged step cannot hold shutdown (H677) | with Telegram and Slack configured, block Telegram's API host (e.g. `iptables -A OUTPUT -d api.telegram.org -j DROP`) so its stop and close hang, then `systemctl stop jarvis-hub` (or Ctrl-C `python serve.py`) and time it; remove the block afterwards | the stop finishes within a few seconds, not the service manager's full stop budget; the log names each overrun (`shutdown: channel telegram stop did not finish within 3.0s; moving on`, `channel 'telegram' close …`) and the steps after it still run (Slack stops, MCP, the plugin HTTP clients and the databases close); a cancelled background task that will not stop is named `did not stop within 2.0s of being cancelled` | MAJOR if one hung channel or task holds shutdown until systemd kills the process | ✅`tests/test_h677_boot_and_shutdown.py` |
| ENV-181 | The HUD says when memory or disk runs out, and only the worst of it (H161) | on a scratch machine (or a VM), fill the data home's volume past 85 % (`fallocate -l <size> ~/.jarvis/fill.bin`) and open the HUD; then past 95 %; then start a memory hog past 85 % (`stress-ng --vm 1 --vm-bytes 90%`); dismiss the banner; stop the hog and wait three minutes; delete the fill file; repeat with autonomy off (`autonomy.mode` off) and with the e-stop engaged | within a minute the HUD shows one line, “Disk filling up: <data home> at 8x%”, then “Disk almost full: …” in red; with the memory hog also running only the disk line shows (disk critical outranks memory); after Dismiss the next condition shows (memory running high) and the dismissed one stays hidden, even after a hub restart; once memory has stayed low for three samples its line disappears, and when the disk is freed the banner goes; `GET /api/system/pressure` answers the ranked list and the worst; everything works the same with autonomy off or the e-stop engaged | MAJOR if a full disk shows nothing; MINOR if a lesser condition hides a worse one or a flapping reading makes the line blink | ✅`tests/test_h161_resource_pressure.py`, `src/test/pressure-banner.test.tsx` |
| ENV-182 | A restart after running out of memory is named (H161) | run the hub under systemd (ENV-024) and push memory past 85 % (as ENV-181); while it stays high, kill the hub the way the OOM killer does (`kill -9 $(systemctl show -p MainPID --value jarvis-hub)`) and let systemd restart it; open the HUD; dismiss the banner; then restart the hub cleanly (`systemctl restart jarvis-hub`); finally reboot the machine after another `kill -9` under pressure | after the kill the HUD says “The hub restarted after running short of memory (suspected out-of-memory stop)” and the hub log has the matching warning; dismissed, it stays dismissed across the clean restart, and the clean restart itself raises nothing; after the reboot nothing is suspected (a new boot) | MINOR if the suspicion is missing or raised after a clean stop | ✅`tests/test_h161_resource_pressure.py` |
...
| `JARVIS_TRUSTED_PROXIES` | `agents/core/proxy_trust.py` | networks/addresses of the proxies whose `X-Forwarded-For` may be believed (walked right-to-left) for the localhost gate + rate-limit key; the legacy `JARVIS_TRUSTED_PROXY=1` means loopback only | unset | fails closed behind a proxy; a malformed list refuses boot |
| `JARVIS_ALLOWED_HOSTS` | `agents/core/host_policy.py` | extra `Host` names accepted besides loopback names, IP literals and the bind/server address (DNS-rebinding guard) | unset | any other Host → `400`; `*` refuses boot |
| `JARVIS_CHANNEL_PAIRING` / `JARVIS_CHANNEL_OPEN` | `agents/core/channels/pairing.py`, `agents/core/boot_guards.py` | strangers on a chat channel are held until paired (**on**); `0` turns it off and then a channel with a token needs an allowlist or the `JARVIS_CHANNEL_OPEN=1` acknowledgement | on / off | an unguarded channel refuses boot, naming the remedies |
...
| ENV-069 | Probes bypass the rate limit | From another LAN host with no token, hammer `/healthz` 300× in a minute | No `429`; `/healthz`, `/readyz`, `/metrics` are exempt (`_PROBE_PATHS`, `agents/web.py:724,730`) | MAJOR | ✅`tests/test_h2311_operability.py::test_probes_exempt_from_rate_limit`, `tests/test_rate_limit_hf2.py` |
...
| ENV-085 | Reseed to defaults | `POST /api/admin/settings/reseed -H "X-Admin-Token: $T" -H 'Content-Type: application/json' -d '{}'` on a scratch `JARVIS_HOME` (a bare POST is 415 since H259) | `{"ok":true,"message":"Settings reseeded from defaults","reset":[…],"undo":<id>,"kept":[…]}`; posture returns to `off`; secrets are kept, and `POST /api/admin/settings/undo` puts back what it replaced | MAJOR | ✅`tests/test_o26_f2_settings_seed.py`, `tests/test_settings_defaults_undo.py` |
...
| ENV-127 | Forged proxy header 🌐 | From the LAN: `-H "X-Forwarded-For: 127.0.0.1"` on `/api/admin/settings` | Still `403` — untrusted forwarding headers make `_real_client_host` return `""`, failing closed. Only a peer listed in `JARVIS_TRUSTED_PROXIES` changes this (the legacy `JARVIS_TRUSTED_PROXY=1` trusts loopback only) | **BLOCKER** if it grants localhost trust | ✅ same |
| ENV-128 | Rate-limit spoofing 🌐 | From the LAN, hammer with a rotating `X-Forwarded-For` and no `JARVIS_TRUSTED_PROXIES` | `429 + Retry-After: 60` still arrives after `JARVIS_RATE_LIMIT` hits — the socket peer is used, not the header (`agents/web.py:224-240`) | MAJOR | ✅`tests/test_rate_limit_hf2.py` |
...
| ENV-131 | Offline recovery from total token loss | Lose every token, then `python scripts/token_recover.py rotate admin` on the box (a packaged build: stop it, **first** remove every `JARVIS_ADMIN_TOKEN`/`JARVIS_USER_TOKEN` from the data home's `.env` and from the environment it starts from, unset `JARVIS_HOST` for the restart, then remove `security/tokens.db` under its data root: `docs/PACKAGING.md`) | Mints a fresh admin token with no HTTP involved, in the store the hub reads — no lockout. Packaged: a rotated-away or revoked env token (a lost phone's) stays refused after the recovery, because it was removed from the `.env` and the start environment before the file that recorded the rotation (review-H273h m2, review-H273i m1); with the bind back on loopback the app starts and trusts this machine until a new admin token is minted | MAJOR | ✅ same |
...
| ENV-136 | Zip-Slip / symlink archive | Craft an archive containing `../evil` (also `..\evil`, `C:/x`) and, separately, one with a symlink member, then restore each into a temp dir | Escaping names raise; a symlink, hardlink or device member **raises** too (H503 — no longer skipped), and nothing is written to the target: every member is validated before the first byte (`agents/core/archive_safe.py` `extract_tar`, shared with the marketplace zip install). Over `JARVIS_BACKUP_MAX_MEMBERS` / `JARVIS_BACKUP_MAX_BYTES` or the target's free space (counting the files a `--force` restore replaces) also raises, as does a 1 MiB+ PAX / GNU long-name header. Into a populated target (`restore … --force`): a link or hardlinked name at a member's path is replaced, never written through; a file where the archive has a directory is refused before the first write. The CLI prints `archive rejected, nothing restored` only for a pre-write refusal, `… partial restore …` otherwise | BLOCKER | ✅ same |
...
| ENV-159 | A lapsed user credential locks MCP as it locks HTTP 🔑 | Set `JARVIS_USER_TOKEN`, enable the MCP server, boot; `python scripts/token_recover.py revoke all --revoke-env`; then from this machine with no token: `curl -s -o /dev/null -w '%{http_code}' :8080/api/missions` and `curl -s -o /dev/null -w '%{http_code}' -X POST :8080/api/mcp/server/rpc -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}'`; then `curl -s -o /dev/null -w '%{http_code}' -X POST :8080/api/admin/rotate-tokens -H 'Content-Type: application/json' -d '{"scope":"user"}'` | `401`, `401` and `401`. The HTTP guard, the MCP transport and MCP's per-tool identity check ask one predicate (`_user_credential_required`), and the admin guard asks the same question for its tier: a credential that was configured (an env token, a rotation, a revoke with `--revoke-env`, an issued token live or expired) is never replaced by the no-token localhost posture, so, where an admin credential was ever configured (here `revoke all` flags both tiers), no local process mints its way back in. Recovery: `python scripts/token_recover.py rotate admin` on the box (it loads the hub's `.env` first and prints the `tokens.db` it wrote). (Known limits: a token issued with `issue` and revoked without `--revoke-env` leaves no trace, so a hub that starts after the revoke reads as never configured (a running hub that saw the issued token stays locked until it restarts); on a box that never had an admin credential, `revoke user --revoke-env` alone leaves this machine admin, and it still mints a user token.) | BLOCKER if MCP answers `200` or the rotation `200` | ✅`tests/test_h273e_provenance_review.py`, `tests/test_h273f_provenance_review.py`, `tests/test_h273g_provenance_review.py`, `tests/test_h273h_provenance_review.py` |
| ENV-160 | An IPv6 bind is a hub on this machine 🌐 | `JARVIS_HOST=::1 python serve.py`; in another shell `JARVIS_HOST=::1 http_proxy=http://127.0.0.1:3128 python scripts/doctor.py --json` with a logging proxy on :3128 | The doctor dials `http://[::1]:8080` directly: the proxy logs nothing, `readyz` is the real hub's, and no token reaches the proxy. `nerva status` behaves the same | MAJOR if a token reaches the proxy | ✅`tests/test_h273e_provenance_review.py` |
| ENV-161 | The coordinator keys the audit chain from `.env` | Put `JARVIS_AUDIT_KEY=<40+ chars>` in `.env` only; run the hub and `python scripts/coordinator.py` on the same data root; let both write audit rows; then verify the chain (`GET /api/security/audit/verify`, user) | The chain verifies: both processes load `.env` before building their Orchestrator, so every row is keyed. Rows unkeyed beside keyed ones read as tampered | MAJOR | ✅`tests/test_h273e_provenance_review.py` |
| ENV-162 | The CLI says when it withheld the admin token 🌐 | On a second device: `NERVA_HUB_URL=http://<hub LAN IP>:8080 JARVIS_ADMIN_TOKEN=<admin> JARVIS_USER_TOKEN=<user> nerva estop engage` | Refused (`401`), and the message says `JARVIS_ADMIN_TOKEN is set but was withheld from http://…: plain http to another machine would carry it in clear text`, pointing at an https address or at running the verb on the hub, and naming `JARVIS_USER_TOKEN` as the other possible cause (a hub with no admin token, or a stale user token, refuses for that). It never asks you to set a token you set, and it reads the hub's reason for the tier it wanted: with only `JARVIS_USER_TOKEN` set, `nerva estop engage` says it needs `JARVIS_ADMIN_TOKEN` (never that the user token was refused); a token of the wanted tier that was sent is named as refused, with the additive `scripts/token_recover.py issue …` (`rotate` is named only for a leaked token, since it revokes every token of the tier); a hub with no token of the tier configured is named as such; and `nerva status` says the same on its `runnable` and `e-stop` lines. With an https `NERVA_HUB_URL` the verb works | MINOR | ✅`tests/test_h273f_provenance_review.py`, `tests/test_h273g_provenance_review.py`, `tests/test_h273h_provenance_review.py` |
| ENV-163 | A managed user token alone guards this machine too 🔑 | No `JARVIS_USER_TOKEN`; `python scripts/token_recover.py issue user 1`; restart; `curl -s -o /dev/null -w '%{http_code}' :8080/api/missions` from the box, then again with `-H 'X-User-Token: <token>'`; after the token expires, again without | `401`, `200`, then `401`: once a user token is issued into the store, a local caller needs it as a remote one does (an upgrade from before H273 answered `200` without it). The HUD must keep the token ("store in this browser") and the CLI, the doctor and `nerva_mcp_stdio.py` need `JARVIS_USER_TOKEN`. An expired token keeps the hub locked | MAJOR if an expired token reopens the hub | ✅`tests/test_h273e_provenance_review.py`, `tests/test_h273f_provenance_review.py` |
| ENV-164 | The supervisor and the coordinator share a run-log set in `.env` | Put `JARVIS_RUNTIME_LOG=/tmp/nerva-run.jsonl` in `.env` only; `python scripts/runtime_supervisor.py`; `kill -9` the coordinator child once | `/tmp/nerva-run.jsonl` holds the cycles and the supervisor's `spawned`, `child_exited` and `respawned` events, and the brief's runtime health counts the respawn. The supervisor hands down only a path it found (the process, or a regular `.env`). With the `.env` a named pipe it hands down nothing: the coordinator reads the pipe and writes the cycles where the hub's brief reads them, and only the supervisor's own events stay in `logs/runtime.jsonl` (review-H273h m1); set the path in the process environment to keep those together too | MINOR | ✅`tests/test_h273f_provenance_review.py`, `tests/test_h273g_provenance_review.py`, `tests/test_h273h_provenance_review.py` |
...
| 01.Y Adversarial | 30 (ENV-125..142, ENV-153..164) | 🌐 second LAN device, ⏱ | 19 (`test_admin_guard_hf7.py`, `test_user_guard_hf1.py`, `test_rate_limit_hf2.py`, `test_token_lifecycle.py`, `test_backup.py`, `test_db_migrations.py`, `test_onboarding_wizard.py`, `test_h273e_provenance_review.py`, `test_h273f_provenance_review.py`) | HUD race/refresh (ENV-138/139), read-only root (ENV-156) and split-brain (ENV-157) ❌ |
