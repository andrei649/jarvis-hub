...
| ENV-131 | Offline recovery from total token loss | Lose every token, then `python scripts/token_recover.py rotate admin` on the box (a packaged build: stop it, **first** remove every `JARVIS_ADMIN_TOKEN`/`JARVIS_USER_TOKEN` line from `Documents/Nerva/.env`, then remove `security/tokens.db` under its data root: `docs/PACKAGING.md`) | Mints a fresh admin token with no HTTP involved, in the store the hub reads — no lockout. Packaged: a rotated-away or revoked env token (a lost phone's) stays refused after the recovery, because its line was removed before the file that recorded the rotation (review-H273h m2) | MAJOR | ✅ same |
