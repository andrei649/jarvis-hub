...
#### WFL-088 — Manual flag, rubric score, thumbs, and "add to dataset"
- **Surface:** `POST /api/review/flag`, `POST /api/review/{id}/vote`, `POST /api/review/{id}/dataset` · **Tier:** all **user** · **Auto:** ✅tests/test_h10_25_review_queue.py (7)
- **Steps:** 1) Manual flag:
...
- **The bug to confirm at (6):** the promoted case is `{"input": <text_preview>, "expected": "", "verdict", "rubric", "source":"human_review", "trace_id"}` (`observability/review_queue.py:88-97`), but `run_dataset` builds `EvalCase(prompt=c.get("prompt",""), expect_contains=c.get("expect_contains"))` (`observability/datasets.py:258-266`). So a review-promoted case has an **empty prompt** and **no criterion** — and `EvalHarness._evaluate` with neither `expect_contains` nor `scorer` returns `(True, 1.0)` ("No criterion — pass by default", `observability/eval.py:116-117`). Now `POST /api/eval/datasets/run -d '{"name":"qa_review"}'` and read the score.
- **Expected at that final step (the meta-check):** a truthful stack would report the case as unscoreable. What the source predicts: `score: 1.0`, `passed: 1`, `total: 1`, with `results[0].response` being whatever the model says to an **empty prompt**. That is a perfect 100% eval score derived from a flagged-as-bad answer — a fabricated metric produced by the eval stack itself.
- **FAIL if:** you observe `score: 1.0` on a promoted review case → **MAJOR**, and say plainly in the report that the "add to dataset → run the dataset" loop produces a meaningless perfect score. If the HUD EVAL card then shows `last_score: 1` next to a queue of bad answers → **BLOCKER** (a green metric asserting quality that does not exist).
...
| 10.14 Review Queue | 6 (087–092) | 🤖, ⏱ for 091 | 4 ✅ / 2 ❌ | WFL-088 is the "promoted case scores 1.0" meta-defect |
...
9. **`GET /api/workflows/traces` is open-tier and echoes prompt/output content.** `engine.py:183-192` stores 160-char `input_preview`/`output_preview` with no redaction, and the route has no guard (`route_auth.json`: `open`). Any secret in a workflow input is readable there. → WFL-132.
10. **Review-queue → dataset promotion produces an unrunnable case that scores a perfect 1.0.** `observability/review_queue.py:88-97` emits `{"input", "expected", …}` while `observability/datasets.py:258-266` reads `prompt`/`expect_contains`; with neither, `observability/eval.py:116-117` returns `(True, 1.0)`. So the flagged-as-bad answer becomes a 100%-passing eval case run against an empty prompt. → WFL-088.
11. **The EVAL DATASETS card's regression view can never show a regression.** `frontend/src/gap.tsx:868` reads `cmp.regressions`/`cmp.improvements`; the API emits `regressed`/`improved` (`datasets.py:239-241`). The same card renders `v{x.version}` (`gap.tsx:858`) against an API field named `latest_version` (`datasets.py:157`), so the version chip reads `vundefined`. → WFL-078.
12. **The REVIEW QUEUE card shows no text.** `gap.tsx:879` reads `it.preview || it.text`; the API sends `text_preview` (`review_queue.py:49`). Rows render as bare 👍/👎 buttons. → WFL-090.
