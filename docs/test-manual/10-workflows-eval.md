...
| 10.10 Templates / sandbox / H32 | 5 (067–071) | 🖥 Docker + pinned image | 2 ✅ / 2 ⚠️ / 1 ❌ | H32 HTTP drive route shipped (A8-i); still no HUD trigger |
...
15. ~~**No HTTP entry point for the H32 acquisition loop.**~~ **FIXED (A8-i, 2026-08-02)** — `POST /api/acquisition/{request_id}/drive` (`agents/core/routers/acquisition.py:230`, admin) drives reuse-check → research → generate → sandbox → propose, and `capture_gap` is production-wired as the agent tool-loop's `gap_callback` (`agents/core/autonomy_coordinator.py:527`). Residue: still no HUD trigger — curl + admin token only (see §12.6 "Honest limit, updated 2026-08-02").
