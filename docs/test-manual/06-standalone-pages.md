...
| PGE-176 | Rate limiting from a LAN device 🌐 | from a second device, POST >`JARVIS_RATE_LIMIT` (default 120) widget messages in a minute | `429` + `Retry-After`; localhost is exempt. **Since 2026-08-02** the widget also routes through the governed gateway, so the per-channel window (`channels.rate_limit`, default **10**/min) bites first — expect throttling well before 120 (CHN-060). | MINOR | ✅ |
...
| PGE-249 | A background HUD keeps a streaming reply (H182) 🖥 | macOS 14+, desktop app: send a long turn, switch to another app for five minutes; then quit, relaunch with `NERVA_DESKTOP_BACKGROUND_THROTTLING=suspend` and repeat; try `=sometimes` | With the default the reply finishes while the window is in the background (the turn is not cancelled); with `suspend` WebKit may pause the page; `sometimes` prints “…is not one of disabled, throttle, suspend; using disabled” and behaves as the default. Linux and Windows ignore the policy | MINOR | ✅`tests/test_h182_power.py` (rustc unit tests of `throttling.rs`) |
| PGE-250 | The tray says when Nerva is listening (H222) 🖥 | Desktop app: hide both windows; turn the HUD mic on from the floating chat, then off; say the wake word to the host pipeline; hover the tray icon | macOS: the menu bar shows `● listening` while a mic records and `○ wake word` while the wake word is armed, nothing otherwise, and the tooltip names the state; Linux shows the title next to the icon; Windows shows the tooltip only. The tray has no control that opens or closes a mic | MINOR | ✅`tests/test_h222_listening.py` (rustc unit tests of `indicator.rs`), `src/test/listening.test.tsx`, `src/test/desktop-listening.test.ts` |
