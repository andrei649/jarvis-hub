...
- **Kernel default-off, with a fail-closed MCP route-mutation exception.** The action kernel (`JARVIS_ACTION_KERNEL`) remains opt-in for legacy action families; all privileged-action kinds are kernel-mediated (zero `PENDING_KERNEL` remain) once enabled. MCP mutating route tools are stricter: disabled/unavailable/raising kernel, `DENY`, and `QUEUE` all refuse before the adapter, and a durable authorization audit row must succeed before an explicit `GRANT` can write. A capability token is mandatory for admin.\*/kg.write; the already-authenticated operator route mints its own short-lived token when absent.
- **MCP `ask_*` calls persist conversation state.** HTTP RPC identity is enforced at the transport through OAuth/user-token or the local-boundary policy. The production runner writes the caller text before agent routing and normally writes the reply afterward; this transcript persistence is outside the Action Kernel and has no mandatory pre-write security-audit row. Downstream actions keep their own authority/kernel gates. Transcript lifecycle follows `retention.enabled` plus `retention.conversation_ttl_days` (90-day configured default once retention is enabled; otherwise no automatic deletion), so MCP callers must treat prompts and replies as retained owner data.
