...
| T2 | **Unauthorized privileged action** — autonomy acts without approval | The **action kernel** (`kernel.authorize`) is an additional **deny** layer, not the only one — with the kernel off, legacy payment contracts and approval queues still run, while MCP route mutation and local-model lifecycle fail closed. When enabled: kill-switch + capability + policy → `grant/deny/queue`; irreversible actions are queued for approval. `host.control` covers LM Studio/Ollama start/load/unload and the **action-auth matrix** (`test_action_auth_matrix.py`) fails CI if a privileged action bypasses the kernel. |
...
| T11 | **Supply-chain** — tampered dependency or CI action | Hash-pinned lockfiles (`--require-hashes`); SHA-pinned GitHub Actions; weekly scheduled CodeQL scan; SAST (bandit/semgrep) + secret-scan (gitleaks) + dependency-CVE (pip-audit) lanes on every PR (`AUD-10`, `.github/workflows/security.yml`) plus the lockfile-drift lane (`lockfile.yml`). *(2026-08-29: removed by owner decision (#981); restored 2026-09-02 (CTO decision D1) — they block a merge only once the owner marks them required in branch protection.)* |
...
- **Single-user model.** No per-user isolation/RLS today — one user per install (a 1.0 decision, ratified 2026-09-01: [`docs/decisions/2026-07-11-single-user-1.0.md`](decisions/2026-07-11-single-user-1.0.md) / H23.23). Multi-tenant is **out of scope** for 1.x; per-user isolation reopens only when a design partner needs several distinct people on one shared install.
- **Kernel default-off, with fail-closed MCP/local-model exceptions.** The action kernel (`JARVIS_ACTION_KERNEL`) remains opt-in for legacy action families; all privileged-action kinds are classified (zero `PENDING_KERNEL`). MCP mutating route tools and LM Studio/Ollama lifecycle are stricter: disabled/unavailable/raising kernel, `DENY`, and `QUEUE` all refuse before effect, and a durable authorization audit row must succeed before an explicit `GRANT` can mutate. A capability token is mandatory for admin.\*/kg.write; the already-authenticated operator route mints its own short-lived token when absent.
- **MCP `ask_*` calls persist conversation state and can carry one explicit host effect.** HTTP RPC identity is enforced at the transport through OAuth/user-token or the localhost boundary. The production runner writes the caller text before agent routing and normally writes the reply afterward; transcript persistence is outside the Action Kernel and has no mandatory pre-write security-audit row. Direct skill commands are refused. An explicit LM Studio/Ollama start/load/unload request is allowed only through `ask_jarvis`, and only after owner identity/local-boundary verification, `system-control` permission, the `host.control` contract, Action Kernel `GRANT`, and durable audit preflight; direct `handle_input(channel="mcp")` calls cannot manufacture that server-scoped authority. Transcript lifecycle follows `retention.enabled` plus `retention.conversation_ttl_days` (90-day configured default once retention is enabled; otherwise no automatic deletion), so MCP callers must treat prompts and replies as retained owner data.
