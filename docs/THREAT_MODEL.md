...
| T2 | **Unauthorized privileged action** — autonomy acts without approval | The **action kernel** (`kernel.authorize`) is the single front door: kill-switch + capability + policy → `grant/deny/queue`; irreversible actions are queued for approval; the **action-auth matrix** (`test_action_auth_matrix.py`) fails CI if a privileged action bypasses the kernel. |
...
| T5 | **Audit tampering** | The audit chain is **HMAC-SHA256** with an off-box key (`JARVIS_AUDIT_KEY`); a forged/edited row fails verification (`security/audit.py`, `test_audit_hardening.py`). |
...
- **Kernel migration in progress.** The action kernel is default-off (`JARVIS_ACTION_KERNEL`) and migrating callers in waves; admin security routes (B1) and the audited strict-egress downgrade (B3) are still `PENDING_KERNEL`.
