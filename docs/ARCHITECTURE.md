...
| `agents/core/todo_tool.py` | `todo` ToolRPC tool (H315): the model's own checklist for the session — items `{id, content, status}` (pending / in_progress / completed / cancelled), replace or merge by id, every call answers with the whole list and the loop never stubs it; bounded (50 items, 200 chars, one in progress, 256 plans in memory) with named refusals; ungated and session-local (`tool_profiles.SESSION_LOCAL_TOOLS`, every posture); the owner reads it via `GET /sessions/todo` · `/sessions/{id}/todo`, `nerva todo`, the Decision Inbox, and a `todo_updated` tool event (ids and statuses only); a forget clears it | `TodoStore`, `TODOS`, `register_todo_tool`, `TodoError` |
