...
| `agents/core/proxy_trust.py` | Reverse-proxy trust as a bounded CIDR allowlist (`JARVIS_TRUSTED_PROXIES`; the legacy `JARVIS_TRUSTED_PROXY=1` means loopback only): forwarding headers honoured only from a listed peer, `X-Forwarded-For` walked right-to-left past trusted hops, fail closed otherwise; a malformed list refuses boot | `parse_trusted_proxies`, `trusted_proxies`, `is_trusted_peer`, `forwarded_client`, `assert_parseable_trusted_proxies` |
