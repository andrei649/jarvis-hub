...
| `agents/core/orchestrator_bindings.py` | Structural contract, typed writer API, and inventory for lifecycle slots populated outside `Orchestrator`; every slot exists as explicit `None` before plugin/web/autonomy/ambient/scheduler wiring runs | `ExternalOrchestratorBindings`, `bind_external_orchestrator_attribute`, `EXTERNAL_BINDING_WRITERS` |
