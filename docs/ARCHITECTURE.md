...
| `agents/core/archive_safe.py` | **The only archive extractor/producer (H503).** Backup verify/restore (tar) and the marketplace zip install both unpack through it: names normalised (`\` folded; absolute, drive-lettered, UNC, `..` refused), regular files + dirs only — symlink/hardlink/device members **raise**, validated before the first byte; member-count + uncompressed-bytes caps (clamped to free space). Producer side: `atomic_write` (sibling temp → fsync → `os.replace`) and a link-skipping export walk. `tests/test_archive_safe.py` fails if another product module extracts an archive | `extract_tar`, `extract_zip`, `normalize_member`, `ArchiveLimits`, `ArchiveRejected`, `atomic_write`, `collect_regular_files` |
