...
| `agents/core/autonomy/jobs.py` + `agents/core/routers/jobs.py` | Owner-scheduled jobs: SQLite `JobStore`, `JobRunner` on the heartbeat APScheduler, actions `remind` / `ask` (notepad between runs) / `brief` / `task` (through the autonomy queue), self-pause after 3 failures with one incident, e-stop aware, five blueprints; a message that would land in the owner's quiet hours is held and flushed when the night ends, an `urgent` one spends the daily interrupt budget; admin routes under `/api/jobs` | `JobStore`, `JobRunner.fire`, `JobRunner.flush_held`, `JobRunner.quiet_hours`, `resolve_schedule`, `cron_kwargs`, `BLUEPRINTS`, `instantiate_blueprint` |
