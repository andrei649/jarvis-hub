...
| `agents/core/llm/host_protocol.py` | H368 — the host decides the wire protocol: `api.anthropic.com` → Anthropic Messages, the `api.openai.com` family → OpenAI, `bedrock-runtime.<region>.amazonaws.com` → Bedrock Converse, matched on the parsed hostname (never a substring, so lookalikes and path spoofs mandate nothing). `egress.llm_async_client` refuses a mismatched backend before the request and its credential leave (ledger row `allowed=False`), and `HybridRouter.detect` refuses a mismatched compatible base URL | `host_mandated_protocol`, `protocol_refusal`, `backend_protocol`, `BACKEND_PROTOCOLS`, `HostProtocolRefused` |
| `agents/core/llm/provider_routing.py` | H583 — OpenRouter's `provider` object (`sort` / `only` / `ignore` / `order` / `require_parameters` / `data_collection`) from the `llm.openrouter_*` settings; `data_collection` seeded `deny`; slugs validated on write and at build, an invalid knob refuses the backend rather than widening routing; sent to the openrouter profile only | `build_provider_block`, `provider_routing_from_settings`, `slug_list_problem`, `ProviderRoutingInvalid` |
