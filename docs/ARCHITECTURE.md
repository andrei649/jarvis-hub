...
| `agents/core/image_generation_runtime.py`, `media_backends/comfyui.py` | Optional local ComfyUI generation: exact durable ToolRPC approval, kernel check at execution, one-attempt consumption, fixed workflow, bounded PNG validation and atomic artifact publication; configuration is separate from measured reachability | `LocalImageRuntime`, `ComfyUIConfig`, `ComfyUIBackend` |
