...
| `agents/core/file_tools.py` | Governed file tools: `file_read` (a `.pdf`/`.docx` as extracted text when its parser is installed, `raw=true` for bytes; `offset` pages from any byte and a short read returns `next_offset`, never splitting a UTF-8 character — H661)/`file_list`/`file_search` (literal, bounded content search with every cap reported) ungated inside `JARVIS_FILE_ROOTS`; `file_write`/`file_delete` gated ask-tier ToolRPC tools that snapshot the previous bytes first. Kernel kind `file.write`, rollback mode `restore`. Flag `JARVIS_FILE_TOOLS` (off ⇒ `register_file_tools` is a no-op) | `FileTools.search_files`, `FileScope`, `FILE_WRITE_CONTRACT`, `restore_snapshot`, `register_file_tools`, `GATED_TOOL_KINDS` |
