...
| `agents/core/llm/anthropic.py` | Claude API backend (text + tool turns). A tool turn marks the session-stable prefix — the system prompt and the tool array — for prompt caching, and reads the provider's own token counts back onto `ToolTurn.usage` so the cost meter stops estimating (H363) | `ClaudeBackend` |
