...
| `agents/core/orchestrator_bindings.py` | Structural contract and writer inventory for lifecycle slots populated outside `Orchestrator`; every slot exists as explicit `None` before plugin/web/autonomy/ambient/scheduler wiring runs | `ExternalOrchestratorBindings`, `EXTERNAL_BINDING_WRITERS` |
