...
| **F-06** | WorldView secure-mode vs Jarvis bridge auth mismatch | **Plausible (not re-verified here).** Bridge sends no Authorization; enabling `WORLDVIEW_AUTH_SECRET` would break read calls. Only matters for non-local WorldView. |
| **F-07** | PluginGate not a hard egress boundary | **Plausible (design).** Manifest is declarative; `PluginHTTPClient` doesn't enforce per-plugin domain/LAN/SSRF. Defense-in-depth, P2. |
