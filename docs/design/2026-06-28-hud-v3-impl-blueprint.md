...
|---|---|---|---|---|---|
| B1 ✅ | Decision Inbox (`v3-decisions.jsx`) | `gap.tsx` `DecisionInboxPanel` (Autonomy cluster, first) | `GET /autonomy/tasks?status=blocked` · `POST /autonomy/tasks/{id}/decision` `{action,payload}` | **The resolve action was genuinely missing** — the queue was *read* (network fan) but never resolvable. Now **accept / edit / reject / defer** (all 4 backend actions) on the blocked queue + a **dry-run preview** (`GET /api/autonomy/tasks/{id}/preview` — see effects/irreversible/would-execute before approving); edit opens an inline JSON payload editor + re-gates; empty = honest "all clear". **Shipped PR 12 (accept/reject/defer) + PR 15 (edit) + PR 17 (dry-run preview).** | `cockpit` / `autonomy` |
