'use strict';
/* HUD v3 · NEURAL MESH — native cinematic live brain (replaces the /brain iframe).
   Arc-reactor core · cost-sized model shell · tier-coloured agent constellation that
   slowly rotates · comet token-flow on attribution edges · auto-choreographed cascades
   so it's alive on camera. Original implementation (not derived from third-party code). */
const { useRef:uRm, useEffect:uEm, useState:uSm } = React;
...
const TIER_COLOR = { CNS:'#2bb8f0', command:'#2bb8f0', BUS:'#ffb23f', business:'#ffb23f',
  TEC:'#a78bfa', tech:'#a78bfa', FND:'#41f59b', foundation:'#41f59b' };
const agentColor = a => (a && (TIER_COLOR[a.tier]||TIER_COLOR[String(a.tier||'').toLowerCase()])) || '#5fa8d8';

function NeuralMesh({ agents, activeId, onSelect, motion, cinema, t }){
  const wrapRef = uRm(null), canvasRef = uRm(null);
  const S = uRm({ nodes:[], edges:[], particles:[], rings:[], stars:[], hover:null,
    w:640, h:460, cx:320, cy:230, dpr:1, raf:0, tick:0, lastPulse:0, lastCascade:0, cascadeI:-1, focus:null });
  const [tip,setTip] = uSm(null);
  ...
  function build(){
    ...
    const nodes=[], edges=[]; const R=Math.min(W,H);
    ...
    const mR=R*0.20;
    MESH_MODELS.forEach((m,i)=>{ const ang=-Math.PI/2 + i*(2*Math.PI/MESH_MODELS.length);
    ...
    list.forEach((a,i)=>{ const ang=-Math.PI/2 + i*(2*Math.PI/list.length);
  ...
  function resize(){
    ...
    const r=wrap.getBoundingClientRect(); st.w=Math.max(220,r.width); st.h=Math.max(160,r.height);
  ...
  uEm(()=>{ resize(); const ro=new ResizeObserver(resize); if(wrapRef.current) ro.observe(wrapRef.current);
    const loop=()=>{ draw(); S.current.raf=requestAnimationFrame(loop); }; S.current.raf=requestAnimationFrame(loop);
    return ()=>{ cancelAnimationFrame(S.current.raf); ro.disconnect(); };
  // eslint-disable-next-line
  },[]);
  uEm(()=>{ build(); },[agents]);
  uEm(()=>{ S.current.focus=activeId; },[activeId]);
  uEm(()=>{ if(!window.JarvisMock) return; return window.JarvisMock.streamSub(evt=>{ if(evt&&evt.agent) fire(evt.agent, true); }); },[]);

  function draw(){
    const st=S.current, cv=canvasRef.current; if(!cv) return; const ctx=cv.getContext('2d');
    ctx.setTransform(st.dpr,0,0,st.dpr,0,0); st.tick++;
    const W=st.w, H=st.h, cx=st.cx, cy=st.cy;
    // fade previous frame → comet trails
    ctx.globalCompositeOperation='source-over';
    ctx.fillStyle='rgba(4,7,13,'+(calm?0.4:0.26)+')'; ctx.fillRect(0,0,W,H);
    // starfield + core bloom (additive)
    ...
    // position nodes (slow constellation rotation + breathing)
    const rot=calm?0:st.tick*(cinema?0.0011:0.0016);
    st.nodes.forEach(n=>{ if(n.kind==='core'){ n.x=cx; n.y=cy; n._r=n.r; return; }
    ...
    // edges (additive glow)
    st.edges.forEach(e=>{ const a=node(e.a), b=node(e.b); if(!a||!b) return;
    ...
    // particles (additive comets)
    st.particles = st.particles.filter(p=>p.life<1);
    st.particles.forEach(p=>{ p.life += calm?p.sp*0.4:p.sp; if(p.life<0) return; const a=node(p.e.a), b=node(p.e.b); if(!a||!b) return;
    ...
    ctx.globalAlpha=1;
    // node halos (additive)
    st.nodes.forEach(n=>{ const active=n.agent&&n.agent.status&&n.agent.status!=='idle'; const isHov=hov===n.id||foc===n.id;
      if(n.kind==='core'||active||isHov||n.kind==='model'){ const c=colorFor(n); const g=ctx.createRadialGradient(n.x,n.y,1,n.x,n.y,n._r+(n.kind==='core'?16:9));
    ...
    // node cores + arc-reactor (over)
    ctx.globalCompositeOperation='source-over';
    st.nodes.forEach(n=>{ const c=colorFor(n); const isHov=hov===n.id; const dim=(hov&&!isHov&&n.kind!=='core')||(foc&&foc!==n.id&&n.kind==='agent');
  ...
  function onMove(e){ const st=S.current, r=canvasRef.current.getBoundingClientRect(), mx=e.clientX-r.left, my=e.clientY-r.top;
    let hit=null, hd=15; st.nodes.forEach(n=>{ const d=Math.hypot(n.x-mx,n.y-my); if(d<hd){ hd=d; hit=n; } });
...
Object.assign(window, { NeuralMesh });
