...
O26-P3.2 HUD depth tail has an active Data Spaces assign/unassign branch in progress.
...
| O26-P3.2 HUD punch-list reconciliation/depth | 🟡 partial (#507 + active Data Spaces branch) | `docs/design/HUD_V2_REMAINING.md` now distinguishes shipped controls from real remaining work, and `hud-p3-2-reconciliation.test.ts` pins that shipped TTS/mic/cognition/trust and Console controls are not re-listed as missing. The active depth branch wires Data Spaces assign/unassign in `DataSpacesPanel`; remaining local controls are capability grants/check UI, room history drawer, current-mesh task fan, preferences/tweaks UI, and self-hosted fonts. |
