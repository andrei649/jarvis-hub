...
| AUD-14 email-port env-int | 🟡 in progress | `SMTP_PORT` and `IMAP_PORT` now use shared `env_int()` parsing instead of raw `int(os.environ...)` in `web.py`; focused env-config guard is green locally. |
