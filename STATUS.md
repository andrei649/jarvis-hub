...
0.45 High-Risk Automation Contracts now has its first two live adoptions: #529 moves `PaymentBroker` request/approval admissibility onto `PAYMENT_CONTRACT`, and #531 moves actionable Signal Layer recommendation queueing onto `SIGNAL_RECOMMENDATION_CONTRACT` before anything can enter the preview-only approval queue.
