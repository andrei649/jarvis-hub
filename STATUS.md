...
| AUD-14 log rotation env-int | 🟡 in progress (#604) | `JARVIS_LOG_MAX_MB` and `JARVIS_LOG_BACKUPS` now use shared `env_int()` parsing while preserving settings-DB fallback before hardcoded defaults; focused operability/env-config checks are green locally. |
