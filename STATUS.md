...
| TASK-3 channel ingress taint | 🟡 active/local-green (#590) | `Gateway.route()` now marks untrusted inbound channel messages with private `_inbound_meta` while preserving plain `str` handler text; `ChannelInboxStore` persists only public `tainted`, `taint_source`, and `injection_flags` fields; `Orchestrator.channel_handler()` strips private gateway metadata before outbound adapter sends. PR CI/merge pending. |
