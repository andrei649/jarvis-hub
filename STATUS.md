...
| AUD-14 LLM model-name config | 🟢 active/local-green | `agents/core/llm/model_config.py` centralizes default model IDs plus the `JARVIS_DEEP_MODEL` override; `HybridRouter` now reads the shared deep-model config per instance/detect and no longer owns the direct env read. New guard tests pin the override helper and router integration (+3). |
