...
0.45 High-Risk Automation Contracts now has its first live adoption in #529: `PaymentBroker` evaluates `PAYMENT_CONTRACT` for request and approval admissibility, preserving existing mandate denial codes while moving the gate onto the reusable contract layer. Current WIP in #531 extends the same pattern to Signal Layer recommendations: actionable recommendations evaluate `SIGNAL_RECOMMENDATION_CONTRACT` before they can enter the preview-only approval queue.
