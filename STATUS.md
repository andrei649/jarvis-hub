...
| LivingMemory re-projection embedder wiring | 🟡 in progress | `SchedulerService.run_memory_maintenance()` passes `orch.memory.embed` into `LivingMemory.reproject_stale()` when available, and re-projection serializes structured tier content deterministically before embedding. |
