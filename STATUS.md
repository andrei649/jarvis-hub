...
H18.16 mobile memory/notes parity is in draft PR #572 on `codex-mobile-memory-notes`: the native Memory tab reads `GET /memory` and `GET /api/notes`, rendering recent turns plus session notes without clear/save/rewrite controls.
