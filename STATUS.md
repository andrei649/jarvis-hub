...
| DailyReflector durable LivingMemory handoff | 🟡 in progress | `ReflectionRunStore` gives nightly reflection durable same-day idempotency across restarts, `/api/reflection/run` now uses `force=True` for manual reruns, and extracted lessons encode into LivingMemory/core only when `cognition.memory_enabled` is active. Tier records store lesson digest/length metadata, not raw conversation text. Focused reflection suite is 14 passed. |
