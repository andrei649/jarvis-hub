...
0.45 High-Risk Automation Contracts has three merged live adoptions: #529 moves `PaymentBroker` request/approval admissibility onto `PAYMENT_CONTRACT`, #531 moves actionable Signal Layer recommendation queueing onto `SIGNAL_RECOMMENDATION_CONTRACT`, and #533 moves plugin-known/enabled/agent/network checks onto `PLUGIN_CALL_CONTRACT`. The next slice is draft PR #535 on `codex-o45-social-draft-contract`: `SocialBroker.request()` evaluates `SOCIAL_DRAFT_CONTRACT` before preview/enqueue, so patched contract denials cannot enter the approval queue.
...
| 0.45 social draft contract gate | 🟡 draft PR #535 | `agents/core/social.py` now exposes `SOCIAL_DRAFT_CONTRACT`; valid X post/reply/DM drafts still queue through the ask-tier approval path, while a denied live contract decision returns a controlled reason before preview/enqueue. Local red/green: the new regression first proved patched contracts were ignored, then `tests/test_social_h12_21.py` passed (16 passed). |
