...
| R3-B2 memory/forget contracts | 🟡 draft PR (#582) | External KG writes now evaluate `KG_WRITE_CONTRACT` before kernel mediation/mutation; destructive `purge_data` and `/api/admin/forget` evaluate `DATA_PURGE_CONTRACT` before live-memory clear or at-rest deletion. |
