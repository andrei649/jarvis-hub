...
| LivingMemory recall + real eval mode | 🟡 in progress | `agents/core/memory/living_recall.py` post-fusion re-ranks only hits whose ids match LivingMemory `turn_ref`, annotates non-private provenance, then preserves the existing `rag_guard` prompt fence. `run_recall_eval()` ingests eval facts into a deterministic real `MemoryManager` and `/api/memory/eval/run?mode=recall` exposes the mode while keyword remains default. Focused red/green suite is 6 passed; adjacent memory/eval suite is 44 passed. |
