...
| AUD-14 call-config env-json | 🟡 in progress | `JARVIS_CALL_CONFIG` now uses shared `env_json_object()` parsing instead of local `json.loads(os.environ...)` in `AutonomyCoordinator.build_executor`; focused env-config/executor/call-broker sweep is green locally. |
