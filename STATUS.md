...
| R3-B4 MCP route-tool contracts | 🟡 draft PR #586 (local green) | Mutating MCP route tools now evaluate `MCP_MUTATING_ROUTE_CONTRACT` after identity succeeds and before Action Kernel mediation or the write adapter. Denied contracts audit `refused-contract`, return a controlled MCP tool error, and never call the route writer. CI pending. |
