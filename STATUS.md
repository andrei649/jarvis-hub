...
| LivingMemory core persistence | 🟡 in progress | `CoreMemory(path=...)` persists its bounded de-duplicated fact ring via `JsonStore`, `LivingMemory(core_path=...)` reloads it, and production Orchestrator registers LivingMemory with `memory_logs/cognition/core_memory.json` under `JARVIS_HOME`. Focused memory/consolidation suite is 26 passed. |
