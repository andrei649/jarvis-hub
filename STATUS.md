...
| AUD-14 CORS-origins env-list | 🟡 in progress | `JARVIS_CORS_ORIGINS` now uses shared `env_list()` parsing instead of local split/strip parsing in `web.py`; focused env-config/lifespan checks are green locally. |
