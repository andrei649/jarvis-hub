...
| AUD-14 channel-rates env-map | 🟡 in progress | `JARVIS_CHANNEL_SEND_RATES` now uses shared `env_int_map()` parsing instead of a module-local parser in `channels/send_rate_limit.py`; focused env-config/send-rate sweep is green locally. |
