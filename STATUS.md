...
| AUD-14 analytics max-events env-int | 🟡 in progress (#600) | `JARVIS_ANALYTICS_MAX_EVENTS` now uses shared `env_int(..., minimum=0)` instead of import-time `int(os.environ...)`, so malformed values fall back to the bounded default instead of crashing analytics import; focused analytics/env-config checks are green locally. |
