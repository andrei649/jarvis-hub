...
| AUD-14 vector-dimension env-int | 🟡 in progress | `VECTOR_DIMENSION` now uses shared `env_int(..., minimum=1)` parsing instead of raw `int(os.getenv...)` in `memory/manager.py`; focused memory embedding suite is green locally. |
