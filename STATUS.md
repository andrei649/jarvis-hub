...
| LivingMemory re-projection maintenance | 🟡 in progress (#559) | `LivingMemory.reproject_stale(embedder=...)` persists upgraded vector/embed-version fields for stale tier records, and `SchedulerService.run_memory_maintenance()` now includes a best-effort `reprojection` section. The production path stays default-off (`embedder_unavailable`) until a real embedder is wired. |
