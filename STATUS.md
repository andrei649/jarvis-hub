...
| LivingMemory core prompt injection | 🟡 in progress | `_build_agent_turn_text()` now receives a gated `[core memory]` block from `living.core.list()` even when vector recall is disabled. Facts are whitespace-normalized, capped per line, and labeled as background facts, not instructions. Focused LivingMemory suite is 8 passed. |
