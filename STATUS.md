...
| AUD-14 task-budget env-float | 🟡 in progress (#598) | `JARVIS_TASK_MAX_SECONDS` now uses shared `env_float(..., minimum=0.0)` instead of local try/except parsing in `AutonomyCoordinator.build_executor`; focused budget/env-config checks are green locally. |
