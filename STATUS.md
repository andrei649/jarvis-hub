...
| AUD-14 webhook-channels env-json | 🟡 in progress | `JARVIS_WEBHOOK_CHANNELS` now uses shared `env_json_object()` parsing instead of local `json.loads()` in `web.py`; focused env-config/lifespan checks are green locally. |
