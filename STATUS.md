...
> **Current version:** v0.11.0 (feature-complete + refactor done; productionizing toward 1.0) · **Tests:** ~4,636 collected (13 skipped; H30 final gate 2026-07-13) + frontend **227 vitest** + mobile **70 jest** + Playwright HUD/flow suites (HUD-v3 Console port complete: every blueprint surface + the north-star Decision Inbox + native Neural-Mesh canvas + cinema mode) · **Agents:** 17 active (16 cabinet incl. Howard + Argus, the WorldView bridge; + 17 bench) · **HTTP routes:** 382 (+ feedback widget/summary, H23.21; + onboarding, H23.20; + canonical `/api/capabilities`, H27.8; + `/api/metrics/capabilities`, V2; + `/api/security/loop-breaker` status/reset, K3; + `/api/metrics/kernel`, Gate-K observability) — `web.py` decomposed into **46 per-domain routers** (CLN-3 + House); only 9 app-shell/chat/admin routes remain inline (4,636→1,282 LOC)
...
| H28 governed operator completion | ✅ hermetic code complete; owner-live gate pending | Accessibility-first Windows actuation, proven-local fallback, durable human-approved ToolRPC→TaskExecutor→Action Kernel execution, and click/type/launch are wired default-off. The canonical S1 operator pack passes **7/7 hermetic production-rail cases** with measured `ungoverned_actions == 0`; wave-1 browser/desktop/grounding modules graduated from park policy while wave 2/3, `training/`, `rust/`, and self-protection remain frozen. Real Windows UIA and installed Playwright Chromium validation is still owner-host opt-in and is not reported as executed here. |
| H29 Media Director completion | ✅ merged (#674) | Real catalog id/query and governed-browser URL resolution, request-scoped K3 interruption budgets, explicit duration support/verification, and the controlled `image_gen`/`media_gen`/`media_skill` graduation are complete behind existing default-off guards. The five-case hermetic H29 reality pack measures `ungoverned_actions == 0`; browser Console and native mobile surfaces use the same guarded API, require nested verified truth for success, keep device mutation admin-gated, and embed no remote media. Final local evidence: **4,520** tests green in CI topology after excluding three reproduced baseline/host assertions (two Gemini cache worker-state leaks that pass serially on both branches, plus `test_sys_info_honest` rejecting this machine's real RTX 5090); the **261** focused backend/action/reality/parity gate, frontend **219 Vitest**, mobile **65 Jest**, both TypeScript checks, production HUD build, Ruff, Bandit, and status-sync are green. Real device drivers remain owner-host seams. |
| H30 House Brain completion | ✅ implementation complete; PR/CI gate in progress | Strict-local read-first Home Assistant state, a public/private bi-temporal house model, fail-closed presence, durable kernel-mediated actuation, one-shot strong confirmation, verified rollback, browser/native mobile surfaces, and authenticated room-aware voice are complete behind default-off guards. The canonical **7/7** hermetic H30 pack measures `ungoverned_actions == 0` across HA and H29 media host seams and deliberately fails an unapproved mutation; the optional read-only live probe is double owner-gated and reports missing setup as degraded. Final local gates: the **4,636-test** backend collection is green in exact CI topology after deselecting only the documented `test_sys_info_honest` host assertion for this machine's real RTX 5090; H30 reality **4**, H27 verification **8**, frontend **227 Vitest**, mobile **70 Jest**, both TypeScript checks, production HUD build, Ruff/C901/Bandit/diff-check/status-sync are green. Real HA/satellite/device execution remains owner-host validation. |
...
| H20.R1 Agent Runtime v2 Wave 1 | 🟡 draft-ready, default-OFF | LM Studio can now drive a bounded model→ToolRPC→model loop over the governed allowlist through the same `Agent.generate_response()` seam used by normal and streamed turns. The in-process fake-LM-Studio reality harness proves the disabled legacy request, enabled echo loop, unknown/gated denial, and exactly-once final persistence (4 cases green). Targeted Ruff and the 16-file compatibility gate exit 0; adversarial re-review is **Ready: Yes** with no Critical/Important findings after strict provider parsing, bounded valid-JSON results, reasoning-final parity, and hard fan-out remediation. The isolated serial suite completed with **3,991 passed, 7 skipped, 1 environment-sensitive baseline failure**: `test_sys_info_honest` rejects the real detected `NVIDIA GeForce RTX 5090 Laptop GPU`, reproduced unchanged on detached clean `origin/main`. This is the execution spine, **not Hermes parity**: governed file/process, browser/build, multimedia artifact, browser SSE lifecycle, cloud-provider tool calling, MCP discovery/execution, and model-directed delegation remain open. Draft-PR publication is pending. |
