...
H18.18 mobile security posture parity is in draft PR #576 on `codex-mobile-security-posture`: the native Status tab now has a read-only Trust card over governance, posture, kill-switch, and loop-breaker security reads without halt/reset/capability-write controls.
