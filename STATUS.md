...
0.45 High-Risk Automation Contracts now has six merged live adoptions: #529 moves `PaymentBroker` request/approval admissibility onto `PAYMENT_CONTRACT`, #531 moves actionable Signal Layer recommendation queueing onto `SIGNAL_RECOMMENDATION_CONTRACT`, #533 moves plugin-known/enabled/agent/network checks onto `PLUGIN_CALL_CONTRACT`, #535 moves governed social draft-before-send requests onto `SOCIAL_DRAFT_CONTRACT`, #537 moves governed Notion/GitHub/Calendar write-back drafts onto `WRITEBACK_DRAFT_CONTRACT`, and #539 moves outbound call requests onto `CALL_REQUEST_CONTRACT` before preview/enqueue. Active WIP #541 adds the same contract layer to gated Tool-RPC calls before kernel mediation and approval enqueue.
...
| 0.45 Tool-RPC contract gate | 🟡 draft (#541) | `agents/core/tool_rpc.py` now exposes `TOOL_RPC_CALL_CONTRACT`; gated Tool-RPC calls evaluate it after allowlist/args validation but before Action Kernel mediation and approval enqueue. Local red/green: the new regression first proved patched contracts were ignored, then passed; focused Tool-RPC/kernel/contracts/action-auth sweep, ruff, py_compile, and status sync are green. PR CI is pending. |
