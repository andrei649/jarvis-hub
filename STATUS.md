...
0.45 High-Risk Automation Contracts now has its first three live adoptions: #529 moves `PaymentBroker` request/approval admissibility onto `PAYMENT_CONTRACT`, #531 moves actionable Signal Layer recommendation queueing onto `SIGNAL_RECOMMENDATION_CONTRACT`, and current WIP in #533 moves plugin-known/enabled/agent/network checks onto `PLUGIN_CALL_CONTRACT`.
