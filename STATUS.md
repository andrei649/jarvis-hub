...
0.45 High-Risk Automation Contracts now has four merged live adoptions: #529 moves `PaymentBroker` request/approval admissibility onto `PAYMENT_CONTRACT`, #531 moves actionable Signal Layer recommendation queueing onto `SIGNAL_RECOMMENDATION_CONTRACT`, #533 moves plugin-known/enabled/agent/network checks onto `PLUGIN_CALL_CONTRACT`, and #535 moves governed social draft-before-send requests onto `SOCIAL_DRAFT_CONTRACT` before preview/enqueue. The next slice is draft PR #537 on `codex-o45-writeback-contract-gate`: governed Notion/GitHub/Calendar write-back drafts evaluate `WRITEBACK_DRAFT_CONTRACT` before preview/enqueue.
